"""
Daedalus MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Daedalus-specific extra)

Usage:
    daedalus mcp serve
    daedalus mcp serve --verbose
...
def _get_sessions_dir() -> Path:
    ...
    try:
        ...
        return get_daedalus_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("DAEDALUS_HOME", Path.home() / ".daedalus")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from daedalus_constants import get_daedalus_home
        directory_file = get_daedalus_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("DAEDALUS_HOME", Path.home() / ".daedalus")
...
class EventBridge:
    ...
    This is the Daedalus equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            ...
            db_file = Path(os.environ.get("DAEDALUS_HOME", Path.home() / ".daedalus")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    ...
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            ...
            "Install with: pip install 'daedalus[mcp]'"
        )

    mcp = FastMCP(
        "daedalus",
        instructions=(
            "Daedalus Agent messaging bridge. Use these tools to interact with "
