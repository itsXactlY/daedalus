...
**Self-versioning state snapshots for Daedalus Agent — content-addressed, WAL-protected, branch-safe.**

## Why Not Git?

Git is version control for **code**. Daedalus needs version control for **runtime state**.
...
Only Daedalus internal state — never the dev repo:
...
Excluded: `daedalus/` (git-managed), `venv/`, `sessions/`, `logs/`, `cache/`, `skills/`.
...
All hooks are debounced and try/except wrapped. Even if the snapshot engine completely fails, Daedalus continues unaffected.
...
The Runtime Snapshot Engine makes Daedalus a **self-healing system**:
