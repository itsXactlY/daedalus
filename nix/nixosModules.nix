# nix/nixosModules.nix — NixOS module for daedalus
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: daedalus runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $DAEDALUS_HOME/.env
# and read by daedalus at startup — no container recreation needed for env changes.
#
# Tool resolution: the daedalus wrapper uses --suffix PATH for nix store tools,
...
#     environmentFiles = [ config.sops.secrets."daedalus/env".path ];
#   };
#
{ inputs, ... }: {
  ...
  let
    ...
    daedalus = inputs.self.packages.${pkgs.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-daedalus)
    deepConfigType = lib.types.mkOptionType {
      name = "daedalus-config-attrs";
      description = "Daedalus YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "daedalus-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "daedalus-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'DAEDALUS_DOC_EOF'\n${value}\nDAEDALUS_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "daedalus";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/daedalus";
    ...
    # daedalus user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "daedalus-container-entrypoint" ''
      set -eu

      DAEDALUS_UID="''${DAEDALUS_UID:?DAEDALUS_UID must be set}"
      DAEDALUS_GID="''${DAEDALUS_GID:?DAEDALUS_GID must be set}"

      # ── Group: ensure a group with GID=$DAEDALUS_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$DAEDALUS_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="daedalus"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$DAEDALUS_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$DAEDALUS_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$DAEDALUS_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$DAEDALUS_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="daedalus"
        TARGET_HOME="/home/daedalus"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$DAEDALUS_UID" -g "$DAEDALUS_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$DAEDALUS_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$DAEDALUS_UID:$DAEDALUS_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure DAEDALUS_HOME is owned by the target user
      if [ -n "''${DAEDALUS_HOME:-}" ] && [ -d "$DAEDALUS_HOME" ]; then
        chown -R "$DAEDALUS_UID:$DAEDALUS_GID" "$DAEDALUS_HOME"
      fi
      ...
      if [ ! -f /var/lib/daedalus-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        echo "First boot: provisioning agent tools..."
        apt-get update -qq
        apt-get install -y -qq sudo nodejs npm curl
        touch /var/lib/daedalus-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/daedalus ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/daedalus
        chmod 0440 /etc/sudoers.d/daedalus
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$DAEDALUS_UID" --regid="$DAEDALUS_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $DAEDALUS_HOME/.env.
    ...
    # Default: /var/lib/daedalus/workspace → /data/workspace.
  ...
  in {
    options.services.daedalus = with lib; {
      enable = mkEnableOption "Daedalus Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = daedalus;
        description = "The daedalus package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/daedalus";
        description = "State directory. Contains .daedalus/ subdir (DAEDALUS_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Daedalus config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $DAEDALUS_HOME/.env at activation time.
          Daedalus reads this file on every startup via load_daedalus_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $DAEDALUS_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $DAEDALUS_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `daedalus gateway`.";
      };
      ...
      addToSystemPackages = mkOption {
        type = types.bool;
        default = false;
        description = "Add daedalus CLI to environment.systemPackages.";
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.daedalus.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.daedalus        0750 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."daedalus-setup" = lib.stringAfter [ "users" "setupSecrets" ] ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.daedalus
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.daedalus ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 0750 ${cfg.stateDir} ${cfg.stateDir}/.daedalus ${cfg.stateDir}/home ${cfg.workingDirectory}

          # Merge Nix settings into existing config.yaml.
          # Preserves user-added keys (skills, streaming, etc.); Nix keys win.
          # If configFile is user-provided (not generated), overwrite instead of merge.
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.daedalus/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.daedalus/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.daedalus/config.yaml
            chmod 0640 ${cfg.stateDir}/.daedalus/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.daedalus/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.daedalus/.managed
          chmod 0644 ${cfg.stateDir}/.daedalus/.managed

          # Seed auth file if provided
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.daedalus/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.daedalus/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.daedalus/auth.json
              fi
            ''}
          ''}
          ...
          # Daedalus reads $DAEDALUS_HOME/.env at startup via load_daedalus_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.daedalus/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0600 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'DAEDALUS_NIX_ENV_EOF'
${envFileContent}
DAEDALUS_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
                echo "" >> "$ENV_FILE"
                cat "${f}" >> "$ENV_FILE"
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.daedalus = {
          description = "Daedalus Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            DAEDALUS_HOME = "${cfg.stateDir}/.daedalus";
            DAEDALUS_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $DAEDALUS_HOME/.env by the activation script. load_daedalus_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${cfg.package}/bin/daedalus"
              "gateway"
            ] ++ cfg.extraArgs);
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.daedalus = {
          description = "Daedalus Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              DAEDALUS_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              DAEDALUS_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env DAEDALUS_UID="$DAEDALUS_UID" \
                --env DAEDALUS_GID="$DAEDALUS_GID" \
                --env DAEDALUS_HOME=${containerDataDir}/.daedalus \
                --env DAEDALUS_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/daedalus gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
