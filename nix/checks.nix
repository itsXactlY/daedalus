...
{ inputs, ... }: {
  perSystem = { pkgs, system, lib, ... }:
    let
      daedalus = inputs.self.packages.${system}.default;
      ...
      configKeys = pkgs.runCommand "daedalus-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${daedalusVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = lib.optionalAttrs pkgs.stdenv.hostPlatform.isLinux {
        ...
        package-contents = pkgs.runCommand "daedalus-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${daedalus}/bin/daedalus || (echo "FAIL: daedalus binary missing"; exit 1)
          test -x ${daedalus}/bin/daedalus || (echo "FAIL: daedalus binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${daedalus}/bin/daedalus version 2>&1 | grep -qi "daedalus" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "daedalus-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in daedalus daedalus daedalus-acp; do
            test -x ${daedalus}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "daedalus-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking daedalus --help ==="
          ${daedalus}/bin/daedalus --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${daedalus}/bin/daedalus --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "daedalus-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${daedalus}/share/daedalus/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${daedalus}/share/daedalus/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "DAEDALUS_BUNDLED_SKILLS" ${daedalus}/bin/daedalus || \
            (echo "FAIL: DAEDALUS_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: DAEDALUS_BUNDLED_SKILLS set in wrapper"
        ...
        # Verify DAEDALUS_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "daedalus-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(DAEDALUS_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking DAEDALUS_MANAGED guards ==="
          check_blocked "config set" ${daedalus}/bin/daedalus config set model foo
          check_blocked "config edit" ${daedalus}/bin/daedalus config edit
        ...
        in pkgs.runCommand "daedalus-config-roundtrip" {
          ...
          merge_and_load() {
            local daedalus_home="$1"
            export DAEDALUS_HOME="$daedalus_home"
            ${configMergeScript} ${nixSettings} "$daedalus_home/config.yaml"
            ${daedalusVenv}/bin/python3 -c '
import json, sys
