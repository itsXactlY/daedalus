#!/usr/bin/env python3
"""
SQLite State Store for Daedalus Agent.
...
from daedalus_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Tuple, TypeVar

from daedalus_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from daedalus_state_portability import SessionPortabilityMixin
from daedalus_state_schema import SessionSchemaMixin
from daedalus_state_search import SessionSearchMixin
...
def _configured_transcript_limit(key: str, fallback: int) -> int:
    ...
    """
    try:
        from daedalus_cli.config import load_config_readonly
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``daedalus -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_daedalus_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.daedalus`` even when a test fixture later
    redirects ``DAEDALUS_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_daedalus_home()`` fresh so a runtime
       ``DAEDALUS_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_daedalus_home() / "state.db"
...
# ~/.daedalus/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects DAEDALUS_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without DAEDALUS_HOME, a stale worktree
# without the re-pin, or a developer shell that exports DAEDALUS_HOME to the
...
#: ``~/.daedalus``).  The test conftest injects the pre-sandbox production
#: root here so custom-``DAEDALUS_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Daedalus root for the guard.

    Deliberately avoids ``Path.home()`` / ``daedalus_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``daedalus_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "daedalus"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "daedalus"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".daedalus"
        return root.resolve()
    except Exception:
        return None
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Daedalus home *root*.
    ...
    under ``~/.daedalus/daedalus/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``DAEDALUS_HOME``) paths.
    """
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Daedalus root {root}). "
                "Tests must run against a temporary DAEDALUS_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "DAEDALUS_HOME. If this test genuinely needs the live "
...
# daedalus_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``daedalus_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``daedalus update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Daedalus-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Daedalus"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Daedalus-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `daedalus doctor`. This warning fires once per "
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see daedalus_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with daedalus_cli.config.
        from daedalus_cli.config import cfg_get, load_config_readonly
...
def _backup_db_file(db_path: Path) -> Optional[Path]:
    ...
    connection's POSIX advisory locks (see ``daedalus_cli.sqlite_safe_read``).
    ...
    try:
        from daedalus_cli.sqlite_safe_read import has_live_connection
    except ImportError:
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Daedalus home tree (``get_daedalus_home()``) — the safe repair scope:
      Daedalus owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``daedalus_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_daedalus_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Daedalus needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (daedalus_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_daedalus_state.py:600-632)
        ...
        probe_session_id = f"_daedalus_fts_health_probe_{time.time_ns()}"
        try:
...
# (``~/.daedalus/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``daedalus sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("DAEDALUS_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_daedalus_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("DAEDALUS_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship daedalus_state without daedalus_cli). A
    ...
    """
    try:
        from daedalus_cli.sqlite_safe_read import connect_tracked
    except ImportError:
        logger.debug(
            "daedalus_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch daedalus_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``daedalus_cli.backup.is_zeroed_sqlite_file`` when available; this
    local copy keeps SessionDB openable without importing the CLI package
    in constrained embed paths.
    """
    try:
        from daedalus_cli.backup import is_zeroed_sqlite_file
    ...
    if size <= 0:
        return False
    from daedalus_cli.sqlite_safe_read import read_header_bytes_preopen
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `daedalus snapshot list` / "
                "`daedalus snapshot restore <id>`.",
                path,
            )
            return None
...
# ── Read-only health/stats probes (daedalus doctor, dashboards) ──────────
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple daedalus processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Daedalus
    ...
    # `daedalus update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `daedalus snapshot list` / "
                    f"`daedalus snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`daedalus db optimize`),
            ...
            # ``daedalus_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `daedalus update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `daedalus sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Daedalus process held the "
    ...
    #
    # `optimize_fts_storage()` (the `daedalus sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`daedalus sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``daedalus sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``daedalus --resume`` into a fresh
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: daedalus_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``daedalus -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``daedalus sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``DAEDALUS_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Daedalus, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Daedalus session.

        A Daedalus session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Daedalus session is already bound to any Telegram DM topic.
