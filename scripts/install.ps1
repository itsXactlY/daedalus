# ============================================================================
# Daedalus Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   irm <YOUR_DAEDALUS_REPO_RAW_URL>/scripts/install.ps1 | iex
#
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$DaedalusHome = "$env:LOCALAPPDATA\daedalus",
    [string]$InstallDir = "$env:LOCALAPPDATA\daedalus\daedalus"
)
...
$RepoUrlSsh = "git@github.com:<YOUR_DAEDALUS_REPO>.git"
$RepoUrlHttps = "https://github.com/<YOUR_DAEDALUS_REPO>.git"
$PythonVersion = "3.14"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ Daedalus Agent Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
...
function Test-Node {
    ...
    $managedNode = "$DaedalusHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$DaedalusHome\node;$env:Path"
        Write-Success "Node.js $version found (Daedalus-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.daedalus/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\daedalus-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$DaedalusHome\node") { Remove-Item -Recurse -Force "$DaedalusHome\node" }
                Move-Item $extractedDir.FullName "$DaedalusHome\node"
                $env:Path = "$DaedalusHome\node;$env:Path"

                $version = & "$DaedalusHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.daedalus/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/<YOUR_DAEDALUS_REPO>/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\daedalus-$Branch.zip"
                $extractPath = "$env:TEMP\daedalus-extract"
...
function Set-PathVariable {
    Write-Info "Setting up daedalus command..."
    
    if ($NoVenv) {
        $daedalusBin = "$InstallDir"
    } else {
        $daedalusBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so daedalus is globally available
    # On Windows, the daedalus.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$daedalusBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$daedalusBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $daedalusBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set DAEDALUS_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\daedalus instead
    # of the Unix default ~/.daedalus
    $currentDaedalusHome = [Environment]::GetEnvironmentVariable("DAEDALUS_HOME", "User")
    if (-not $currentDaedalusHome -or $currentDaedalusHome -ne $DaedalusHome) {
        [Environment]::SetEnvironmentVariable("DAEDALUS_HOME", $DaedalusHome, "User")
        Write-Success "Set DAEDALUS_HOME=$DaedalusHome"
    }
    $env:DAEDALUS_HOME = $DaedalusHome
    
    # Update current session
    $env:Path = "$daedalusBin;$env:Path"
    
    Write-Success "daedalus command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.daedalus directory structure
    New-Item -ItemType Directory -Force -Path "$DaedalusHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DaedalusHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DaedalusHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DaedalusHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DaedalusHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DaedalusHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DaedalusHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DaedalusHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DaedalusHome\skills" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DaedalusHome\whatsapp\session" | Out-Null
    
    # Create .env
    $envPath = "$DaedalusHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.daedalus/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.daedalus/.env"
        }
    } else {
        Write-Info "~/.daedalus/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$DaedalusHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.daedalus/config.yaml from template"
        }
    } else {
        Write-Info "~/.daedalus/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$DaedalusHome\SOUL.md"
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.daedalus/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.daedalus/"
    
    # Seed bundled skills into ~/.daedalus/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.daedalus/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.daedalus/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$DaedalusHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.daedalus/skills/"
            }
        }
    }
}
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m daedalus_cli.main setup
    } else {
...
function Start-GatewayIfConfigured {
    ...
    $daedalusCmd = "$InstallDir\venv\Scripts\daedalus.exe"
    if (-not (Test-Path $daedalusCmd)) {
        $daedalusCmd = "daedalus"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$DaedalusHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'daedalus whatsapp' to pair via QR code..."
        Write-Host ""
        $response = Read-Host "Pair WhatsApp now? [Y/n]"
        if ($response -eq "" -or $response -match "^[Yy]") {
            try {
                & $daedalusCmd whatsapp
            } catch {
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$DaedalusHome\logs\gateway.log"
            Start-Process -FilePath $daedalusCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$DaedalusHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: daedalus gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: daedalus gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$DaedalusHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$DaedalusHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$DaedalusHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$DaedalusHome\daedalus\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   daedalus              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   daedalus setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   daedalus config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   daedalus config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   daedalus gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   daedalus update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri '<YOUR_DAEDALUS_REPO_RAW_URL>/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
