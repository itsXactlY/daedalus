#!/bin/bash
# ============================================================================
# Daedalus Agent Installer
# ============================================================================
# Installation script for Linux and macOS.
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/NousResearch/daedalus/main/scripts/install.sh | bash
#
# Or with options:
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/daedalus.git"
REPO_URL_HTTPS="https://github.com/NousResearch/daedalus.git"
DAEDALUS_HOME="$HOME/.daedalus"
INSTALL_DIR="${DAEDALUS_INSTALL_DIR:-$DAEDALUS_HOME/daedalus}"
PYTHON_VERSION="3.14"
NODE_VERSION="22"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.daedalus/daedalus)"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/NousResearch/daedalus/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$DAEDALUS_HOME/node/bin/node" ]; then
        export PATH="$DAEDALUS_HOME/node/bin:$PATH"
        local found_ver=$("$DAEDALUS_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Daedalus-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.daedalus/node/..."
    ...
    # Place into ~/.daedalus/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$DAEDALUS_HOME/node"
    mkdir -p "$DAEDALUS_HOME"
    mv "$extracted_dir" "$DAEDALUS_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$DAEDALUS_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$DAEDALUS_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$DAEDALUS_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$DAEDALUS_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$DAEDALUS_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.daedalus/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Daedalus Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="daedalus-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Daedalus behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up daedalus command..."

    if [ "$USE_VENV" = true ]; then
        DAEDALUS_BIN="$INSTALL_DIR/venv/bin/daedalus"
    else
        DAEDALUS_BIN="$(which daedalus 2>/dev/null || echo "")"
        if [ -z "$DAEDALUS_BIN" ]; then
            log_warn "daedalus not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$DAEDALUS_BIN" ]; then
        log_warn "daedalus entry point not found at $DAEDALUS_BIN"
    ...
    mkdir -p "$HOME/.local/bin"
    ln -sf "$DAEDALUS_BIN" "$HOME/.local/bin/daedalus"
    log_success "Symlinked daedalus → ~/.local/bin/daedalus"
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Daedalus Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
    ...
    # Export for current session so daedalus works immediately
    export PATH="$HOME/.local/bin:$PATH"

    log_success "daedalus command ready"
}

copy_config_templates() {
    log_info "Setting up configuration files..."

    # Create ~/.daedalus directory structure (config at top level, code in subdir)
    mkdir -p "$DAEDALUS_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.daedalus/.env (top level, easy to find)
    if [ ! -f "$DAEDALUS_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$DAEDALUS_HOME/.env"
            log_success "Created ~/.daedalus/.env from template"
        else
            touch "$DAEDALUS_HOME/.env"
            log_success "Created ~/.daedalus/.env"
        fi
    else
        log_info "~/.daedalus/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.daedalus/config.yaml (top level, easy to find)
    if [ ! -f "$DAEDALUS_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$DAEDALUS_HOME/config.yaml"
            log_success "Created ~/.daedalus/config.yaml from template"
        fi
    else
        log_info "~/.daedalus/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$DAEDALUS_HOME/SOUL.md" ]; then
        cat > "$DAEDALUS_HOME/SOUL.md" << 'SOUL_EOF'
# Daedalus Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Daedalus communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.daedalus/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.daedalus/"

    # Seed bundled skills into ~/.daedalus/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.daedalus/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.daedalus/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$DAEDALUS_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$DAEDALUS_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.daedalus/skills/"
        fi
    fi
}
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ...
            *)
                ...
                log_info "This is standard Playwright setup — Daedalus itself does not require root access."
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'daedalus setup' after install."
        return 0
    fi
    ...
    # Run daedalus setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m daedalus_cli.main setup < /dev/tty
    else
        python -m daedalus_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$DAEDALUS_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Daedalus to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$DAEDALUS_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'daedalus whatsapp' to pair via QR code..."
            echo ""
            read -p "Pair WhatsApp now? [Y/n] " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
                DAEDALUS_CMD="$HOME/.local/bin/daedalus"
                [ ! -x "$DAEDALUS_CMD" ] && DAEDALUS_CMD="daedalus"
                $DAEDALUS_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'daedalus whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'daedalus gateway install' later."
        return 0
    fi
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        DAEDALUS_CMD="$HOME/.local/bin/daedalus"
        if [ ! -x "$DAEDALUS_CMD" ]; then
            DAEDALUS_CMD="daedalus"
        fi

        if command -v systemctl &> /dev/null; then
            log_info "Installing systemd service..."
            if $DAEDALUS_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $DAEDALUS_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: daedalus gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: daedalus gateway"
            fi
        else
            log_info "systemd not available — starting gateway in background..."
            nohup $DAEDALUS_CMD gateway > "$DAEDALUS_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.daedalus/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: daedalus gateway"
        fi
    else
        log_info "Skipped. Start the gateway later with: daedalus gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.daedalus/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.daedalus/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.daedalus/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.daedalus/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.daedalus/daedalus/"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}daedalus${NC}              Start chatting"
    echo -e "   ${GREEN}daedalus setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}daedalus config${NC}       View/edit configuration"
    echo -e "   ${GREEN}daedalus config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}daedalus gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}daedalus update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    echo -e "${YELLOW}⚡ Reload your shell to use 'daedalus' command:${NC}"
    echo ""
    LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
