...
DEFAULT_CONFIG = {
    ...
    # SQLite journal mode used by every Daedalus database opener. WAL is the
    ...
    "agent": {
        ...
        # the Daedalus-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Daedalus
        ...
        # identity slot (SOUL.md). Empty by default. The DAEDALUS_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Daedalus adds a coding operating brief
        ...
        # ``DAEDALUS_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             DAEDALUS_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force DAEDALUS_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Daedalus auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Daedalus sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Daedalus
        ...
        #  "/home/user/.daedalus/cache/documents:/output"]
        ...
        # bundled Daedalus image, which drops to the `daedalus` user via
    ...
    "browser": {
        ...
        "camofox": {
            ...
            # app owns the visible browser and Daedalus should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``daedalus chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.daedalus/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: daedalus sweeps the checkpoint base at startup
        ...
        # ``daedalus checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .daedalus.md, or .cursorrules before Daedalus applies
    ...
    # Single-query (``daedalus -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Daedalus truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Daedalus' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); daedalus =
                                      # Daedalus' compression threshold triggers
    ...
    "auxiliary": {
        ...
        # Invoked by ``daedalus kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``daedalus kanban decompose`` and the kanban
        ...
        # ``daedalus profile describe <name> --auto`` and the dashboard's
    ...
    "display": {
        ...
        # Which interface bare `daedalus` (and `daedalus chat`) launches by default:
        ...
        # REPL and `--tui` (or DAEDALUS_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `daedalus --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `daedalus -c` muscle memory.  Default off so existing
        # users aren't surprised.  DAEDALUS_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `daedalus --tui` drops a one-time hint
        ...
        # model — see daedalus_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to DAEDALUS_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/daedalus`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `daedalus pets`. Disabled until
        ...
        "pet": {
            ...
            # get_daedalus_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``DAEDALUS_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``DAEDALUS_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``DAEDALUS_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``DAEDALUS_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``DAEDALUS_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/daedalus``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Daedalus appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.daedalus/cache/piper-voices/
    ...
    # "Hey Daedalus" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey daedalus",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_daedalus" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.daedalus/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # Give subagents the memory-provider tool surface (mazemaker_* tools
        # from memory.provider) by NOT skipping the memory manager. Default
        # False keeps children isolated (skip_memory=True) for normal
        # delegations; enable for router models like Alice-Router whose whole
        # training surface IS the mazemaker_* tool set.
        "memory_tools": False,
        # Route mazemaker_* tool calls (recall/think/get/...) through the
        # delegation model instead of calling the pod directly. When the main
        # model (gateway/Discord "The Architect") requests a mazemaker_* tool,
        # the call is delegated to the router (ALICE mode, memory_tools=True),
        # which picks the exact tool + args from its fine-tuned training and
        # returns a Kontext-Waechter balance line. Default OFF — direct pod
        # calls keep working until the operator turns routing on.
        ...
        # trim, the full text is spilled to ~/.daedalus/cache/delegation/
    ...
    # response. If not, Daedalus feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Daedalus auto-pauses the goal and
    ...
    "moa": {
        ...
        # <daedalus_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    "skills": {
        ...
        # Substitute ${DAEDALUS_SKILL_DIR} and ${DAEDALUS_SESSION_ID} in SKILL.md
        ...
        "write_approval": False,
        # Auto-route: at each turn the harness deterministically matches the
        # user's message against skill names/descriptions and injects the
        # top matches into the API-bound user message (never persisted, never
        # touches the cached system prompt). This restores skill autonomy the
        # name-only skills index took away — the operator no longer has to
        # command which skill to use; the harness surfaces the candidates and
        # the model reads the full SKILL.md via skill_view for the winner.
        #   false — disable entirely (back to pure LLM-compliance selection)
        "auto_route": True,
        # How many matches to surface per turn.
        "auto_route_top_n": 5,
        # Minimum token-overlap score (0..1) before a skill is surfaced.
        # Raise to cut noise, lower to surface more candidates.
    ...
    # cron daemon.
    #
    # See `daedalus curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `daedalus curator run --consolidate` overrides
        ...
        # normally restored on every `daedalus update`, so pruning them only
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (DAEDALUS_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Daedalus' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.daedalus/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or DAEDALUS_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `daedalus doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``daedalus_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Daedalus to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $DAEDALUS_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via DAEDALUS_CRON_MAX_PARALLEL env var.
        ...
        # DAEDALUS_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `daedalus -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `daedalus` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `daedalus kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     daedalus's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Daedalus tools (terminal, read_file, write_file, patch,
    ...
    # update model picker lists without shipping a daedalus release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://hermes.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `daedalus model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal DAEDALUS_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.daedalus/sessions/sessions.json entirely.
        ...
        # the DAEDALUS_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.daedalus.gateway` that defenses 1-2 don't
        ...
        # ``DAEDALUS_GATEWAY_MAX_STARTS`` / ``DAEDALUS_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.daedalus/.env,
        ...
        # behavior: files must live under the Daedalus cache, under
        ...
        # user. Bridged to DAEDALUS_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Daedalus
        # cache (~/.daedalus/cache/{documents,images,audio,video,screenshots})
        ...
        # to DAEDALUS_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # DAEDALUS_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to DAEDALUS_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.daedalus/state.db.
    ...
    "sessions": {
        ...
        # the default of ``daedalus sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.daedalus/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `daedalus sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `daedalus update` prints a one-line notice with
        ...
        # ~/.daedalus/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to DAEDALUS_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # DAEDALUS_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    },
    ...
    "updates": {
        ...
        #     DBs) into <DAEDALUS_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``daedalus backup``-style zip
        #     of DAEDALUS_HOME into <DAEDALUS_HOME>/backups/, restorable with
        #     ``daedalus import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `daedalus update` does with uncommitted local changes to the
        ...
        # Refresh an already-installed cua-driver during `daedalus update`.
    ...
    "lsp": {
        ...
        #              ``<DAEDALUS_HOME>/lsp/bin/`` on first use.
    ...
    # `daedalus tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.daedalus/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.daedalus/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.daedalus/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Daedalus may use this encrypted
            ...
            # ~/.daedalus/bin/ on first use.  When False you must install
            ...
            # `daedalus secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.daedalus/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Daedalus disables it for our users unless they
        # explicitly opt in here. When false (default), Daedalus sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Daedalus also calls
    ...
    #
    # Configure with `daedalus egress setup`.  Disabled by default — the rest of
    # Daedalus works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.daedalus/bin/ on
    ...
    # Daedalus Desktop (Electron app) launch options. These only affect
    # `daedalus desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the DAEDALUS_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
...
OPTIONAL_ENV_VARS = {
    ...
    # DAEDALUS_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to DAEDALUS_TOOL_PROGRESS_MODE for backward
    ...
    # DAEDALUS_TOOL_PROGRESS is fully unsupported since the v12 config support
