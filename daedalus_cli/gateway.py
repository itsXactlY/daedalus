"""
Gateway subcommand for daedalus CLI.

Handles: daedalus gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "daedalus-gateway*",
...
def find_gateway_pids(exclude_pids: set | None = None) -> list:
    ...
    patterns = [
        "daedalus_cli.main gateway",
        "daedalus_cli/main.py gateway",
        "daedalus gateway",
        "gateway/run.py",
    ]
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (DAEDALUS_HOME-scoped).
...
def _profile_suffix() -> str:
    """Derive a service-name suffix from the current DAEDALUS_HOME.

    Returns ``""`` for the default ``~/.daedalus``, the profile name for
    ``~/.daedalus/profiles/<name>``, or a short hash for any other custom
    DAEDALUS_HOME path.
    """
    import hashlib
    import re
...
def _profile_arg(daedalus_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when DAEDALUS_HOME is a named profile.

    For ``~/.daedalus/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        daedalus_home: Optional explicit DAEDALUS_HOME path. Defaults to the current
            ``get_daedalus_home()`` value. Should be passed when generating a
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this DAEDALUS_HOME.

    Default ``~/.daedalus`` returns ``daedalus-gateway`` (backward compatible).
    Profile ``~/.daedalus/profiles/coder`` returns ``daedalus-gateway-coder``.
    Any other DAEDALUS_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    daedalus gateway uninstall")
    print_info("    sudo daedalus gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Daedalus can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo daedalus gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo daedalus gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo daedalus gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.daedalus`` → ``ai.daedalus.gateway.plist`` (backward compatible).
    Profile ``~/.daedalus/profiles/coder`` → ``ai.daedalus.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.daedalus.gateway-{suffix}" if suffix else "ai.daedalus.gateway"
...
def _daedalus_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current DAEDALUS_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_daedalus_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/<home>                     → /home/alice/<home>
      /root/<home>/profiles/coder      → /home/alice/<home>/profiles/coder
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        daedalus_home = _daedalus_home_for_target_user(home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m daedalus_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="DAEDALUS_HOME={daedalus_home}"
Restart=on-failure
RestartSec=30
KillMode=mixed
KillSignal=SIGTERM
...
"""
    ...
    daedalus_home = str(get_daedalus_home().resolve())
    profile_arg = _profile_arg(daedalus_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Daedalus install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}daedalus gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}daedalus gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}daedalus gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}daedalus gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}daedalus gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.daedalus.gateway-{suffix}" if suffix else "ai.daedalus.gateway"
...
def generate_launchd_plist() -> str:
    ...
    working_dir = str(PROJECT_ROOT)
    daedalus_home = str(get_daedalus_home().resolve())
    log_dir = get_daedalus_home() / "logs"
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>daedalus_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>DAEDALUS_HOME</key>
        <string>{daedalus_home}</string>
    </dict>
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  daedalus gateway status             # Check status")
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Daedalus install")
    else:
        print("⚠ Service definition is stale relative to the current Daedalus install")
        print("  Run: daedalus gateway start")
    ...
    if deep:
        log_file = get_daedalus_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Daedalus Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_daedalus_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'daedalus pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: daedalus pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'daedalus gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from daedalus_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"DaedalusAgent\"")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: daedalus gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: daedalus gateway")
    else:
        print()
        print_info("No platforms configured. Run 'daedalus gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: daedalus gateway run")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if is_linux():
                ...
                if linger_ok is False:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print("    daedalus gateway restart")
                    return
        ...
        if is_linux() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print("  daedalus gateway install")
                print("  sudo daedalus gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  daedalus gateway          # Run in foreground")
                print("  daedalus gateway install  # Install as user service")
                print("  sudo daedalus gateway install --system  # Install as boot-time system service")
