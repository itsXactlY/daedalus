"""daedalus webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    daedalus webhook subscribe <name> [options]
    daedalus webhook list
    daedalus webhook remove <name>
    daedalus webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.daedalus/webhook_subscriptions.json and are
...
def _save_subscriptions(subs: Dict[str, dict]) -> None:
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as fh:
            fh.write(json.dumps(subs, indent=2, ensure_ascii=False))
            fh.flush()
            os.fsync(fh.fileno())
...
def _setup_hint() -> str:
  ...
  Then start the gateway: daedalus gateway run
"""
...
def webhook_command(args):
    """Entry point for 'daedalus webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: daedalus webhook {subscribe|list|remove|test}")
        print("Run 'daedalus webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (daedalus gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: daedalus webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from daedalus webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (daedalus gateway run)")
