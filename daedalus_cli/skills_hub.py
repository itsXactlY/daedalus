#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the Daedalus Skills Hub.

Powers both:
  - `daedalus skills <subcommand>` (CLI argparse entry point)
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    c.print(table)
    c.print("[dim]Use: daedalus skills inspect <identifier> to preview, "
            "daedalus skills install <identifier> to install[/]\n")
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                "It ships with daedalus but is not activated by default.\n"
                "Installing will copy it to your skills directory where the agent can use it.\n\n"
                f"Files will be at: [cyan]{display_daedalus_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="daedalus skills install <id> to install"))
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: daedalus skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: daedalus skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    "Usage: daedalus skills publish <path> --to github --repo owner/repo\n")
            return

        auth = GitHubAuth()
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_daedalus_home()}/.env or run 'gh auth login'.\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via Daedalus Skills Hub.\n\n"
                        f"This skill was scanned by the Daedalus Skills Guard before submission.",
...
DEFAULT_SKILLS_REPO = "itsXactlY/daedalus"


def do_sync(ref: str = "main", repo: str = DEFAULT_SKILLS_REPO,
            dry_run: bool = False, prune: bool = False) -> None:
    """Refresh the BUNDLED skills from the source repository.

    Distinct from `skills update`, which upgrades individually hub-installed
    skills. This one replaces the 100-odd curated skills that ship with the
    distribution, and exists because a packaged install never receives them:
    they are SKILL.md files under a top-level skills/ directory, not Python
    package data, so the wheel carries none of them. A pip/uvx-installed agent
    — which is what runs inside the pod — comes up reporting "No skills
    installed" while the repository holds every one of them.

    `daedalus update` cannot stand in for this. It resolves PROJECT_ROOT, which
    on a packaged install is site-packages and not a git checkout, so its git
    path is unavailable and its zip fallback would unpack the entire repository
    over site-packages — putting skills nowhere the agent looks and leaving the
    rest of the tree behind as debris.

    Only skills/** is extracted, straight into get_bundled_skills_dir().
    """
    import io
    import shutil
    import tarfile
    ...
    from daedalus_constants import get_bundled_skills_dir

    dest = Path(get_bundled_skills_dir())
    url = f"https://github.com/{repo}/archive/refs/heads/{ref}.tar.gz"

    print(f"→ {repo}@{ref} → {dest}")
    try:
    ...
    seen: set[str] = set()
    tmp = Path(tempfile.mkdtemp(prefix="daedalus-skills-"))
    try:
        with tarfile.open(fileobj=io.BytesIO(blob), mode="r:gz") as tf:
            for m in tf.getmembers():
                # <repo>-<ref>/skills/... — keep only the skills subtree.
                ...
                if not rel:
                    continue
                # Refuse anything that escapes the destination once joined.
                target = (dest / rel).resolve()
                if not str(target).startswith(str(dest.resolve())):
                    print(f"  ! skipped (path escape): {m.name}")
                    continue
                if m.isdir():
                    continue
                if not (m.isfile() or m.issym()):
                    continue  # no devices, no hardlinks
                ...
                if out.exists() and out.read_bytes() == data:
                    ...
                    continue
                if dry_run:
                    print(f"  {'~' if out.exists() else '+'} {rel}")
                    if out.exists():
                        updated += 1
                    else:
                        added += 1
                    continue
                # Decide new-vs-changed BEFORE writing: checking afterwards
                # always sees the file we just created, so a first sync
                # reported every skill as "changed" and none as new.
                existed = out.exists()
    ...
    removed = 0
    if prune and not dry_run and dest.exists():
        # Off by default: the same directory is where hub-installed and
        # hand-written skills live, and a sync is not a mandate to delete work
        # the repository never knew about.
        for f in sorted(dest.rglob("*"), reverse=True):
            if f.is_file() and str(f.relative_to(dest)) not in seen:
                f.unlink()
                removed += 1
            elif f.is_dir() and not any(f.iterdir()):
                f.rmdir()

    total = len(list(dest.rglob("SKILL.md"))) if dest.exists() else 0
    verb = "would write" if dry_run else "wrote"
    print(f"✓ {verb} {added} new, {updated} changed, {unchanged} already current"
          + (f", {removed} pruned" if removed else ""))
    print(f"  {total} SKILL.md now in {dest}")
...
def skills_command(args) -> None:
    ...
    if action == "browse":
        ...
        do_sync(
            ...
            repo=getattr(args, "repo", DEFAULT_SKILLS_REPO),
        ...
        if snap_action == "export":
            ...
            _console.print("Usage: daedalus skills snapshot [export|import]\n")
    elif action == "tap":
        tap_action = getattr(args, "tap_action", None)
        repo = getattr(args, "repo", "") or getattr(args, "name", "")
        if not tap_action:
            _console.print("Usage: daedalus skills tap [list|add|remove]\n")
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print("Usage: daedalus skills [browse|search|install|inspect|list|check|update|audit|uninstall|publish|snapshot|tap]\n")
