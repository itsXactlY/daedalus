"""``daedalus plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.daedalus/plugins/``.
...
def _plugins_dir() -> Path:
    ...
    plugins = get_daedalus_home() / "plugins"
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_daedalus_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_daedalus_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_daedalus_home()}/.env later)[/dim]")
...
def cmd_install(identifier: str, force: bool = False) -> None:
        ...
        if target.exists():
            if not force:
                console.print(
                    ...
                    f"[bold]daedalus plugins update {plugin_name}[/bold] to pull latest."
                )
                sys.exit(1)
    ...
    console.print("[dim]  daedalus gateway restart[/dim]")
    console.print()
...
def cmd_list() -> None:
    ...
    if not dirs:
        ...
        console.print("[dim]Install with:[/dim] daedalus plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] daedalus plugins")
    console.print("[dim]Enable/disable:[/dim] daedalus plugins enable/disable <name>")
...
def plugins_command(args) -> None:
    """Dispatch daedalus plugins subcommands."""
