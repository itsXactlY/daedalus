"""daedalus claw — OpenClaw migration commands.

Usage:
    daedalus claw migrate              # Interactive migration from ~/.openclaw
    daedalus claw migrate --dry-run    # Preview what would be migrated
    daedalus claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    daedalus claw cleanup              # Archive leftover OpenClaw directories
    daedalus claw cleanup --dry-run    # Preview what would be archived
"""
...
from daedalus_cli.config import get_daedalus_home, get_config_path, load_config, save_config
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_daedalus.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_daedalus", script_path)
...
def claw_command(args):
    """Route daedalus claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: daedalus claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Daedalus")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'daedalus claw <command> --help' for options.")
...
def _cmd_migrate(args):
    ...
    print_info(f"Target:      {daedalus_home}")
...
def _offer_source_archival(source_dir: Path, auto_yes: bool = False):
    ...
    if auto_yes or prompt_yes_no(f"Archive {source_dir} now?", default=True):
        ...
        print_info("Skipped. You can archive later with: daedalus claw cleanup")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  daedalus claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  daedalus claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  daedalus config set OPENROUTER_API_KEY sk-or-v1-...")
