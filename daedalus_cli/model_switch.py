...
- ``daedalus_cli.providers``        -- canonical provider identity + overlays
- ``daedalus_cli.model_normalize``  -- per-provider name formatting
...
from daedalus_cli.providers import (
    determine_api_mode,
    get_label,
    is_aggregator,
    resolve_provider_full,
)
from daedalus_cli.model_normalize import (
...
_DAEDALUS_MODEL_WARNING = (
    ...
    "for use with Daedalus Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)


def _check_daedalus_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* looks like a Daedalus LLM model."""
    if "daedalus" in model_name.lower():
        return _DAEDALUS_MODEL_WARNING
    return ""
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'daedalus doctor' — config issues detected:"
    ...
    daedalus_warn = _check_daedalus_model_warning(new_model)
    if daedalus_warn:
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from daedalus_cli/models.py (OPENROUTER_MODELS,
    ...
    from daedalus_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    if not cust_base or "openrouter.ai" in cust_base:
        return CustomAutoResult(
            ...
            error_message=(
                ...
                "in .env, or run: daedalus setup -> Custom OpenAI-compatible endpoint"
            ),
        )
