"""Slash command definitions and autocomplete for the Daedalus CLI.
...
def is_interrupt_then_dispatch(command_name: str | None) -> bool:
    ...
    return cmd is not None and cmd.name in _INTERRUPT_THEN_DISPATCH_COMMANDS


def should_bypass_active_session(command_name: str | None) -> bool:
    """Return True for any resolvable slash command.

    Every gateway-registered slash command either has a specific handler
    in gateway/run.py or reaches the running-agent catch-all — in both
    cases the command is dispatched, not queued.
    """
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``daedalus skills
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`daedalus_cli.plugins.PluginContext.register_command`. Lookup is lazy
    so importing this module never forces plugin discovery.
    """
    try:
        from daedalus_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def discord_skill_commands_by_category(
    ...
    scan root) are returned as uncategorized.
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /daedalus handler.

    Maps both canonical names and aliases so /daedalus bg do stuff works
    the same as /daedalus background do stuff.
    """
    overrides = _resolve_config_gates()
