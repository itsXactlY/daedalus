"""
Multi-provider authentication system for Daedalus Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.daedalus/auth.json with cross-process file locking.
...
from daedalus_cli.config import get_daedalus_home, get_config_path, read_raw_config
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `daedalus model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("DAEDALUS_OAUTH_TRACE", "").strip().lower()
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `daedalus logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'daedalus doctor' for full diagnostics:"]
        for ci in issues:
...
def resolve_provider(
    ...
    *,
    ...
    raise AuthError(
        "No inference provider configured. Run 'daedalus model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.daedalus/.env.",
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("DAEDALUS_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `daedalus auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `daedalus auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `daedalus auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Daedalus auth store (~/.daedalus/auth.json)."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `daedalus auth` to re-authenticate."
            )
            relogin_required = True
        raise AuthError(
            message,
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Daedalus auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Daedalus's own Codex token store."""
    try:
        ...
        if cli_tokens:
            ...
            print("⚠️  Migrating Codex credentials to Daedalus's own auth store.")
            print("   This avoids conflicts with Codex CLI and VS Code.")
            print("   Run `daedalus auth` to create a fully independent session.\n")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("DAEDALUS_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("DAEDALUS_CA_BUNDLE")
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Daedalus is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("DAEDALUS_PORTAL_BASE_URL")
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "daedalus-cli"),
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Daedalus is not logged into Nous Portal.",
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("DAEDALUS_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("DAEDALUS_COPILOT_ACP_ARGS", "").strip()
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set DAEDALUS_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def detect_external_credentials() -> List[Dict[str, Any]]:
    ...
    Returns a list of dicts, each with:
      - provider: str   -- Daedalus provider id (e.g. "openai-codex")
    ...
    if cli_tokens:
        ...
        found.append({
            "provider": "openai-codex",
            "path": str(codex_path),
            "label": f"Codex CLI credentials found ({codex_path}) — run `daedalus auth` to create a separate session",
        })
...
def _prompt_model_selection(
    ...
    """
    from daedalus_cli.models import _format_price_per_mtok
...
def login_command(args) -> None:
    """Deprecated: use 'daedalus model' or 'daedalus setup' instead."""
    print("The 'daedalus login' command has been removed.")
    print("Use 'daedalus auth' to manage credentials,")
    print("'daedalus model' to select a provider, or 'daedalus setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    ...
    if cli_tokens:
        ...
        print("Daedalus will create its own session to avoid conflicts with Codex CLI / VS Code.")
        try:
        ...
        if do_import in ("y", "yes"):
            _save_codex_tokens(cli_tokens)
            base_url = os.getenv("DAEDALUS_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
            config_path = _update_config_for_provider("openai-codex", base_url)
            print()
            print("Credentials imported. Note: if Codex CLI refreshes its token,")
            print("Daedalus will keep working independently with its own session.")
            print(f"  Config updated: {config_path} (model.provider=openai-codex)")
            return
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Daedalus login via {pconfig.name}...")
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `daedalus model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("Daedalus will use OpenRouter for inference.")
        else:
            print("Run `daedalus model` or configure an API key to use Daedalus.")
    else:
