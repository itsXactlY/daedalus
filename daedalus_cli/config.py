"""
Configuration management for Daedalus Agent.

Config files are stored in ~/.daedalus/ for easy access:
- ~/.daedalus/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.daedalus/.env         - API keys and secrets

This module provides:
- daedalus config          - Show current configuration
- daedalus config edit     - Open config in editor
- daedalus config set      - Set a specific value
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("DAEDALUS_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_daedalus_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Daedalus is running in package-manager-managed mode.

    Two signals: the DAEDALUS_MANAGED env var (set by the systemd service),
    or a .managed marker file in DAEDALUS_HOME (set by the NixOS activation
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "daedalus update"


def format_managed_message(action: str = "modify this Daedalus installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("DAEDALUS_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Daedalus installation is managed by NixOS "
            f"(DAEDALUS_MANAGED={env_hint}).\n"
            "Edit services.daedalus.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Daedalus installation is managed by Homebrew "
            f"(DAEDALUS_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade daedalus"
        )

    return (
        f"Cannot {action}: this Daedalus installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Daedalus."
    )
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into DAEDALUS_HOME if the user doesn't have one yet."""
...
def ensure_daedalus_home():
    """Ensure ~/.daedalus directory structure exists with secure permissions."""
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.daedalus.config`` entries, then checks
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'daedalus setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Daedalus won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'daedalus doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("DAEDALUS_TOOL_PROGRESS")
            old_mode = get_env_value("DAEDALUS_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from DAEDALUS_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from DAEDALUS_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("DAEDALUS_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from DAEDALUS_TIMEZONE)")
            else:
                config["timezone"] = ""
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.daedalus/config.yaml as-is, without merging defaults or migrating.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    Ported from upstream daedalus v0.20.0 (NousResearch, tag v2026.8.3,
    config.py cfg_get) — the canonical helper for the
    ``cfg.get("X", {}).get("Y", default)`` pattern. Handles missing
    intermediate keys, non-dict intermediates, and ``cfg is None``. Explicit
    ``None`` values are returned as-is (``default`` is only returned when a
    key is *absent*).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.daedalus/config.yaml."""
    import copy
...
def load_config_readonly() -> Dict[str, Any]:
    """Load configuration without writing or migrating.

    Compatibility shim: the 0.20→0.8.0 port dropped this name (upstream
    0.8.0 only ships ``load_config``/``read_raw_config``), but many
    background subsystems (curator, gen registries, web_search_registry,
    daedalus_state, gateway platforms) still import it. Returns the same
    merged DEFAULT_CONFIG + user-override view as ``load_config()`` — the
    name only promises "no write/migrate side effects", not "raw YAML".
    Callers that specifically need the raw user YAML use ``read_raw_config``.
    """
    return load_config()
...
#   openai-codex (OAuth — daedalus auth) — OpenAI Codex
#   nous         (OAuth — daedalus auth) — Nous Portal
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.daedalus/.env."""
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.daedalus/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.daedalus/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.daedalus/.env and os.environ.
...
def show_config():
    ...
    if terminal.get('backend') == 'singularity':
    ...
    print(color("  daedalus config edit     # Edit config file", Colors.DIM))
    print(color("  daedalus config set <key> <value>", Colors.DIM))
    print(color("  daedalus setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: daedalus config set <key> <value>")
            print()
            print("Examples:")
            print("  daedalus config set model anthropic/claude-sonnet-4")
            print("  daedalus config set terminal.backend docker")
            print("  daedalus config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'daedalus config migrate' to add them")
        ...
        print("  daedalus config           Show current configuration")
        print("  daedalus config edit      Open config in editor")
        print("  daedalus config set <key> <value>   Set a config value")
        print("  daedalus config check     Check for missing/outdated config")
        print("  daedalus config migrate   Update config with new options")
        print("  daedalus config path      Show config file path")
        print("  daedalus config env-path  Show .env file path")
        sys.exit(1)
