"""On-demand supply-chain audit for Daedalus Agent installs.

Scans three surfaces a Daedalus user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Daedalus venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.daedalus/plugins``
...
def _discover_plugins(daedalus_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.daedalus/plugins``.
    ...
    """
    plugins_dir = daedalus_home / "plugins"
...
def _discover_mcp() -> list[Component]:
    ...
    try:
        from daedalus_cli.mcp_config import _get_mcp_servers
    except Exception:
        return []
...
def _discover_components(
    *,
    ...
    skip_mcp: bool = False,
    daedalus_home: Optional[Path] = None,
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = daedalus_home or Path(get_daedalus_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `daedalus security audit`."""
    home = Path(get_daedalus_home())
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, daedalus_home=home
    )
