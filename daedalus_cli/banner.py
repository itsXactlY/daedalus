...
Pure display functions with no DaedalusCLI state dependency.
"""
import json
import logging
import os
...
COMPACT_BANNER = """
[bold #FFD700]╔══════════════════════════════════════════════════════════════╗[/]
[bold #FFD700]║[/]  [#FFBF00]⚕ DAEDALUS[/] [dim #B8860B]- AI Agent Framework[/]              [bold #FFD700]║[/]
...
def build_compact_banner() -> str:
    """Return the width-fitted compact banner for narrow terminals.

    The compact banner implementation lives in ``cli.py`` (``_build_compact_banner``),
    which reads skin colors + the ``DAEDALUS_FAST_STARTUP_BANNER`` env flag. This
    alias keeps a canonical ``daedalus_cli.banner.build_compact_banner`` symbol for
    callers that import the compact renderer by its public name.
    """
    from cli import _build_compact_banner as _impl
...
_UPSTREAM_REPO_URL = "https://github.com/itsXactlY/daedalus.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/daedalus"
...
def check_for_updates() -> Optional[int]:
    """Check whether a Daedalus update is available.

    Two paths: if ``DAEDALUS_REVISION`` is set (nix builds embed it), compare
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Daedalus git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$DAEDALUS_HOME/daedalus/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``daedalus_cli/build_info.py``) and return it as a frozen
...
_RELEASE_URL_BASE = "https://github.com/itsXactlY/daedalus/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Daedalus checkout. Cached per-process. Release URL always points at the
    canonical itsXactlY/daedalus repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Daedalus Agent v{VERSION} ({RELEASE_DATE})"
...
def _normalize_hero_lines(hero: str) -> str:
    """Pad every line of hero art to equal full string length.

    The banner renders the hero inside a centered grid column; Rich centers
    each line independently, so lines of differing lengths get shifted relative
    to each other and the ASCII art scrambles (top rows drift sideways).
    Padding every line to the same full string length makes the centering a
    no-op.

    Handles multi-tag lines (e.g. ``[bold #FFD700]...[/][#FFBF00]...[/]``) by
    stripping all Rich markup tags, measuring full string length, and inserting
    NBSP padding right after the last closing tag so the art's own leading
    spaces stay intact. Normalizing full string length prevents Rich's grid
    auto-sizing from causing shifts because Rich sizes columns based on the
    full string length (including markup), not the visible width.
    """
    lines = hero.split("\n")
    if not lines:
        return hero

    max_full = max(len(ln) for ln in lines)

    out = []
    for i, ln in enumerate(lines):
        pad_needed = max_full - len(ln)
        ...
        j = 0
        while j < len(ln):
            if ln[j] == "[" and j + 1 < len(ln) and ln[j + 1] == "/":
                close_end = ln.find("]", j + 2)
                if close_end != -1:
                    last_close_pos = close_end
                    j = close_end + 1
            else:
                j += 1

        if last_close_pos > 0:
            new_line = ln[: last_close_pos + 1] + " " * pad_needed + ln[last_close_pos + 1:]
            out.append(new_line)
        else:
            out.append(ln)
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else DAEDALUS_CADUCEUS
    except Exception:
        _bskin = None
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        try:
            ...
            from daedalus_cli.moa_config import normalize_moa_config
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from daedalus_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
