"""
Profile management for multiple isolated Daedalus instances.

Each profile is a fully independent DAEDALUS_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.daedalus/profiles/<name>/`` by default.

The "default" profile is ``~/.daedalus`` itself — backward compatible,
zero migration needed.

Usage::

    daedalus profile create coder          # fresh profile + bundled skills
    daedalus profile create coder --clone  # also copy config, .env, SOUL.md
    daedalus profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    daedalus -p coder chat                 # or via flag
    daedalus profile use coder             # set as sticky default
    daedalus profile delete coder          # remove profile + alias + service
"""
...
def _get_profiles_root() -> Path:
    ...
    Always ``~/.daedalus/profiles/`` — anchored to the user's home,
    NOT to the current DAEDALUS_HOME (which may itself be a profile).
    This ensures ``coder profile list`` can see all profiles.
    """
    return Path.home() / ".daedalus" / "profiles"


def _get_default_daedalus_home() -> Path:
    """Return the default (pre-profile) DAEDALUS_HOME path."""
    return Path.home() / ".daedalus"


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_daedalus_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its DAEDALUS_HOME directory."""
    if name == "default":
        return _get_default_daedalus_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, daedalus subcommands, existing binaries in PATH.
    """
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.daedalus)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches DAEDALUS_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "DAEDALUS_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.daedalus).\n"
            "To remove everything, use: daedalus uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        from daedalus_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["DAEDALUS_HOME"] = old_home
        elif "DAEDALUS_HOME" in os.environ:
            del os.environ["DAEDALUS_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.daedalus/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: daedalus profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from DAEDALUS_HOME.

    Returns ``"default"`` if DAEDALUS_HOME is not set or points to ``~/.daedalus``.
    Returns the profile name if DAEDALUS_HOME points into ``~/.daedalus/profiles/<name>``.
    Returns ``"custom"`` if DAEDALUS_HOME is set to an unrecognized path.
    """
    from daedalus_constants import get_daedalus_home
    daedalus_home = get_daedalus_home()
    resolved = daedalus_home.resolve()

    default_resolved = _get_default_daedalus_home().resolve()
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if inferred_name == "default":
        raise ValueError(
            ...
            "Specify a different name: daedalus profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    ...
    return '''# Daedalus Agent profile completion
# Add to ~/.bashrc: eval "$(daedalus completion bash)"

_daedalus_profiles() {
    local profiles_dir="$HOME/.daedalus/profiles"
...
_daedalus_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_daedalus_profiles)" -- "$cur"))
        return
    fi
...
complete -F _daedalus_completion daedalus
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for daedalus profile names."""
    return '''#compdef daedalus
# Daedalus Agent profile completion
# Add to ~/.zshrc: eval "$(daedalus completion zsh)"

_daedalus() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.daedalus/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.daedalus/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    Called early in the CLI entry point, before any daedalus modules
    are imported, to set the DAEDALUS_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: daedalus profile create {profile_name}"
        )
