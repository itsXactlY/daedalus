"""`daedalus checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.daedalus/checkpoints/``.  Actions:

    daedalus checkpoints               # same as `status`
    daedalus checkpoints status        # total size, project count, breakdown
    daedalus checkpoints list          # per-project checkpoint counts + workdir
    daedalus checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    daedalus checkpoints clear [-f]    # nuke the entire base (asks first)
    daedalus checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    daedalus checkpoints
    daedalus checkpoints prune --retention-days 3 --max-size-mb 200
    daedalus checkpoints clear -f
...
from daedalus_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: daedalus checkpoints clear-legacy")
    return 0
