...
Add, remove, or reorder entries here — both `daedalus setup` and
`daedalus` provider-selection will pick up the change automatically.
"""
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        nous:daedalus-3                           →  ("nous", "daedalus-3")
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Daedalus' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``daedalus_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from daedalus_cli.auth import resolve_api_key_provider_credentials
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from daedalus_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "DaedalusAgent/1.0",
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from daedalus_constants import AI_GATEWAY_BASE_URL
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Daedalus will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
