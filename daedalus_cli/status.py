"""
Status command for daedalus CLI.

Shows the status of all Daedalus Agent components.
"""
...
from daedalus_cli.auth import AuthError, resolve_provider
from daedalus_cli.colors import Colors, color
from daedalus_cli.config import get_env_path, get_env_value, get_daedalus_home, load_config
from daedalus_cli.models import provider_label
from daedalus_cli.nous_subscription import get_nous_subscription_features
from daedalus_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all Daedalus Agent components."""
    ...
    try:
        from daedalus_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: daedalus model)'}"
    )
    if nous_logged_in:
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: daedalus model)'}"
    )
    ...
    if sys.platform.startswith('linux'):
        ...
        from daedalus_cli.gateway import get_launchd_label
        try:
            result = subprocess.run(
    ...
    jobs_file = get_daedalus_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    print(color("  Run 'daedalus doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'daedalus setup' to configure", Colors.DIM))
    print()
