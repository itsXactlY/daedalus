#!/usr/bin/env python3
"""
Daedalus CLI - Main entry point.

Usage:
    daedalus                     # Interactive chat (default)
    daedalus chat                # Interactive chat
    daedalus gateway             # Run gateway in foreground
    daedalus gateway start       # Start gateway as service
    daedalus gateway stop        # Stop gateway service
    daedalus gateway status      # Show gateway status
    daedalus gateway install     # Install gateway service
    daedalus gateway uninstall   # Uninstall gateway service
    daedalus setup               # Interactive setup wizard
    daedalus logout              # Clear stored authentication
    ...
    daedalus acp                 Run as an ACP server for editor integration
    daedalus sessions browse     Interactive session picker with search

    daedalus claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (daedalus tools, daedalus setup, daedalus model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'daedalus {command_name}' requires an interactive terminal.\n"
...
def _apply_profile_override() -> None:
    ...
    for i, arg in enumerate(argv):
        if arg in ("--profile", "-p") and i + 1 < len(argv):
            candidate = argv[i + 1]
            if _PROFILE_RE.match(candidate):
    ...
    if profile_name is not None:
        try:
            ...
            daedalus_home = resolve_profile_env(profile_name)
...
from daedalus_cli import __version__, __release_date__
...
def cmd_chat(args):
    ...
    if not _has_any_provider_configured():
        ...
        from daedalus_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
    ...
    if _fast_startup:
        os.environ["DAEDALUS_DEFER_AGENT_STARTUP"] = "1"
        os.environ["DAEDALUS_FAST_STARTUP_BANNER"] = "1"
...
def cmd_gateway(args):
    ...
    from daedalus_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from daedalus_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Daedalus?")
        print()
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  daedalus gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Daedalus Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  daedalus gateway")
        ...
        print()
        print("  Or install as a service: daedalus gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'daedalus whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    _require_tty("setup")
    from daedalus_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``daedalus model``) and the setup wizard
    ...
    from daedalus_cli.config import load_config, get_env_value
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from daedalus_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from daedalus_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from daedalus_cli.config import get_env_value, save_env_value
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from daedalus_cli.config import get_env_value, save_config, save_env_value
    from daedalus_cli.nous_subscription import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from daedalus_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from daedalus_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from daedalus_cli.auth import _save_model_choice, deactivate_provider
    from daedalus_cli.config import get_env_value, load_config, save_config
    ...
    from daedalus_cli.models import probe_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from daedalus_cli.config import save_env_value, load_config, save_config
    from daedalus_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from daedalus_cli.copilot_auth import copilot_device_code_login
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Daedalus turns to `copilot --acp`.")
    print("  Daedalus currently starts its own ACP subprocess for each request.")
    print("  Daedalus uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set DAEDALUS_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from daedalus_cli.config import get_env_value, save_env_value, load_config, save_config
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from daedalus_cli.config import get_env_value, save_env_value, load_config, save_config
    from daedalus_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(f"    Daedalus will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def cmd_login(args):
    """Authenticate Daedalus CLI with a provider."""
    from daedalus_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from daedalus_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from daedalus_cli.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from daedalus_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from daedalus_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from daedalus_cli.webhook import webhook_command
    webhook_command(args)


def cmd_security(args):
    """Dispatch `daedalus security <subcmd>`."""
...
def cmd_doctor(args):
    ...
    from daedalus_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from daedalus_cli.config import config_command
    config_command(args)
...
def cmd_uninstall(args):
    """Uninstall Daedalus Agent."""
    _require_tty("uninstall")
    from daedalus_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``daedalus update --gateway`` so interactive prompts (stash restore,
...
def _update_via_zip(args):
    """Update Daedalus Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/itsXactlY/daedalus/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="daedalus-update-")
        zip_path = os.path.join(tmp_dir, f"daedalus-{branch}.zip")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("daedalus-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Daedalus behaves unexpectedly.")
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Daedalus couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Daedalus couldn't drop the saved stash entry.")
...
OFFICIAL_REPO_URLS = {
    "https://github.com/itsXactlY/daedalus.git",
    "git@github.com:itsXactlY/daedalus.git",
    "https://github.com/itsXactlY/daedalus",
    "git@github.com:itsXactlY/daedalus",
}
OFFICIAL_REPO_URL = "https://github.com/itsXactlY/daedalus.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    from daedalus_constants import get_daedalus_home
    return (get_daedalus_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from daedalus_constants import get_daedalus_home
        (get_daedalus_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from itsXactlY/daedalus.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/itsXactlY/daedalus.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/itsXactlY/daedalus.git' to add later.")
...
def _restore_original_branch(git_cmd, cwd, original_branch, update_branch):
    """Switch back to the branch the user was on before the update moved HEAD.

    ``daedalus update`` temporarily checks out the update target (main). Without
    the switch-back, fork feature-branch installs silently end up running
    upstream main after every update — all local rework commits vanish from
    the running runtime with no warning (the return-switch only existed in the
    no-commits path before this fix).
    """
    if not original_branch or original_branch in {"HEAD", update_branch}:
        return
    try:
        result = subprocess.run(
    ...
    if result is not None and result.returncode == 0:
        ...
        print(
            f"  ⚠ Could not switch back to branch '{original_branch}' — run manually: "
            f"git checkout {original_branch}"
        )


class _UpdateOutputStream:
    """Stream wrapper used during ``daedalus update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.daedalus/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``daedalus update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``daedalus update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.daedalus/logs/update.log`` and to silently absorb
       ``BrokenPipeError`` when the terminal vanishes.

    ``SIGINT`` (Ctrl-C) and ``SIGTERM`` (systemd shutdown) are
    intentionally left alone.

    In gateway mode the update is already spawned detached from a
    terminal, so this function is a no-op.

    Returns a dict that ``cmd_update`` passes to ``_finalize_update_output``.
    """
    state = {
    ...
    try:
        from daedalus_cli.config import get_daedalus_home as _get_daedalus_home

        logs_dir = _get_daedalus_home() / "logs"
...
def cmd_update(args):
    """Update Daedalus Agent to the latest version."""
    import shutil
    from daedalus_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Daedalus Agent")
        return
    ...
    try:
        ...
        if not git_dir.exists():
            if sys.platform == "win32":
                ...
                print("  curl -fsSL https://raw.githubusercontent.com/itsXactlY/daedalus/main/scripts/install.sh | bash")
                sys.exit(1)
        ...
        try:
            ...
            if needs_migration:
                ...
                if gateway_mode:
                    ...
                    print("    Run 'daedalus config migrate' later to apply any new config/env options.")
                    response = "n"
                else:
                    try:
                ...
                if response in ('', 'y', 'yes'):
                    ...
                    if gateway_mode and missing_env:
                        print("  ℹ API keys require manual entry: daedalus config migrate")
                else:
                    print()
                    print("Skipped. Run 'daedalus config migrate' later to configure.")
            else:
            ...
            try:
                ...
                if is_linux():
                    ...
                    for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                        try:
                            result = subprocess.run(
                                scope_cmd + ["list-units", "daedalus-gateway*", "--plain", "--no-legend", "--no-pager"],
            ...
            print("  daedalus model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``daedalus -c Pokemon Agent Dev`` without quoting the
    ...
    _SUBCOMMANDS = {
        ...
        "status", "cron", "doctor", "curator", "config", "pairing", "skills", "tools",
...
def cmd_profile(args):
    ...
    from daedalus_cli.profiles import (
    ...
    if action is None:
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → daedalus -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.daedalus)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  daedalus profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     daedalus -p {name} chat")
                else:
            ...
            if clone or clone_all:
                profile_dir_display = f"~/.daedalus/profiles/{name}"
        ...
        name = args.profile_name
        from daedalus_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from daedalus_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        from daedalus_cli.profiles import rename_profile
        try:
        ...
        from daedalus_cli.profiles import export_profile
        name = args.profile_name
        ...
        from daedalus_cli.profiles import import_profile
        try:
...
def cmd_completion(args):
    ...
    from daedalus_cli.profiles import generate_bash_completion, generate_zsh_completion
...
def cmd_logs(args):
    """View and filter Daedalus log files."""
    from daedalus_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for daedalus CLI."""
    parser = argparse.ArgumentParser(
        prog="daedalus",
        description="Daedalus Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    daedalus                        Start interactive chat
    daedalus chat -q "Hello"        Single query mode
    daedalus -c                     Resume the most recent session
    daedalus -c "my project"        Resume a session by name (latest in lineage)
    daedalus --resume <session_id>  Resume a specific session by ID
    daedalus setup                  Run setup wizard
    daedalus logout                 Clear stored authentication
    daedalus auth add <provider>    Add a pooled credential
    daedalus auth list              List pooled credentials
    daedalus auth remove <p> <t>    Remove pooled credential by index, id, or label
    daedalus auth reset <provider>  Clear exhaustion status for a provider
    daedalus model                  Select default model
    daedalus config                 View configuration
    daedalus config edit            Edit config in $EDITOR
    daedalus config set model gpt-4 Set a config value
    daedalus gateway                Run messaging gateway
    daedalus -s daedalus-dev,github-auth
    daedalus -w                     Start in isolated git worktree
    daedalus gateway install        Install gateway background service
    daedalus sessions list          List past sessions
    daedalus sessions browse        Interactive session picker
    daedalus sessions rename ID T   Rename/title a session
    daedalus logs                   View agent.log (last 50 lines)
    daedalus logs -f                Follow agent.log in real time
    daedalus logs errors            View errors.log
    daedalus logs --since 1h        Lines from the last hour
    daedalus update                 Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Daedalus Agent"
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: daedalus-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Daedalus Agent with an interactive wizard. "
                    "Run a specific section: daedalus setup model|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Daedalus CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: daedalus-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    auth_logout = auth_subparsers.add_parser("logout", help="Log out of a provider (clears its auth state)")
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Daedalus Agent components"
    )
    ...
    checkpoints_parser = subparsers.add_parser(
        ...
        description="View total size, prune stale/orphan checkpoints, or clear the store. "
                     "Safe to run any time — none of these require the agent to be running.",
    )
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the Daedalus "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.daedalus/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    security_audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the Daedalus Python venv",
    )
    security_audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    try:
        from daedalus_cli.curator import register_cli as _register_curator_cli
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Daedalus Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from daedalus_cli.pairing import pairing_command
    ...
    # Distinct from `update`, which upgrades hub-installed skills one by one.
    # `sync` refreshes the BUNDLED set from the source repo — the 100 curated
    # skills that a packaged install never receives, because they are SKILL.md
    # files outside any Python package and so are absent from the wheel.
    skills_sync = skills_subparsers.add_parser(
        "sync", help="Refresh the bundled skills from the source repository")
    skills_sync.add_argument("--ref", default="main", help="Branch or tag (default: main)")
    skills_sync.add_argument("--repo", default="itsXactlY/daedalus", help="owner/name (default: itsXactlY/daedalus)")
    skills_sync.add_argument("--dry-run", action="store_true", help="Show what would change, write nothing")
    skills_sync.add_argument("--prune", action="store_true",
                             help="Also delete local skills the repo no longer has "
                                  "(off by default: hub-installed and hand-written "
                                  "skills share this directory)")
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from daedalus_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/daedalus-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from daedalus_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from daedalus_cli.memory_setup import memory_command
            memory_command(args)
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'daedalus tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from daedalus_cli.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from daedalus_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description=(
            "Manage MCP server connections and run Daedalus as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'daedalus mcp add' to connect to a new server, or\n"
            "'daedalus mcp serve' to expose Daedalus conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Daedalus as an MCP server (expose conversations to other agents)",
    )
    ...
    mcp_login_p = mcp_sub.add_parser("login", help="Force re-authentication for an OAuth MCP server")
    mcp_login_p.add_argument("name", help="Server name to re-authenticate")

    mcp_reauth_p = mcp_sub.add_parser("reauth", help="Re-authenticate one or all OAuth MCP servers")
    mcp_reauth_p.add_argument("name", nargs="?", help="Server name (omit with --all)")
    mcp_reauth_p.add_argument("--all", action="store_true", help="Re-authenticate every OAuth server, one at a time")
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Daedalus"
    )
    ...
    def cmd_claw(args):
        from daedalus_cli.claw import claw_command
        claw_command(args)
    ...
    uninstall_parser = subparsers.add_parser(
        ...
        description="Remove Daedalus Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        ...
        description="Start Daedalus Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Daedalus Agent as an ACP server."""
        try:
    ...
    logs_parser = subparsers.add_parser(
        ...
        epilog="""\
Examples:
    daedalus logs                    Show last 50 lines of agent.log
    daedalus logs -f                 Follow agent.log in real time
    daedalus logs errors             Show last 50 lines of errors.log
    daedalus logs gateway -n 100     Show last 100 lines of gateway.log
    daedalus logs --level WARNING    Only show WARNING and above
    daedalus logs --session abc123   Filter by session ID
    daedalus logs --since 1h         Lines from the last hour
    daedalus logs --since 30m -f     Follow, starting from 30 min ago
    daedalus logs list               List available log files with sizes
""",
    )
