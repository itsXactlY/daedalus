...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        def _discover() -> None:
            token = None
            try:
                from daedalus_constants import set_daedalus_home_override
            ...
            try:
                ...
                if token is not None:
                    try:
                        from daedalus_constants import reset_daedalus_home_override
...
def _resolve_discovery_timeout(
    ...
    When ``single_query`` is True (``daedalus -z "..."`` / ``-q``), the larger
    ...
    try:
        from daedalus_cli.config import load_config, DEFAULT_CONFIG
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``daedalus dashboard``).  Those processes populate THIS module's
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``daedalus -z``) can construct
