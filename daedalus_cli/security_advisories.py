"""
Security advisory checker for Daedalus Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``daedalus doctor --ack <id>``; the ack is persisted to
...
1. ``daedalus doctor`` (and ``daedalus doctor --ack <id>``)
...
run in environments where the rest of Daedalus failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including daedalus when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.daedalus/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: daedalus doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'daedalus doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_daedalus_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``daedalus doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `daedalus doctor` on the gateway host for details.")
