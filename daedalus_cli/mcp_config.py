"""
MCP Server Management CLI — ``daedalus mcp`` subcommand.

Implements ``daedalus mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.daedalus/config.yaml under the ``mcp_servers`` key.
"""
...
from daedalus_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_daedalus_home,  # noqa: F401 — used by test mocks
)
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  daedalus mcp add ink --url "https://mcp.ml.ink/mcp"')
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: daedalus mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_daedalus_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  daedalus mcp add <name> --url <endpoint>')
        _info('  daedalus mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    A clean probe is not proof of authentication -- some MCP servers serve
    initialize + tools/list without auth, so listing tools can succeed even
    when the OAuth flow never completed. Checking the token file directly
    (same path DaedalusTokenStorage writes to) avoids reporting a false
    success in that case.
    """
    try:
        from tools.mcp_oauth import _get_token_dir, _safe_filename
        token_path = _get_token_dir() / f"{_safe_filename(name)}.json"
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``daedalus mcp login`` and
    ``daedalus mcp reauth`` so both behave identically for a single server.

    Adapted from nousresearch/main -- that version also wraps the probe in a
    ``force_interactive_oauth()`` context manager and calls
    ``humanize_oauth_registration_error()`` for friendlier error text;
    neither exists in this fork's tools/mcp_oauth.py. Skipped rather than
    ported wholesale (out of scope for this fix): interactive OAuth here
    still follows the existing ``_is_interactive()`` gate in
    tools/mcp_oauth.py, and failures show the raw exception.
    """
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info(
                "Some providers do not support automatic client registration. "
                "For those you must create an OAuth client yourself and add "
                "its credentials to config.yaml:"
            )
            print()
            ...
            print()
            _info("Then re-run `daedalus mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_login(args):
    ...
    Use this when tokens are stuck in a bad state (server revoked, refresh
    token consumed externally, etc.), to re-authenticate with different
    scopes/account, or after a tool call returns needs_reauth.
    """
    name = args.name
    servers = _get_mcp_servers()
...
def cmd_mcp_reauth(args):
    ...
    ``daedalus mcp reauth <name>`` re-auths a single server (same as login).
    ``daedalus mcp reauth --all`` discovers every ``auth: oauth`` server in
    config and re-auths them ONE AT A TIME -- a human can only complete one
    browser OAuth flow at a time, so concurrent re-auth would open N tabs
    at once and N-1 would time out.
    """
    servers = _get_mcp_servers()
    ...
    if not name:
        ...
        _info("Usage: daedalus mcp reauth <name>   |   daedalus mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'daedalus mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    ...
    if handler:
        ...
        _info("daedalus mcp add <name> --url <endpoint>        Add an MCP server")
        _info("daedalus mcp add <name> --command <cmd>         Add a stdio server")
        _info("daedalus mcp remove <name>                      Remove a server")
        _info("daedalus mcp list                               List servers")
        _info("daedalus mcp test <name>                        Test connection")
        _info("daedalus mcp configure <name>                   Toggle tools")
        _info("daedalus mcp login <name>                       Force re-authentication")
        _info("daedalus mcp reauth <name>|--all                 Re-authenticate one or all OAuth servers")
        print()
