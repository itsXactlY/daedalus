"""
Unified tool configuration for Daedalus Agent.

`daedalus tools` and `daedalus setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.daedalus/config.yaml under
...
PLATFORMS = {
    ...
    "homeassistant": {"label": "🏠 Home Assistant", "default_toolset": "daedalus-homeassistant"},
    "api_server": {"label": "🌐 API Server", "default_toolset": "daedalus-api-server"},
}
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_daedalus_home()}/daedalus && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in daedalus directory)")
        ...
        if camofox_dir.exists():
            ...
            _print_info("    Or use Podman: podman run -p 9377:9377 -e CAMOFOX_PORT=9377 jo-inc/camofox-browser")
        elif not shutil.which("npm"):
            _print_warning("    Node.js not found. Install Camofox via Podman:")
            _print_info("      podman run -p 9377:9377 -e CAMOFOX_PORT=9377 jo-inc/camofox-browser")
    ...
    if not env_vars:
        ...
        if managed_feature:
            ...
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.daedalus/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Daedalus Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print()
    from daedalus_constants import display_daedalus_home
    print(color(f"  Tool configuration saved to {display_daedalus_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'daedalus' or gateway restart.", Colors.DIM))
    print()
