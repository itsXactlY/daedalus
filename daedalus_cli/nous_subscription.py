...
from daedalus_cli.auth import get_nous_auth_status
from daedalus_cli.config import get_env_value, load_config
...
def get_nous_subscription_explainer_lines() -> list[str]:
    ...
    return [
        ...
        "Change these later with: daedalus setup tools, daedalus setup terminal, or daedalus status.",
    ]


def apply_nous_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level Nous defaults shared by `daedalus setup` and `daedalus model`."""
    if not managed_nous_tools_enabled():
        return set()
