...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_daedalus_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:daedalus_pkce",
    ...
    raise SystemExit(f"`daedalus auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    if removed.source.startswith("env:"):
        ...
        if env_var:
            from daedalus_cli.config import remove_env_value
    ...
    elif removed.source == "daedalus_pkce" and provider == "anthropic":
        from daedalus_constants import get_daedalus_home
        oauth_file = get_daedalus_home() / ".anthropic_oauth.json"
        if oauth_file.exists():
            oauth_file.unlink()
            print("Cleared Daedalus Anthropic OAuth credentials")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `daedalus auth status openrouter`.")
