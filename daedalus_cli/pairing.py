...
Usage:
    daedalus pairing list              # Show all pending + approved users
    daedalus pairing approve <platform> <code>  # Approve a pairing code
    daedalus pairing revoke <platform> <user_id> # Revoke user access
    daedalus pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle daedalus pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: daedalus pairing {list|approve|revoke|clear-pending}")
        print("Run 'daedalus pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        remaining_min = max(1, round((lockout_until - _time.time()) / 60))
        print(f"\n  Platform '{platform}' is locked out after too many failed approval attempts.")
        print(f"  Lockout clears in ~{remaining_min} minute(s).\n")
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'daedalus pairing list' to see pending codes.\n")
