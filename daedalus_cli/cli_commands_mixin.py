...
``cli.py``'s ``DaedalusCLI`` class. ``DaedalusCLI`` inherits ``CLICommandsMixin`` so
...
from daedalus_constants import display_daedalus_home, is_termux as _is_termux_environment
from agent.turn_context import extract_api_content_sidecar
from daedalus_cli.browser_connect import (
...
class CLICommandsMixin:
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything Daedalus changed (checkpoint baseline)
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            print("  No changes — Daedalus hasn't edited any files here yet.")
            return
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Daedalus config/state.
        ...
        """
        from daedalus_cli.backup import (
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_daedalus_home()}/state-snapshots/):\n")
    ...
    def _handle_export_command(self, command: str):
        ...
        """
        from daedalus_cli.profiles import export_profile, get_active_profile_name
        ...
        try:
            ...
            print("  Share it: the other user runs /import or `daedalus profile import <archive>`.")
    ...
    def _handle_import_command(self, command: str):
        ...
        try:
            ...
            pass
        print(f"  Use it: daedalus -p {imported}")
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `daedalus journey`).
        ...
        from cli import _cprint
        from daedalus_cli.journey import register_cli
    ...
    def _handle_paste_command(self):
        ...
        from daedalus_cli.clipboard import has_clipboard_image
    ...
    def _handle_copy_command(self, cmd_original: str) -> None:
        ...
        try:
            from daedalus_cli.clipboard import (
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run daedalus chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_profile_command(self):
        ...
        from daedalus_cli.slash_exec import CommandContext, execute_command
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `daedalus gateway` running?")
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from daedalus_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_hatch_command(self, cmd: str):
        ...
        from daedalus_cli.pets import _set_active
    ...
    def _handle_suggestions_command(self, cmd: str):
        ...
        args = " ".join(tokens)
        try:
            from daedalus_cli.suggestions_cmd import handle_suggestions_command
    ...
    def _handle_blueprint_command(self, cmd: str):
        ...
        try:
            from daedalus_cli.blueprint_cmd import handle_blueprint_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to daedalus_cli.curator so the CLI and the `daedalus curator`
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from daedalus_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        ...
        if args and args[0].lower() in {"pending", "approve", "apply", "reject",
            ...
            from daedalus_cli.write_approval_commands import handle_pending_subcommand
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``daedalus bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: daedalus bundles create "
        ...
        _cprint(
            ...
            f"Manage with `daedalus bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Daedalus-managed isolated debug "
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        cross-process schedules use `daedalus cron`.
        """
        from cli import _DIM, _RST, _cprint
        from daedalus_cli.heartbeat import parse_interval, format_interval
        ...
        if interval is None:
            ...
            return
        if interval < 0:
            from daedalus_cli.heartbeat import MIN_INTERVAL_SECONDS
        ...
        _cprint(
            ...
            f"lives only while this Daedalus process runs — use `daedalus cron` for "
    ...
    def _handle_goal_draft(self, objective: str) -> None:
        ...
        from cli import _DIM, _RST, _cprint
        from daedalus_cli.goals import draft_contract
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="daedalus_prompt_")
        try:
    ...
    def _handle_focus_command(self, cmd_original: str) -> None:
        ...
        from daedalus_cli.focus_view import (
            FOCUS_CONFIG_KEY,
    ...
    def _set_tool_progress_mode(self, mode: str) -> None:
        ...
        """
        from daedalus_cli.focus_view import normalize_tool_progress_mode
    ...
    def _note_focus_hidden_line(self, function_name: str) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from daedalus_cli.focus_view import would_display_tool_line
    ...
    def _emit_focus_recovery_line(self) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from daedalus_cli.focus_view import format_hidden_line
    ...
    def _handle_approvals_command(self, cmd_original: str) -> None:
        ...
        from cli import _cprint
        from daedalus_cli.approval_mode import run_approval_mode_command
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Daedalus is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Daedalus is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will redirect the current run while Daedalus is busy; /stop still cancels it."
    ...
    def _handle_indicator_command(self, cmd: str):
        ...
        """
        from cli import _ACCENT, _DIM, _RST, _cprint, save_config_value
        from daedalus_constants import DEFAULT_INDICATOR_STYLE, INDICATOR_STYLES
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        ...
        """
        from daedalus_cli.debug import run_debug_share
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Daedalus Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``daedalus update`` so the user sees update output directly and gets
        ...
        raw = self._prompt_text_input_modal(
            title="⚕  Update Daedalus Agent",
            detail="This will exit the current session and run `daedalus update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Daedalus' hotword listener.
