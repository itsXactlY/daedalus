"""``daedalus logs`` — view and filter Daedalus log files.

Supports tailing, following, session filtering, level filtering, and
relative time ranges.  All log files live under ``~/.daedalus/logs/``.

Usage examples::

    daedalus logs                    # last 50 lines of agent.log
    daedalus logs -f                 # follow agent.log in real time
    daedalus logs errors             # last 50 lines of errors.log
    daedalus logs gateway -n 100     # last 100 lines of gateway.log
    daedalus logs --level WARNING    # only WARNING+ lines
    daedalus logs --session abc123   # filter by session ID substring
    daedalus logs --since 1h         # lines from the last hour
    daedalus logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_daedalus_home() / "logs" / filename
    ...
    if follow:
        print(f"--- {display_daedalus_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_daedalus_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_daedalus_home()}/logs/")
        return

    print(f"Log files in {display_daedalus_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'daedalus chat' to generate logs)")
