"""Agent-construction and session-resume display methods for ``DaedalusCLI``.

Extracted from ``cli.py`` as part of the god-file decomposition campaign
(``~/.daedalus/plans/god-file-decomposition.md``, Phase 4 step 2). This mixin holds
...
Behavior-neutral: every method is lifted verbatim from ``DaedalusCLI``. ``self.*``
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``DaedalusCLI``."""
    ...
    def _ensure_runtime_credentials(self) -> bool:
            ...
            if isinstance(_primary_exc, AuthError):
                ...
                for _fb in _fb_chain:
                    ...
                    if not _fb_provider or not _fb_model:
                        continue
                    try:
                        from daedalus_cli.fallback_config import resolve_entry_api_key
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("   Run 'daedalus model' to choose a provider, or "
                      "'daedalus setup' for first-time setup.")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: daedalus setup")
            return False
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        exact same flow as ``daedalus model`` (which fronts Quick Setup / Nous
        ...
        try:
            ...
            answer = "n"
        if answer in {"n", "no"}:
            _cprint("  Skipped. Run 'daedalus model' or 'daedalus setup' any time.")
            return False
        ...
        try:
            ...
            print()
            _cprint("  Setup cancelled. Run 'daedalus model' any time.")
            return False
        except Exception as exc:
        ...
        if self._runtime_credentials_ready():
            ...
            return True
        _cprint("  Provider setup didn't complete. Run 'daedalus model' to retry.")
        return False
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        """
        from daedalus_cli.models import resolve_fast_mode_overrides
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        from daedalus_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (daedalus sessions list).",
                        file=sys.stderr,
                    )
                else:
        ...
        try:
            ...
            from daedalus_constants import partial_update_hint
