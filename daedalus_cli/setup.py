"""
Interactive setup wizard for Daedalus Agent.
...
Config files are stored in ~/.daedalus/ for easy access.
"""
...
_DOCS_BASE = "https://hermes.nousresearch.com/docs"
...
def _setup_copilot_reasoning_selection(
    ...
    *,
    ...
    from daedalus_cli.models import github_model_reasoning_efforts, normalize_copilot_model_id
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Daedalus Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Daedalus using environment variables or config commands:")
    print_info("  daedalus config set model.provider custom")
    print_info("  daedalus config set model.base_url http://localhost:8080/v1")
    print_info("  daedalus config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'daedalus setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'daedalus setup')")
...
def _print_setup_summary(config: dict, daedalus_home):
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'daedalus setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'daedalus setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'daedalus setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {daedalus_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('daedalus setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('daedalus setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('daedalus setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('daedalus setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('daedalus setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('daedalus config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('daedalus config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('daedalus config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('daedalus', Colors.GREEN)}              Start chatting")
    print(f"   {color('daedalus gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('daedalus doctor', Colors.GREEN)}       Check for issues")
    print()
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``daedalus model``)
    ...
    provider added to ``daedalus model`` is automatically available here.
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from daedalus_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Daedalus can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Daedalus can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'daedalus setup' or configure AUXILIARY_VISION_* settings")
...
def _install_neutts_deps() -> bool:
    ...
    try:
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "-U", NEUTTS_SPEC, "--quiet"],
        ...
        print_info(f"Try manually: python -m pip install -U {NEUTTS_SPEC}")
        return False
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.daedalus/.env."
            )
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'daedalus setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Daedalus runs shell commands and code.")
    ...
    idx_to_backend = {0: "local", 1: "modal", 2: "ssh", 3: "daytona"}
    backend_to_idx = {"local": 0, "modal": 1, "ssh": 2, "daytona": 3}
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `daedalus setup agent` later to customize.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Daedalus delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://hermes.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Daedalus delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Daedalus delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'daedalus whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'daedalus whatsapp' to choose your mode (separate bot number")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://hermes.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://hermes.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Daedalus from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'daedalus setup gateway' later to configure.")
        return
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: daedalus gateway install")
            else:
                print_info("  You can install later: daedalus gateway install")
                if _is_linux:
                    print_info("  Or as a boot-time service: sudo daedalus gateway install --system")
                print_info("  Or run in foreground:  daedalus gateway")
        else:
            print_info("Start the gateway to bring your bots online:")
            print_info("   daedalus gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `daedalus setup tools` and `daedalus tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from daedalus_cli.config
...
def _offer_openclaw_migration(daedalus_home: Path) -> bool:
    ...
    print_info("Daedalus can import your settings, memories, skills, and API keys.")
    print()
    ...
    try:
        ...
        migrator = mod.Migrator(
            ...
            target_root=daedalus_home.resolve(),
            execute=True,
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Daedalus.")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      daedalus setup           — full or quick (auto-detected)
      daedalus setup model     — just model/provider
      daedalus setup terminal  — just terminal backend
      daedalus setup gateway   — just messaging platforms
      daedalus setup tools     — just tool configuration
      daedalus setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Daedalus Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Daedalus Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Daedalus Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        setup_mode = prompt_choice("How would you like to set up Daedalus?", [
    ...
    print_info(f"Data folder:  {daedalus_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'daedalus config edit'")
...
def _offer_launch_chat():
    ...
    print()
    if prompt_yes_no("Launch daedalus chat now?", True):
        from daedalus_cli.main import cmd_chat
        from types import SimpleNamespace
...
def _run_first_time_quick_setup(config: dict, daedalus_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'daedalus setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    daedalus setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  daedalus setup gateway")
    print()
...
def _run_quick_setup(config: dict, daedalus_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'daedalus setup' and choose 'Full Setup' to reconfigure,")
