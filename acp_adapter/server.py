"""ACP agent server — exposes Daedalus via the Agent Client Protocol."""
...
try:
    from daedalus_cli import __version__ as DAEDALUS_VERSION
except Exception:
...
class DaedalusACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Daedalus AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Daedalus version",
        },
    )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = getattr(state.agent, "enabled_toolsets", None) or ["daedalus-acp"]
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate Daedalus using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="daedalus", version=DAEDALUS_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Daedalus on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            from model_tools import get_tool_definitions
            toolsets = getattr(state.agent, "enabled_toolsets", None) or ["daedalus-acp"]
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Daedalus has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
