"""ACP session manager — maps ACP sessions to Daedalus AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.daedalus/state.db``) so they
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Daedalus AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Daedalus runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.daedalus/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``DAEDALUS_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_daedalus_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=daedalus_home / "state.db")
            return self._db_instance
        except Exception:
