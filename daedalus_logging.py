"""Centralized logging setup for Daedalus Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.daedalus/logs/`` (profile-aware via ``get_daedalus_home()``).
...
def setup_logging(
    *,
    ...
    """Configure the Daedalus logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    daedalus_home
        Override for the Daedalus home directory.  Falls back to
        ``get_daedalus_home()`` (profile-aware).
    log_level
    ...
    if _logging_initialized and not force:
        home = daedalus_home or get_daedalus_home()
        return home / "logs"
...
def _read_logging_config():
    ...
    try:
        import yaml
        config_path = get_daedalus_home() / "config.yaml"
