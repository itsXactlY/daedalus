"""Shared constants for Daedalus Agent.
...
_UNSET = object()
_DAEDALUS_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
...
def set_daedalus_home_override(path: str | Path | None) -> Token:
    """Set a context-local Daedalus home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _DAEDALUS_HOME_OVERRIDE.set(value)


def reset_daedalus_home_override(token: Token) -> None:
    """Restore the previous context-local Daedalus home override."""
    _DAEDALUS_HOME_OVERRIDE.reset(token)


def get_daedalus_home_override() -> str | None:
    """Return the active context-local Daedalus home override, if any."""
    override = _DAEDALUS_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_daedalus_home() -> Path:
    """Return the platform-native default home path.

    Daedalus uses its OWN home (``~/.daedalus``) rather than upstream's
    ``~/.hermes``. A clean install must not silently adopt an existing Hermes
    home: that directory holds another distribution's state.db, sessions,
    auth.json and config.yaml, so inheriting it would both surprise the user
    and pull credentials into a fresh install.

    An existing ``~/.hermes`` is still honoured when this build is deliberately
    pointed at it via ``DAEDALUS_HOME`` (see :func:`_daedalus_home_from_env`),
    which keeps migration a one-liner.
    """
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "daedalus"
    return Path.home() / ".daedalus"


def _daedalus_home_from_env() -> Path:
    """Resolve DAEDALUS_HOME from the process environment only.

    Reads the ``DAEDALUS_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_daedalus_home_override`, so this reflects the process/launch
    ...
    if val:
        return Path(val)
    return _get_platform_default_daedalus_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``DAEDALUS_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_daedalus_home()
    ...
    if active and active != "default":
        ...
        msg = (
            ...
            f"subprocess spawner should pass DAEDALUS_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_daedalus_home() -> Path:
    """Return the Daedalus home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_daedalus_home_override`) → ``DAEDALUS_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``DAEDALUS_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``DAEDALUS_HOME`` explicitly (see the systemd
    template in ``daedalus_cli/gateway.py`` and the kanban dispatcher in
    ``daedalus_cli/kanban_db.py``).  See https://github.com/NousResearch/daedalus/issues/18594.
    """
    override = get_daedalus_home_override()
    if override:
        return Path(override)

    if not os.environ.get("DAEDALUS_HOME", "").strip():
        _warn_profile_fallback_once()

    return _daedalus_home_from_env()


def get_process_daedalus_home() -> Path:
    """Return the Daedalus home for the running process, ignoring task overrides.

    Unlike :func:`get_daedalus_home`, this never follows the context-local
    override set by :func:`set_daedalus_home_override`.  It resolves only the
    process ``DAEDALUS_HOME`` env var (falling back to the platform default),
...
def get_default_daedalus_root() -> Path:
    """Return the root Daedalus directory for profile-level operations.

    In standard deployments this is the platform-native Daedalus home
    (``~/.daedalus`` on POSIX, ``%LOCALAPPDATA%\\daedalus`` on native Windows).

    In Docker or custom deployments where ``DAEDALUS_HOME`` points outside
    ``~/.daedalus`` (e.g. ``/opt/data``), returns ``DAEDALUS_HOME`` directly
    — that IS the root.

    In profile mode where ``DAEDALUS_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.daedalus/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_daedalus_home()
    env_home = os.environ.get("DAEDALUS_HOME", "")
    if not env_home:
        return native_home
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``DAEDALUS_OPTIONAL_SKILLS``.
    """
    override = os.getenv("DAEDALUS_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_daedalus_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``DAEDALUS_OPTIONAL_MCPS``.
    """
    override = os.getenv("DAEDALUS_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``DAEDALUS_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<DAEDALUS_HOME>/skills`` last-resort
    """
    override = os.getenv("DAEDALUS_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
...
def get_daedalus_dir(
    new_subpath: str,
    old_name: str,
    *,
    home: Path | None = None,
) -> Path:
    """Resolve a Daedalus subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to DAEDALUS_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to DAEDALUS_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Daedalus home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local DAEDALUS_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_daedalus_home()
...
def iter_daedalus_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Daedalus-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\daedalus\\node``. POSIX installs use
    ``$DAEDALUS_HOME/node/bin``. Include both shapes on every platform so mixed
...
_DAEDALUS_NODE_TARGET_MAJOR = int(os.environ.get("DAEDALUS_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Daedalus-managed Node trees live under ``$DAEDALUS_HOME/node`` (or a profile's
    ``DAEDALUS_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_daedalus_node_executable``
    used to trust file presence alone, so ``daedalus update`` would pick that
...
def daedalus_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Daedalus-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_daedalus_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%DAEDALUS_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_DAEDALUS_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_DAEDALUS_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$DAEDALUS_HOME/node`` on POSIX.
    ...
    ``DAEDALUS_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_daedalus_managed_node() -> str | None:
    """Install a Daedalus-managed Node tree and return its npm path.
    ...
    Daedalus never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$DAEDALUS_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_daedalus_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_daedalus_node_dirs():
...
def heal_daedalus_managed_node() -> bool:
    """Redownload Daedalus-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not daedalus_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "DAEDALUS_HOME": str(get_daedalus_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_daedalus_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_DAEDALUS_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
...
def find_daedalus_node_executable(command: str) -> str | None:
    """Return a Daedalus-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_DAEDALUS_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_daedalus_managed_node():
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Daedalus-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Daedalus-managed installs.

    This is for Daedalus-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_daedalus_node_executable(command)
    if managed:
        return managed
    if daedalus_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_daedalus_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Daedalus-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_daedalus_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``daedalus update`` — leaving a **dangling symlink** that ``which`` still
...
def display_daedalus_home() -> str:
    """Return a user-friendly display string for the current DAEDALUS_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.daedalus``
        profile:  ``~/.daedalus/profiles/coder``
        custom:   ``/opt/daedalus-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.daedalus``.  For code that needs a real ``Path``, use
    :func:`get_daedalus_home` instead.
    """
    home = get_daedalus_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``DAEDALUS_HOME`` or other path
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{DAEDALUS_HOME}/home`` when the profile-home directory exists."""
    daedalus_home = get_daedalus_home_override() or (env or {}).get("DAEDALUS_HOME") or os.getenv("DAEDALUS_HOME")
    if not daedalus_home:
        return None
    profile_home = os.path.join(daedalus_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("DAEDALUS_REAL_HOME") or os.getenv("DAEDALUS_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Daedalus profile HOME.

    ``DAEDALUS_HOME`` scopes Daedalus state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={DAEDALUS_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{DAEDALUS_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{DAEDALUS_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Daedalus' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["DAEDALUS_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Daedalus itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: NousResearch/daedalus#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under DAEDALUS_HOME.

    Replaces the ``get_daedalus_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, daedalus_logging.py, daedalus_time.py, etc.).
    """
    return get_daedalus_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under DAEDALUS_HOME."""
    return get_daedalus_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under DAEDALUS_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    _ipv4_getaddrinfo._daedalus_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``daedalus_cli`` modules using three different Windows
    ...
    A few sites outside ``daedalus_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``daedalus_cli.main._is_windows`` and friends);
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Daedalus.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("daedalus_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `daedalus update` is exactly
    ...
    return [
        ...
        "    daedalus update",
        "If that also fails, reinstall: https://hermes.nousresearch.com",
    ]
