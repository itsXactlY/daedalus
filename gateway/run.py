...
# Load environment variables from ~/.daedalus/.env first.
# User-managed env files should override stale shell exports on restart.
from dotenv import load_dotenv  # backward-compat for tests that monkeypatch this symbol
from daedalus_cli.env_loader import load_daedalus_dotenv
_env_path = _daedalus_home / '.env'
load_daedalus_dotenv(daedalus_home=_daedalus_home, project_env=Path(__file__).resolve().parents[1] / '.env')

# Bridge config.yaml values into the environment so os.getenv() picks them up.
# config.yaml is authoritative for terminal settings — overrides .env.
_config_path = _daedalus_home / 'config.yaml'
    ...
    try:
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            if "max_turns" in _agent_cfg:
                os.environ["DAEDALUS_MAX_ITERATIONS"] = str(_agent_cfg["max_turns"])
            # Bridge agent.gateway_timeout → DAEDALUS_AGENT_TIMEOUT env var.
            # Env var from .env takes precedence (already in os.environ).
            if "gateway_timeout" in _agent_cfg and "DAEDALUS_AGENT_TIMEOUT" not in os.environ:
                os.environ["DAEDALUS_AGENT_TIMEOUT"] = str(_agent_cfg["gateway_timeout"])
        # Timezone: bridge config.yaml → DAEDALUS_TIMEZONE env var.
        # DAEDALUS_TIMEZONE from .env takes precedence (already in os.environ).
        _tz_cfg = _cfg.get("timezone", "")
        if _tz_cfg and isinstance(_tz_cfg, str) and "DAEDALUS_TIMEZONE" not in os.environ:
            os.environ["DAEDALUS_TIMEZONE"] = _tz_cfg.strip()
        # Security settings
        _security_cfg = _cfg.get("security", {})
        if isinstance(_security_cfg, dict):
            _redact = _security_cfg.get("redact_secrets")
            if _redact is not None:
                os.environ["DAEDALUS_REDACT_SECRETS"] = str(_redact).lower()
    except Exception:
...
os.environ["DAEDALUS_EXEC_ASK"] = "1"
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    from daedalus_cli.runtime_provider import (
    ...
    try:
        runtime = resolve_runtime_provider(
            requested=os.getenv("DAEDALUS_INFERENCE_PROVIDER"),
        )
            ...
            for skill_md in skills_dir.rglob("SKILL.md"):
                ...
                if name == normalized and name in disabled:
                    return (
                        ...
                        f"Enable it with: `daedalus skills config`"
                    )

        # Check optional skills (shipped with repo but not installed)
        from daedalus_constants import get_optional_skills_dir
            ...
            for skill_md in optional_dir.rglob("SKILL.md"):
                ...
                if name == normalized:
                    ...
                    return (
                        ...
                        f"Install it with: `daedalus skills install {install_path}`"
                    )
    except Exception:
        pass
    return None
...
def _load_gateway_config() -> dict:
    """Load and parse ~/.daedalus/config.yaml, returning {} on any error."""
    try:
        config_path = _daedalus_home / 'config.yaml'
        if config_path.exists():
            import yaml
            with open(config_path, 'r', encoding='utf-8') as f:
                return yaml.safe_load(f) or {}
    except Exception:
        logger.debug("Could not load gateway config from %s", _daedalus_home / 'config.yaml')
    return {}
...
def _resolve_daedalus_bin() -> Optional[list[str]]:
    """Resolve the Daedalus update command as argv parts.

    Tries in order:
    1. ``shutil.which("daedalus")`` — standard PATH lookup
    2. ``sys.executable -m daedalus_cli.main`` — fallback when Daedalus is running
       from a venv/module invocation and the ``daedalus`` shim is not on PATH
    ...
    try:
        ...
        if importlib.util.find_spec("daedalus_cli") is not None:
            return [sys.executable, "-m", "daedalus_cli.main"]
    except Exception:
        pass
...
class GatewayRunner:
    ...
    def _has_setup_skill(self) -> bool:
        """Check if the daedalus-setup skill is installed."""
        try:
            from tools.skill_manager_tool import _find_skill
            return _find_skill("daedalus-setup") is not None
        except Exception:
            return False

    # -- Voice mode persistence ------------------------------------------

    _VOICE_MODE_PATH = _daedalus_home / "gateway_voice_mode.json"
    ...
    def _sync_voice_mode_state_to_adapter(self, adapter) -> None:
        ...
        # Lazy import to avoid adding a module-level dep from gateway → daedalus_cli.
        try:
            from daedalus_cli.config import load_config as _load_full_config
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        Checks DAEDALUS_PREFILL_MESSAGES_FILE env var first, then falls back to
        the prefill_messages_file key in ~/.daedalus/config.yaml.
        Relative paths are resolved from ~/.daedalus/.
        """
        import json as _json
        file_path = os.getenv("DAEDALUS_PREFILL_MESSAGES_FILE", "")
        if not file_path:
            try:
                import yaml as _y
                cfg_path = _daedalus_home / "config.yaml"
                if cfg_path.exists():
    ...
    def _load_ephemeral_system_prompt() -> str:
        ...
        Checks DAEDALUS_EPHEMERAL_SYSTEM_PROMPT env var first, then falls back to
        agent.system_prompt in ~/.daedalus/config.yaml.
        """
        prompt = os.getenv("DAEDALUS_EPHEMERAL_SYSTEM_PROMPT", "")
        if prompt:
            return prompt
        try:
            import yaml as _y
    ...
    def _load_background_notifications_mode() -> str:
        ...
        """
        mode = os.getenv("DAEDALUS_BACKGROUND_NOTIFICATIONS", "")
        if not mode:
            try:
                import yaml as _y
                cfg_path = _daedalus_home / "config.yaml"
    ...
    async def start(self) -> bool:
        ...
        """
        logger.info("Starting Daedalus Gateway...")
        ...
        if not _any_allowlist and not _allow_all:
            logger.warning(
                ...
                "Set GATEWAY_ALLOW_ALL_USERS=true in ~/.daedalus/.env to allow open access, "
                "or configure platform allowlists (e.g., TELEGRAM_ALLOWED_USERS=your_id)."
            )
        
        # Discover Python plugins (platform adapters among them) before event
        # hooks.  The CLI startup path does this via an explicit call in
        # daedalus_cli/main.py; the gateway's systemd entry point
        # (`daedalus_cli.main gateway run`) does not go through that path, so
        # plugin platform adapters (e.g. plugins/platforms/discord/adapter.py)
        # would otherwise never get registered with platform_registry.
        try:
        ...
        if not notified and any(
            path.exists()
            for path in (
                _daedalus_home / ".update_pending.json",
                _daedalus_home / ".update_pending.claimed.json",
            )
        ):
    ...
    def _create_adapter(
        ...
        # Discord is now plugin-registered (plugins/platforms/discord/adapter.py)
        # rather than the legacy gateway/platforms/discord.py monolith.
        try:
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
                ...
                if code:
                    ...
                    if adapter:
                        await adapter.send(
                            ...
                            f"`daedalus pairing approve {platform_name} {code}`"
                        )
                else:
                    adapter = self.adapters.get(source.platform)
                    if adapter:
        ...
        if _update_prompts.get(_quick_key):
            ...
            if cmd in ("approve", "yes"):
                ...
                response_text = raw
            if response_text:
                response_path = _daedalus_home / ".update_response"
                try:
        ...
        _raw_stale_timeout = float(os.getenv("DAEDALUS_AGENT_TIMEOUT", 1800))
        ...
        if _quick_key in self._running_agents:
            ...
            from daedalus_cli.commands import resolve_command as _resolve_cmd_inner
        ...
        # in daedalus_cli/commands.py — no hardcoded set to maintain here.
        from daedalus_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command as _resolve_cmd
        ...
        if command:
            try:
                ...
                # hyphens. See daedalus_cli/commands.py:_build_telegram_menu.
        ...
        # loads multiple skills at once, mirroring cli.py's elif ordering
        # where the bundle check comes before the skill check. A bundle
        # whose slash name shadows a skill wins: `command = None` below
        # makes the skill dispatch skip so it can't clobber the bundle
        # message we just built.
        if command:
            try:
                ...
                if bundle_key is not None:
                    user_instruction = event.get_command_args().strip()
                    # build_bundle_invocation_message() applies the global +
                    # per-platform disabled-skill gate internally and keeps
                    # loading the other members when one is disabled/skipped
                    # (noting it in the message header), mirroring cli.py's
                    # load-what-you-can behavior.
                    ...
                    if bundle_result:
                        ...
                        if missing:
                            logger.info(
                                "Skill bundle %s loaded %d skills; missing: %s",
                                bundle_key,
                                len(loaded_names),
                                ", ".join(missing),
                            )
                        event.text = msg
                        # Fall through to normal message processing with the
                        # bundle's skill content.
                        command = None
                    else:
                        # Nothing could be loaded (all members missing or
                        # disabled) — surface a clear message instead of
                        # silently falling through to free-text handling.
                        return (
                            f"Bundle `{bundle_key}` has no loadable skills "
                            f"(all referenced skills are missing or disabled)."
                        )
            except Exception as e:
                logger.debug("Bundle command check failed (non-fatal): %s", e)
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        if history and len(history) >= 4:
            ...
            _hyg_api_key = None
            try:
                _hyg_cfg_path = _daedalus_home / "config.yaml"
            ...
            if not os.getenv(env_key):
                ...
                if adapter:
                    await adapter.send(
                        ...
                        f"A home channel is where Daedalus delivers cron job results "
                    ...
                    if _stt_adapter:
                        try:
                            _stt_msg = (
                                ...
                                "(`pip install faster-whisper` in the Daedalus venv) "
                                "and set `stt.enabled: true` in config.yaml, "
                                "then /restart the gateway."
                            )
                            # Point to setup skill if it's installed
                            if self._has_setup_skill():
                                _stt_msg += "\n\nFor full setup instructions, type: `/skill daedalus-setup`"
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        # Detect profile name from DAEDALUS_HOME path
        # Profile paths look like: ~/.daedalus/profiles/<name>
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from daedalus_cli.commands import gateway_help_lines
        lines = [
            "📖 **Daedalus Commands**\n",
            *gateway_help_lines(),
        ]
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        if persist_global:
            try:
                ...
                from daedalus_cli.config import save_config
                save_config(cfg)
            except Exception as e:
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        lines.append("Setup: `daedalus setup`")
        return "\n".join(lines)
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return "No personalities configured in `~/.daedalus/config.yaml`"
        ...
        try:
            ...
            if "pynacl" in err_lower or "nacl" in err_lower or "davey" in err_lower:
                return (
                    ...
                    "Install or reinstall Daedalus with the messaging extra, e.g. "
                    "`pip install daedalus[messaging]`."
                )
    ...
    async def _handle_voice_channel_input(
        ...
        # the same per-channel context as typed messages.
        channel_prompt = None
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _daedalus_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _run_background_task(
        ...
        try:
            ...
            max_iterations = int(os.getenv("DAEDALUS_MAX_ITERATIONS", "90"))
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        if not args:
            ...
            return (
                ...
                "_Usage:_ `/reasoning <none|low|medium|high|xhigh|show|hide|auto>`"
            )
        ...
        if args == "auto":
            # Fork feature: the task-intensity gauge (agent/task_intensity.py)
            # is wired into the CLI agent loop only. Gateway sessions resolve
            # reasoning per-session/per-model from config; give an honest
            # answer instead of "unknown argument".
            return (
                "🤖 Auto reasoning (task-intensity gauge) is a CLI-side "
                "feature in this fork. On gateway platforms set the level "
                "explicitly, e.g. `/reasoning low` — or enable the gauge "
                "globally with `agent.reasoning_auto: true` in config.yaml "
                "and run through the CLI."
            )
    ...
    _UPDATE_ALLOWED_PLATFORMS = frozenset({
        ...
        Platform.WECOM, Platform.LOCAL,
    })

    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Daedalus Agent to the latest version.

        Spawns ``daedalus update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``daedalus update`` may trigger. Marker
        ...
        import subprocess
        from datetime import datetime
        from daedalus_cli.config import is_managed, format_managed_message

        # Block non-messaging platforms (API server, webhooks, ACP)
        platform = event.source.platform
        if platform not in self._UPDATE_ALLOWED_PLATFORMS:
            return "✗ /update is only available from messaging platforms. Run `daedalus update` from the terminal."

        if is_managed():
            return f"✗ {format_managed_message('update Daedalus Agent')}"
        ...
        daedalus_cmd = _resolve_daedalus_bin()
        if not daedalus_cmd:
            return (
                "✗ Could not locate the `daedalus` command. "
                "Daedalus is running, but the update command could not find the "
                "executable on PATH or via the current Python interpreter. "
                "Try running `daedalus update` manually in your terminal."
            )

        pending_path = _daedalus_home / ".update_pending.json"
        output_path = _daedalus_home / ".update_output.txt"
        exit_code_path = _daedalus_home / ".update_exit_code"
        ...
        # Spawn `daedalus update --gateway` detached so it survives gateway restart.
        ...
        daedalus_cmd_str = " ".join(shlex.quote(part) for part in daedalus_cmd)
        update_cmd = (
            f"PYTHONUNBUFFERED=1 {daedalus_cmd_str} update --gateway"
        ...
        self._schedule_update_notification_watch()
        return "⚕ Starting Daedalus update… I'll stream progress here."
    ...
    async def _watch_update_progress(
        ...
        """Watch ``daedalus update --gateway``, streaming output + forwarding prompts.
        ...
        pending_path = _daedalus_home / ".update_pending.json"
        claimed_path = _daedalus_home / ".update_pending.claimed.json"
        output_path = _daedalus_home / ".update_output.txt"
        exit_code_path = _daedalus_home / ".update_exit_code"
        prompt_path = _daedalus_home / ".update_prompt.json"
        ...
        while loop.time() < deadline:
            ...
            if exit_code_path.exists():
                ...
                try:
                    ...
                    if exit_code == 0:
                        await adapter.send(chat_id, "✅ Daedalus update finished.")
                    else:
                        await adapter.send(chat_id, "❌ Daedalus update failed (exit code {}).".format(exit_code))
                ...
                (_daedalus_home / ".update_response").unlink(missing_ok=True)
        ...
        if not exit_code_path.exists():
            ...
            await _flush_buffer()
            try:
                await adapter.send(chat_id, "❌ Daedalus update timed out after 30 minutes.")
            except Exception:
                pass
            for p in (pending_path, claimed_path, output_path,
                      exit_code_path, prompt_path):
            ...
            if adapter and chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    if exit_code == 0:
                        msg = f"✅ Daedalus update finished.\n\n```\n{output}\n```"
                    else:
                        msg = f"❌ Daedalus update failed.\n\n```\n{output}\n```"
                else:
                    if exit_code == 0:
                        msg = "✅ Daedalus update finished successfully."
                    else:
                        msg = "❌ Daedalus update failed. Check the gateway logs or run `daedalus update` manually for details."
    ...
    def _set_session_env(self, context: SessionContext) -> None:
        ...
        os.environ["DAEDALUS_SESSION_PLATFORM"] = context.source.platform.value
        os.environ["DAEDALUS_SESSION_CHAT_ID"] = context.source.chat_id
        if context.source.chat_name:
            os.environ["DAEDALUS_SESSION_CHAT_NAME"] = context.source.chat_name
        if context.source.thread_id:
            os.environ["DAEDALUS_SESSION_THREAD_ID"] = str(context.source.thread_id)
    
    def _clear_session_env(self) -> None:
        """Clear session environment variables."""
        for var in ["DAEDALUS_SESSION_PLATFORM", "DAEDALUS_SESSION_CHAT_ID", "DAEDALUS_SESSION_CHAT_NAME", "DAEDALUS_SESSION_THREAD_ID"]:
    ...
    async def _enrich_message_with_transcription(
        ...
        if not getattr(self.config, "stt_enabled", True):
            ...
            if self._has_setup_skill():
                disabled_note += (
                    " You have a skill called daedalus-setup that can help "
                    "users configure Daedalus features including voice, tools, and more."
                )
            disabled_note += "]"
            if user_text:
            ...
            try:
                ...
                if result["success"]:
                    ...
                    if (
                        ...
                        if self._has_setup_skill():
                            _no_stt_note += (
                                " You have a skill called daedalus-setup "
                                "that can help users configure Daedalus features "
    ...
    async def _run_agent(
        ...
        progress_mode = (
            _raw_tp
            or os.getenv("DAEDALUS_TOOL_PROGRESS_MODE")
            or "all"
        )
        ...
        def run_sync():
            ...
            os.environ["DAEDALUS_SESSION_KEY"] = session_key or ""
        ...
        try:
            ...
            # DAEDALUS_AGENT_TIMEOUT env var (env var takes precedence).
            # Default 1800s (30 min inactivity).  0 = unlimited.
            _agent_timeout_raw = float(os.getenv("DAEDALUS_AGENT_TIMEOUT", 1800))
            ...
            if pending and pending.strip().startswith("/"):
                ...
                if _pending_cmd_word:
                    try:
                        from daedalus_cli.commands import resolve_command as _rc_pending
...
def _start_cron_ticker(stop_event: threading.Event, adapters=None, loop=None, interval: int = 60):
    ...
    needing a separate `daedalus cron daemon` or system cron entry.
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Prevent two gateways from running under the same DAEDALUS_HOME.
    # The PID file is scoped to DAEDALUS_HOME, so future multi-profile
    # setups (each profile using a distinct DAEDALUS_HOME) will naturally
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            logger.error(
                "Another gateway instance is already running (PID %d, DAEDALUS_HOME=%s). "
                "Use 'daedalus gateway restart' to replace it, or 'daedalus gateway stop' first.",
                existing_pid, daedalus_home,
            )
            print(
                f"\n❌ Gateway already running (PID {existing_pid}).\n"
                f"   Use 'daedalus gateway restart' to replace it,\n"
                f"   or 'daedalus gateway stop' to kill it first.\n"
                f"   Or use 'daedalus gateway run --replace' to auto-replace.\n"
            )
            return False
    ...
    log_dir = setup_logging(daedalus_home=_daedalus_home, mode="gateway")
    ...
    from daedalus_logging import _add_rotating_handler
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Daedalus Gateway - Multi-platform messaging")
