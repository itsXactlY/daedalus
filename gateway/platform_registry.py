...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``daedalus setup`` display).
    ...
    # built-ins).  Used by ``daedalus gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `daedalus`
        # invocation -- including plain `daedalus chat`, which never touches any
        ...
        # `daedalus setup`/`gateway status`, send_message).
