...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `daedalus tools` (opt-in, default OFF).
    ...
    Reads the token through agent.secret_scope so a multiplex profile's
    session sees only its own scoped secrets, not the process-wide
    environment — a scope with an unrelated secret set must not leak a
    different profile's DISCORD_BOT_TOKEN into this check.
