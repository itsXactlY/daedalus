---
name: hermes-skill-supply-chain-audit
title: Hermes Skill Supply-Chain Audit
description: Audits installed Hermes skills against the agent skill marketplace threat taxonomy from arXiv 2605.28588 (76 confirmed malicious payloads in 3,984 ClawHub.ai skills; 13.4% with critical issues). Provides a read-only scan procedure and 5 concrete red-flag patterns to grep for in SKILL.md files.
...
**Why this matters for Hermes:** any third-party skill installed into `~/.hermes/skills/` runs with the same privileges as the agent. A single malicious SKILL.md that triggers shell execution can compromise credentials, install persistent backdoors, or silently exfiltrate data across the operator's entire session.
...
- Matches inside this skill (`hermes-skill-supply-chain-audit/SKILL.md`) and inside `hermes-mcp-security-audit/SKILL.md` are expected and informational — they document the threat, not invoke it.
...
- A scan report file (e.g., `/tmp/hermes-skill-scan-YYYY-MM-DD.txt`) must exist and contain one line per match in the format `<skill-name>\t<line-number>\t<matching-line>`.
