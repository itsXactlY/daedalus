...
def _discover_tools():
    ...
    AST-scans tools/*.py for a top-level ``registry.register(...)`` call
    (tools.registry.discover_builtin_tools) instead of maintaining a
    hand-curated module list — a hardcoded list silently drifts as tool
    files are added (this one had: it was missing discord_tool,
    homeassistant_tool, image_generation_tool, mixture_of_agents_tool,
    tts_tool, rl_training_tool, browser_cdp_tool, browser_dialog_tool,
    leaving their toolsets configurable via ``daedalus tools`` but
    permanently non-functional with no error surfaced anywhere).

    Wrapped in a function so import errors in optional tools (e.g., fal_client
    not installed) don't prevent the rest from loading — discover_builtin_tools
    already logs+skips per-module import failures internally.
    """
...
def handle_function_call(
    ...
    try:
        ...
        try:
            from daedalus_cli.plugins import invoke_hook
            invoke_hook(
                "pre_tool_call",
