...
def backup_env(monkeypatch, tmp_path):
    """Isolate DAEDALUS_HOME + reload modules so every test starts clean."""
    ...
    # Reload so get_daedalus_home picks up the env var fresh.
    import daedalus_constants
    importlib.reload(daedalus_constants)
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd daedalus_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under DAEDALUS_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level DAEDALUS_DIR picks up the tmp HOME."""
    import daedalus_constants
    importlib.reload(daedalus_constants)
