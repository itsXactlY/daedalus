...
class TestPricing:
    ...
    def test_custom_endpoint_model_zero_cost(self):
        ...
        for model in ["FP16_Daedalus_4.5", "Daedalus_4.5_1T_epoch2", "my-local-llama"]:
...
class TestHasKnownPricing:
    ...
    def test_unknown_custom_model(self):
        assert _has_known_pricing("FP16_Daedalus_4.5") is False
...
class TestFormatDuration:
    ...
    def test_days(self):
        result = _format_duration(172800)
        assert result == "2.0d"
...
class TestTerminalFormatting:
    def test_terminal_format_has_sections(self, populated_db):
        ...
        assert "Daedalus Insights" in text
...
class TestEdgeCases:
    ...
    def test_custom_model_shows_zero_cost(self, db):
        ...
        db.create_session(session_id="s1", source="cli", model="FP16_Daedalus_4.5")
        ...
        report = engine.generate(days=30)
        assert report["overview"]["estimated_cost"] == 0.0
        assert "FP16_Daedalus_4.5" in report["overview"]["models_without_pricing"]

        models = report["models"]
        custom = next(m for m in models if m["model"] == "FP16_Daedalus_4.5")
    ...
    def test_mixed_commercial_and_custom_models(self, db):
        ...
        assert "claude-sonnet-4-20250514" in overview["models_with_pricing"]
