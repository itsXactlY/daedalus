...
def test_normalize_usage_flags_missing_prompt_tokens_field():
    """Regression test: a real usage object that omits prompt_tokens (seen with
    some OpenAI-compatible providers, e.g. DeepSeek, on a subset of streamed
    responses) must be distinguishable from a genuine, present, zero value —
    callers (run_agent.py's session-cumulative counters) rely on this flag to
    avoid silently under-adding for that call instead of estimating from the
    last known-good context size. See commit fixing session token undercount.
    """
    usage = SimpleNamespace(
    ...
    assert normalized.prompt_field_missing is True
    assert normalized.prompt_tokens == 0
    assert normalized.output_tokens == 42


def test_normalize_usage_does_not_flag_a_genuine_present_zero():
    usage = SimpleNamespace(
        prompt_tokens=0,
        completion_tokens=0,
    )
    ...
    assert normalized.prompt_field_missing is False
    assert normalized.prompt_tokens == 0
