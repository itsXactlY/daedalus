...
def curator_env(tmp_path, monkeypatch):
    """Isolated DAEDALUS_HOME + freshly reloaded curator + skill_usage modules."""
...
def _write_bundle_file(home, slug: str, skill_ref: str):
    """Write a real skill-bundles/*.yaml referencing *skill_ref* directly.

    Deliberately does NOT stub _bundle_referenced_skills() -- exercises the
    real agent.skill_bundles.get_skill_bundles() lookup end to end, mirroring
    _write_cron_job's realism for the cron protection tests.
    """
    ...
    (bundles_dir / f"{slug}.yaml").write_text(
        _yaml.safe_dump({"name": slug, "skills": [skill_ref]}),
        encoding="utf-8",
    )


def test_candidate_list_marks_bundle_referenced_skills(curator_env, monkeypatch):
    """The LLM review candidate list flags bundle-referenced skills so the
    review pass knows not to prune them, same as the cron column."""
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
    _write_skill(skills_dir, "bundle-dep")
    _write_skill(skills_dir, "plain")
    _backdate(u, "bundle-dep", 1)
    _backdate(u, "plain", 1)
    monkeypatch.setattr(c, "_bundle_referenced_skills", lambda: {"bundle-dep"})

    listing = c._render_candidate_list()
    bundle_line = next(l for l in listing.splitlines() if l.startswith("- bundle-dep"))
    plain_line = next(l for l in listing.splitlines() if l.startswith("- plain"))
    assert "bundle=yes" in bundle_line
    assert "bundle=no" in plain_line


def test_bundle_referenced_skill_survives_inactivity(curator_env):
    """A skill that's a member of a real skill-bundles/*.yaml file must not
    be archived out from under the bundle -- the next /<bundle-slug>
    invocation would silently drop it into the "missing" list otherwise."""
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
    _write_skill(skills_dir, "review-helper")
    _backdate(u, "review-helper", 200)
    _write_bundle_file(curator_env["home"], "review-stack", "review-helper")

    counts = c.apply_automatic_transitions()

    assert counts["archived"] == 0
    assert u.load_usage()["review-helper"]["state"] == u.STATE_ACTIVE


def test_bundle_referenced_skills_helper_is_resilient_to_bad_bundle_dir(curator_env, monkeypatch):
    """A corrupt/unreadable bundle store must never crash the curator --
    mirrors _cron_referenced_skills' best-effort contract."""
    c = curator_env["curator"]
    import agent.skill_bundles as bundles_mod
    monkeypatch.setattr(bundles_mod, "get_skill_bundles", lambda: (_ for _ in ()).throw(OSError("boom")))
    assert c._bundle_referenced_skills() == set()
...
def test_cli_pin_refuses_bundled_skill(curator_env, capsys):
    from daedalus_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
def test_curator_slot_is_canonical_aux_task():
    """Curator must be a first-class aux-task slot.

    The 0.20→0.8.0 port removed the legacy ``_AUX_TASKS`` (CLI picker) and
    ``_AUX_TASK_SLOTS`` (web_server allowlist) registries — the unified
    source of truth is ``DEFAULT_CONFIG['auxiliary']['curator']`` (schema)
    plus the ``daedalus curator`` subcommand registration. This test pins the
    slot so the unification doesn't silently regress.
    """
    ...
    assert hasattr(_curator_cli, "register_cli"), \
        "daedalus_cli.curator.register_cli missing (CLI wiring)"
...
def test_review_fork_forwards_runtime_pool_and_overrides(curator_env, monkeypatch):
    ...
    monkeypatch.setattr(
        "daedalus_cli.config.load_config_readonly",
        lambda: {"model": {"provider": "custom:hyper-charm", "default": "glm-5.2"}},
    )
    monkeypatch.setattr(
...
def test_review_fork_toolset_surface_is_skills_plus_terminal():
    ...
    Resolves the toolsets directly (``resolve_toolset`` no longer takes
    ``include_registry`` — the 0.20→0.8.0 port simplified the signature to
    ``(name, visited=None)``; the old kwarg-gated plugin registry is gone).
    This documents the intended surface (the four prompt-named tools
    present, dead default and lcm_* schema absent) but does not itself
    guard the call-site kwarg. No exact-set pin: intentional additions to
    either toolset must not fail this test.
    """
    from toolsets import resolve_toolset

    surface = set(resolve_toolset("skills")) | set(
