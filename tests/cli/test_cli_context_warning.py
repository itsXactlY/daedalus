...
def _isolate(tmp_path, monkeypatch):
    """Isolate DAEDALUS_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal DaedalusCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = DaedalusCLI.__new__(DaedalusCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["daedalus-core"]
        obj.compact = False
