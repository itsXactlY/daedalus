...
def test_daedalus_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in DaedalusCLI.__init__")

    monkeypatch.setattr("daedalus_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("daedalus_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.DaedalusCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_cli_turn_routing_uses_cheap_model_when_simple(monkeypatch):
    ...
    shell = cli.DaedalusCLI(model="anthropic/claude-sonnet-4", compact=True, max_turns=1)
    shell.provider = "openrouter"
    shell.api_mode = "chat_completions"
    shell.base_url = "https://openrouter.ai/api/v1"
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    monkeypatch.setenv("DAEDALUS_INFERENCE_PROVIDER", "openrouter")
    ...
    shell = cli.DaedalusCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    monkeypatch.setenv("DAEDALUS_ENABLE_NOUS_MANAGED_TOOLS", "1")
    config = {
    ...
    monkeypatch.setattr(
        "daedalus_cli.auth.get_provider_auth_state",
        lambda provider: {"access_token": "nous-token"},
    )
    monkeypatch.setattr(
        "daedalus_cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "daedalus_cli.auth.fetch_nous_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("daedalus_cli.auth._prompt_model_selection", lambda model_ids, current_model="", pricing=None, **kw: "claude-opus-4-6")
    monkeypatch.setattr("daedalus_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("daedalus_cli.auth._update_config_for_provider", lambda provider, url: None)
    monkeypatch.setattr(
        "daedalus_cli.nous_subscription.get_nous_subscription_explainer_lines",
        lambda: ["Nous subscription enables managed web tools."],
    )

    daedalus_main._model_flow_nous(config, current_model="claude-opus-4-6")
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.DaedalusCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.DaedalusCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("daedalus_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("daedalus_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("daedalus_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(daedalus_main, "_prompt_provider_choice", lambda choices, **kwargs: len(choices) - 1)
    monkeypatch.setattr("sys.stdin", type("FakeTTY", (), {"isatty": lambda self: True})())

    daedalus_main.cmd_model(SimpleNamespace())
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "daedalus_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("daedalus_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("daedalus_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("daedalus_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("daedalus_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "daedalus_cli.models.probe_api_models",
    ...
    daedalus_main._model_flow_custom({})
    output = capsys.readouterr().out
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    monkeypatch.setattr(daedalus_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "daedalus_cli.config.load_config",
    ...
    monkeypatch.setattr("daedalus_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("daedalus_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("daedalus_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("daedalus_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(daedalus_main, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    ...
    monkeypatch.setattr("daedalus_cli.auth._login_nous", _fake_login)

    daedalus_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.nousresearch.com",
            inference_url="https://inference.nousresearch.com/v1",
            client_id="daedalus-local",
