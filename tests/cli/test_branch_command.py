...
def session_db(tmp_path):
    ...
    os.environ["DAEDALUS_HOME"] = str(tmp_path / ".daedalus")
    os.makedirs(tmp_path / ".daedalus", exist_ok=True)
    from daedalus_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".daedalus" / "test_sessions.db")
    yield db
    db.close()
...
class TestBranchCommandCLI:
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        DaedalusCLI._handle_branch_command(cli_instance, "/branch refactor approach")
...
class TestBranchCommandDef:
    ...
    def test_branch_in_registry(self):
        ...
        from daedalus_cli.commands import COMMAND_REGISTRY
