"""Tests for daedalus_logging — centralized logging setup."""
...
@pytest.fixture
def daedalus_home(tmp_path, monkeypatch):
    """Provide an isolated DAEDALUS_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_daedalus_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["DAEDALUS_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, daedalus_home):
        log_dir = daedalus_logging.setup_logging(daedalus_home=daedalus_home)
        assert log_dir == daedalus_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, daedalus_home):
    ...
    def test_creates_errors_log_handler(self, daedalus_home):
        daedalus_logging.setup_logging(daedalus_home=daedalus_home)
        root = logging.getLogger()
    ...
    def test_custom_log_level(self, daedalus_home):
        daedalus_logging.setup_logging(daedalus_home=daedalus_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, daedalus_home):
        daedalus_logging.setup_logging(
            daedalus_home=daedalus_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, daedalus_home):
        daedalus_logging.setup_logging(daedalus_home=daedalus_home)
    ...
    def test_writes_to_agent_log(self, daedalus_home):
        daedalus_logging.setup_logging(daedalus_home=daedalus_home)
    ...
    def test_warnings_appear_in_both_logs(self, daedalus_home):
        daedalus_logging.setup_logging(daedalus_home=daedalus_home)

        test_logger = logging.getLogger("test_daedalus_logging.warning_test")
        ...
        agent_log = daedalus_home / "logs" / "agent.log"
        errors_log = daedalus_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, daedalus_home):
        daedalus_logging.setup_logging(daedalus_home=daedalus_home)

        test_logger = logging.getLogger("test_daedalus_logging.info_test")
    ...
    def test_explicit_params_override_config(self, daedalus_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (daedalus_home / "config.yaml").write_text(yaml.dump(config))

        daedalus_logging.setup_logging(daedalus_home=daedalus_home, log_level="WARNING")
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, daedalus_home):
        daedalus_logging.setup_logging(daedalus_home=daedalus_home)
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_daedalus_verbose", False)
        ]
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        daedalus_logging._add_rotating_handler(
            logger, log_path,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, daedalus_home):
        level, max_size, backup = daedalus_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, daedalus_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, daedalus_home):
        import yaml
        config = {"model": "test"}
        (daedalus_home / "config.yaml").write_text(yaml.dump(config))
