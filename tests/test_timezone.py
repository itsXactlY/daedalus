"""
Tests for timezone support (daedalus_time module + integration points).
...
class TestDaedalusTimeNow:
    ...
    def teardown_method(self):
        daedalus_time.reset_cache()
        os.environ.pop("DAEDALUS_TIMEZONE", None)
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["DAEDALUS_TIMEZONE"] = "UTC"
        result = daedalus_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["DAEDALUS_TIMEZONE"] = "UTC"
        daedalus_time.reset_cache()
        r1 = daedalus_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["DAEDALUS_TIMEZONE"] = "Asia/Kolkata"
        daedalus_time.reset_cache()
        r2 = daedalus_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["DAEDALUS_TIMEZONE"] = "Europe/London"
        tz = daedalus_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["DAEDALUS_TIMEZONE"] = "Not/A/Timezone"
        tz = daedalus_time.get_timezone()
        assert tz is None

    def test_get_timezone_name(self):
        os.environ["DAEDALUS_TIMEZONE"] = "Asia/Tokyo"
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When DAEDALUS_TIMEZONE is set, child process sees TZ env var."""
        import json as _json
        os.environ["DAEDALUS_TIMEZONE"] = "Asia/Kolkata"
    ...
    def test_tz_not_injected_when_empty(self):
        """When DAEDALUS_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("DAEDALUS_TIMEZONE", None)
    ...
    def test_daedalus_timezone_not_leaked_to_child(self):
        """DAEDALUS_TIMEZONE itself must NOT appear in child env (only TZ)."""
        import json as _json
        os.environ["DAEDALUS_TIMEZONE"] = "Asia/Kolkata"

        with patch("model_tools.handle_function_call", side_effect=self._mock_handle):
            result = _json.loads(self._execute_code(
                code='import os; print(os.environ.get("DAEDALUS_TIMEZONE", "NOT_SET"))',
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=daedalus_tz) which shifted
        absolute time when system-local tz != Daedalus tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_daedalus_tz(self):
        """Already-aware datetimes should be normalized to Daedalus tz."""
        from cron.jobs import _ensure_aware
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Daedalus tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Daedalus is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Daedalus tz
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Daedalus timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["DAEDALUS_TIMEZONE"] = "US/Eastern"
        daedalus_time.reset_cache()
