    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.daedalus.gateway"])
            ),
        )
...
class TestSystemUnitDaedalusHome:
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'DAEDALUS_HOME=/home/alice/.daedalus' in unit
    ...
    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        ...
        assert 'DAEDALUS_HOME=/home/alice/.daedalus/profiles/coder' in unit
    ...
    def test_user_unit_unaffected_by_change(self):
        ...
        assert f'DAEDALUS_HOME={daedalus_home}' in unit


class TestDaedalusHomeForTargetUser:
    """Unit tests for _daedalus_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("DAEDALUS_HOME", raising=False)

        result = gateway_cli._daedalus_home_for_target_user("/home/alice")
        assert result == "/home/alice/.daedalus"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._daedalus_home_for_target_user("/home/alice")
        assert result == "/home/alice/.daedalus/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("DAEDALUS_HOME", "/opt/daedalus")

        result = gateway_cli._daedalus_home_for_target_user("/home/alice")
        assert result == "/opt/daedalus"
...
class TestProfileArg:
    ...
    def test_default_daedalus_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.daedalus should not produce a --profile flag."""
        daedalus_home = tmp_path / ".daedalus"
        daedalus_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        result = gateway_cli._profile_arg(str(daedalus_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.daedalus/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".daedalus" / "profiles" / "mybot"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile DAEDALUS_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "daedalus"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.daedalus/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".daedalus" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".daedalus" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_daedalus_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
