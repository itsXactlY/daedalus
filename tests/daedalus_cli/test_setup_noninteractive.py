...
class TestNonInteractiveSetup:
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("daedalus_cli.setup.ensure_daedalus_home"),
            patch("daedalus_cli.setup.load_config", return_value={}),
            patch("daedalus_cli.setup.get_daedalus_home", return_value="/tmp/.daedalus"),
            patch("daedalus_cli.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            run_setup_wizard(args)

        out = capsys.readouterr().out
        assert "daedalus config set model.provider custom" in out
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `daedalus` should not prompt for input when no provider exists and stdin is headless."""
        from daedalus_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("daedalus_cli.main._has_any_provider_configured", return_value=False),
            patch("daedalus_cli.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
    ...
    def test_returning_user_terminal_menu_choice_dispatches_terminal_section(self, tmp_path):
        ...
        with (
            patch.object(setup_mod, "ensure_daedalus_home"),
            patch.object(setup_mod, "load_config", return_value=config),
            patch.object(setup_mod, "get_daedalus_home", return_value=tmp_path),
