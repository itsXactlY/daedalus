...
from daedalus_cli.codex_models import DEFAULT_CODEX_MODELS, get_codex_model_ids
...
def test_get_codex_model_ids_adds_forward_compat_models_from_templates(monkeypatch):
    monkeypatch.setattr(
        "daedalus_cli.codex_models._fetch_models_from_api",
...
def test_model_command_uses_runtime_access_token_for_codex_list(monkeypatch):
    from daedalus_cli.main import _model_flow_openai_codex

    captured = {}

    monkeypatch.setattr(
        "daedalus_cli.auth.get_codex_auth_status",
        lambda: {"logged_in": True},
    )
    monkeypatch.setattr(
        "daedalus_cli.auth.resolve_codex_runtime_credentials",
...
def _make_cli(model="anthropic/claude-opus-4.6", **kwargs):
    """Create a DaedalusCLI with minimal mocking."""
    ...
    with (
        ...
        cli = DaedalusCLI(model=model, **kwargs)
    return cli
