"""Comprehensive tests for daedalus_cli.profiles module.
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.daedalus/profiles)
    * DAEDALUS_HOME  -> tmp_path/.daedalus  (so get_daedalus_home() agrees)
    * Creates the bare-minimum ~/.daedalus directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".daedalus"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("DAEDALUS_HOME", str(default_home))
    return tmp_path
...
class TestGetProfileDir:
    ...
    def test_default_returns_daedalus_home(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("default")
        assert result == tmp_path / ".daedalus"
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".daedalus" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".daedalus" / "profiles" / "coder"
        monkeypatch.setenv("DAEDALUS_HOME", str(profile_dir))
    ...
    def test_custom_path_returns_custom(self, profile_env, monkeypatch):
        ...
        monkeypatch.setenv("DAEDALUS_HOME", str(custom))
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("daedalus")
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        assert new_dir == tmp_path / ".daedalus" / "profiles" / "newname"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".daedalus_history",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.daedalus_history",
...
class TestCompletion:
    ...
    def test_bash_completion_has_daedalus_profiles_function(self):
        script = generate_bash_completion()
        assert "_daedalus_profiles" in script

    def test_zsh_completion_has_daedalus_function(self):
        script = generate_zsh_completion()
...
class TestInternalHelpers:
    ...
    def test_profiles_root_under_home(self, profile_env):
        ...
        assert root == tmp_path / ".daedalus" / "profiles"

    def test_default_daedalus_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_daedalus_home()
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".daedalus" / "profiles" / "mybot"
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from daedalus_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".daedalus"
