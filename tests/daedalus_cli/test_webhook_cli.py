"""Tests for daedalus_cli/webhook.py — webhook subscription CLI."""
...
from daedalus_cli.webhook import (
    webhook_command,
    _load_subscriptions,
    _save_subscriptions,
    _subscriptions_path,
...
class TestPersistence:
    ...
    def test_file_written_with_owner_only_permissions(self):
        """webhook_subscriptions.json holds per-route HMAC secrets -- a
        permissive umask must not leave it world/group-readable."""
        webhook_command(_make_args(webhook_action="subscribe", name="secretcheck"))
        path = _subscriptions_path()
        mode = os.stat(path).st_mode & 0o777
        assert mode == 0o600, oct(mode)

    def test_save_subscriptions_direct_call_sets_owner_only_permissions(self):
        _save_subscriptions({"foo": {"secret": "abc123"}})
        path = _subscriptions_path()
        mode = os.stat(path).st_mode & 0o777
        assert mode == 0o600, oct(mode)
...
class TestWebhookEnabledGate:
    def test_blocks_when_disabled(self, capsys, monkeypatch):
        monkeypatch.setattr("daedalus_cli.webhook._is_webhook_enabled", lambda: False)
        webhook_command(_make_args(webhook_action="subscribe", name="blocked"))
        out = capsys.readouterr().out
        assert "not enabled" in out.lower()
        assert "daedalus gateway setup" in out
        assert _load_subscriptions() == {}
    ...
    def test_real_check_disabled(self, monkeypatch):
        monkeypatch.setattr(
            "daedalus_cli.webhook._get_webhook_config",
            lambda: {},
        )
        monkeypatch.setattr(
            "daedalus_cli.webhook._is_webhook_enabled",
            lambda: bool({}.get("enabled")),
        )
        import daedalus_cli.webhook as wh_mod
