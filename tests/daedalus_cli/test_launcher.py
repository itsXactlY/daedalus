"""Tests for the top-level `./daedalus` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./daedalus` should use `daedalus_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "daedalus"
    called = []

    fake_main_module = types.ModuleType("daedalus_cli.main")

    def fake_main():
        called.append("daedalus_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "daedalus_cli.main", fake_main_module)
    ...
    assert called == ["daedalus_cli.main"]
