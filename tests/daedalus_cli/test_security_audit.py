"""Unit tests for daedalus_cli.security_audit — parsers + OSV plumbing.
...
class TestRunAudit:
    def test_no_components_returns_empty(self, tmp_path: Path):
        findings = sa.run_audit(
            skip_venv=True, skip_plugins=True, skip_mcp=True, daedalus_home=tmp_path
        )
        assert findings == []
...
class TestExitCodes:
    ...
    def test_discovery_runs_once_per_audit(self, tmp_path: Path, monkeypatch, capsys):
        ...
        """
        monkeypatch.setattr(sa, "get_daedalus_home", lambda: str(tmp_path))
        calls = {"venv": 0}
