...
class TestHandleSkillsSlashInstallFlags:
    ...
    def test_yes_flag_sets_skip_confirm(self):
        ...
        with patch("daedalus_cli.skills_hub.do_install") as mock_install:
...
class TestHandleSkillsSlashUninstallFlags:
    ...
    def test_yes_flag_sets_skip_confirm(self):
        from daedalus_cli.skills_hub import handle_skills_slash
        with patch("daedalus_cli.skills_hub.do_uninstall") as mock_uninstall:
...
class TestDoInstallSkipConfirm:
    ...
    @patch("daedalus_cli.skills_hub.input", return_value="n")
    def test_without_skip_confirm_prompts_user(self, mock_input):
        """Without skip_confirm, input() is called for confirmation."""
        from daedalus_cli.skills_hub import do_install
        with patch("daedalus_cli.skills_hub._console"), \
...
class TestDoUninstallSkipConfirm:
    ...
    def test_skip_confirm_bypasses_input(self):
        ...
        from daedalus_cli.skills_hub import do_uninstall
        with patch("daedalus_cli.skills_hub._console") as mock_console, \
