"""Tests for daedalus_cli.tools_config platform tool persistence."""

from unittest.mock import patch

from daedalus_cli.tools_config import (
    _configure_provider,
    _get_platform_tools,
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/NousResearch/daedalus/issues/1247
    """
    config = {
        "platform_toolsets": {
    ...
    with patch("daedalus_cli.tools_config.save_config"):
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (daedalus-cli, daedalus-telegram, etc.) must NOT
    ...
    ``daedalus tools``, but daedalus-cli stays in the config and re-enables
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "daedalus-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    ...
    monkeypatch.setattr(
        "daedalus_cli.nous_subscription.get_nous_auth_status",
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("daedalus_cli.tools_config._run_post_setup", lambda key: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    ...
    monkeypatch.setattr(
        "daedalus_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"web", "image_gen", "tts", "browser"},
    )
    monkeypatch.setattr("daedalus_cli.tools_config.save_config", lambda config: None)
    monkeypatch.setattr(
        "daedalus_cli.nous_subscription.get_nous_auth_status",
        lambda: {"logged_in": True},
    )
    ...
    configured = []
    monkeypatch.setattr(
        "daedalus_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from daedalus_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """daedalus-gateway includes list should cover all messaging platforms."""
        from daedalus_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from daedalus_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from daedalus_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
