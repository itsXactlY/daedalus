"""Tests for Codex auth — tokens stored in Daedalus auth store (~/.daedalus/auth.json)."""
...
def _setup_daedalus_auth(daedalus_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Daedalus auth store."""
    ...
    auth_file = daedalus_home / "auth.json"
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_daedalus_auth(daedalus_home)
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    daedalus_home = tmp_path / "daedalus"
    _setup_daedalus_auth(daedalus_home, access_token="")
    monkeypatch.setenv("DAEDALUS_HOME", str(daedalus_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_daedalus_auth(daedalus_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("DAEDALUS_HOME", str(daedalus_home))
    ...
    monkeypatch.setattr("daedalus_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    daedalus_home = tmp_path / "daedalus"
    _setup_daedalus_auth(daedalus_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("DAEDALUS_HOME", str(daedalus_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify Daedalus never writes to ~/.codex/auth.json."""
    daedalus_home = tmp_path / "daedalus"
    codex_home = tmp_path / "codex-cli"
    daedalus_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
...
def test_resolve_returns_daedalus_auth_store_source(tmp_path, monkeypatch):
    daedalus_home = tmp_path / "daedalus"
    _setup_daedalus_auth(daedalus_home)
    monkeypatch.setenv("DAEDALUS_HOME", str(daedalus_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "daedalus-auth-store"
