"""Tests for the update check mechanism in daedalus_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    cache_file.write_text(json.dumps({"ts": time.time(), "behind": 3, "ver": _version}))
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("DAEDALUS_HOME", str(tmp_path))
    with patch("daedalus_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when DAEDALUS_HOME has no git repo."""
    import daedalus_cli.banner as banner
