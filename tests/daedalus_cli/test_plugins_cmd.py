"""Tests for daedalus_cli.plugins_cmd — the ``daedalus plugins`` CLI subcommand."""
...
from daedalus_cli.plugins_cmd import (
    _copy_example_files,
    _read_manifest,
    _repo_name_from_url,
    _resolve_git_url,
...
class TestPluginsCommandDispatch:
    ...
    @patch("daedalus_cli.plugins_cmd.cmd_remove")
    ...
    @patch("daedalus_cli.plugins_cmd.cmd_update")
...
class TestReadManifest:
    ...
    def test_invalid_yaml_returns_empty_and_logs(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="daedalus_cli.plugins_cmd"):
            result = _read_manifest(tmp_path)
        assert result == {}
...
class TestCmdInstall:
    ...
    def test_install_requires_identifier(self):
        from daedalus_cli.plugins_cmd import cmd_install
        import argparse

        with pytest.raises(SystemExit):
            cmd_install("")

    @patch("daedalus_cli.plugins_cmd._resolve_git_url")
    ...
    @patch("daedalus_cli.plugins_cmd._display_after_install")
    @patch("daedalus_cli.plugins_cmd.shutil.move")
    @patch("daedalus_cli.plugins_cmd.shutil.rmtree")
    @patch("daedalus_cli.plugins_cmd._plugins_dir")
    @patch("daedalus_cli.plugins_cmd._read_manifest")
    @patch("daedalus_cli.plugins_cmd.subprocess.run")
...
class TestCmdUpdate:
    ...
    @patch("daedalus_cli.plugins_cmd._sanitize_plugin_name")
    @patch("daedalus_cli.plugins_cmd._plugins_dir")
    @patch("daedalus_cli.plugins_cmd.subprocess.run")
    def test_update_git_pull_success(self, mock_run, mock_plugins_dir, mock_sanitize):
        from daedalus_cli.plugins_cmd import cmd_update
...
class TestCmdRemove:
    ...
    def test_remove_deletes_plugin(self, mock_rmtree, mock_plugins_dir, mock_sanitize):
        from daedalus_cli.plugins_cmd import cmd_remove
...
class TestCmdList:
    ...
    def test_list_empty_plugins_dir(self, mock_plugins_dir):
        from daedalus_cli.plugins_cmd import cmd_list
...
class TestCopyExampleFiles:
    ...
    def test_copies_example_files(self, tmp_path):
        from daedalus_cli.plugins_cmd import _copy_example_files
...
class TestPromptPluginEnvVars:
    ...
    def test_skips_when_no_requires_env(self):
        from daedalus_cli.plugins_cmd import _prompt_plugin_env_vars
        from unittest.mock import MagicMock
    ...
    def test_prompts_for_missing_var_simple_format(self):
        ...
        with patch("daedalus_cli.config.get_env_value", return_value=None), \
    ...
    def test_secret_uses_getpass(self):
        ...
        with patch("daedalus_cli.config.get_env_value", return_value=None), \
             ...
             patch("daedalus_cli.config.save_env_value"):
            _prompt_plugin_env_vars(manifest, console)
