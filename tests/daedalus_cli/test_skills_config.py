"""Tests for daedalus_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestSaveDisabledSkills:
    ...
    def test_saves_global_sorted(self, mock_save):
        from daedalus_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    ...
    @patch("daedalus_cli.config.load_config")
    @patch.dict("os.environ", {"DAEDALUS_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("DAEDALUS_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """DAEDALUS_SESSION_PLATFORM should be used when DAEDALUS_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
        ...
        monkeypatch.setenv("DAEDALUS_HOME", str(tmp_path))
        monkeypatch.delenv("DAEDALUS_PLATFORM", raising=False)
        monkeypatch.setenv("DAEDALUS_SESSION_PLATFORM", "discord")

        from agent.skill_utils import get_disabled_skill_names
        result = get_disabled_skill_names()
        assert result == {"discord-skill", "global-skill"}

    def test_daedalus_platform_takes_precedence(self, tmp_path, monkeypatch):
        """DAEDALUS_PLATFORM should win over DAEDALUS_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("DAEDALUS_HOME", str(tmp_path))
        monkeypatch.setenv("DAEDALUS_PLATFORM", "telegram")
        monkeypatch.setenv("DAEDALUS_SESSION_PLATFORM", "discord")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from daedalus_cli.skills_config import _get_categories
        skills = [
