...
from daedalus_cli.config import set_config_value, config_command


@pytest.fixture(autouse=True)
def _isolated_daedalus_home(tmp_path):
    """Point DAEDALUS_HOME at a temp dir so tests never touch real config."""
...
class TestExplicitAllowlist:
    ...
    def test_explicit_key_routes_to_env(self, key, _isolated_daedalus_home):
...
class TestCatchAllPatterns:
    ...
    def test_api_key_suffix_routes_to_env(self, key, _isolated_daedalus_home):
        set_config_value(key, "secret-456")
        env_content = _read_env(_isolated_daedalus_home)
        assert f"{key}=secret-456" in env_content
        assert key not in _read_config(_isolated_daedalus_home)

    def test_case_insensitive(self, _isolated_daedalus_home):
    ...
    def test_terminal_ssh_prefix_routes_to_env(self, _isolated_daedalus_home):
...
class TestConfigYamlRouting:
    ...
    def test_simple_key(self, _isolated_daedalus_home):
        set_config_value("model", "gpt-4o")
        config = _read_config(_isolated_daedalus_home)
        assert "gpt-4o" in config
        assert "model" not in _read_env(_isolated_daedalus_home)

    def test_nested_key(self, _isolated_daedalus_home):
        set_config_value("terminal.backend", "modal")
        config = _read_config(_isolated_daedalus_home)
        assert "modal" in config
        assert "terminal" not in _read_env(_isolated_daedalus_home)

    def test_terminal_image_goes_to_config(self, _isolated_daedalus_home):
        """TERMINAL_MODAL_IMAGE doesn't match _API_KEY or _TOKEN, so config.yaml."""
        set_config_value("terminal.modal_image", "python:3.12")
        config = _read_config(_isolated_daedalus_home)
...
class TestFalsyValues:
    ...
    def test_empty_string_routes_to_env(self, _isolated_daedalus_home):
    ...
    def test_empty_string_routes_to_config(self, _isolated_daedalus_home):
    ...
    def test_zero_routes_to_config(self, _isolated_daedalus_home):
    ...
    def test_config_command_accepts_empty_string(self, _isolated_daedalus_home):
