...
The notice prints the most recent curator run summary on `daedalus update`,
...
session start) so users normally never see the rename map. `daedalus update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(daedalus_main)
