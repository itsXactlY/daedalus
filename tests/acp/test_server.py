"""Tests for acp_adapter.server — DaedalusACPAgent ACP server."""
...
from acp_adapter.server import DaedalusACPAgent, DAEDALUS_VERSION
...
def agent(mock_manager):
    """DaedalusACPAgent backed by a mock session manager."""
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "daedalus"
        assert resp.agent_info.version == DAEDALUS_VERSION
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert DAEDALUS_VERSION in result
    ...
    def test_model_switch_uses_requested_provider(self, tmp_path, monkeypatch):
        ...
        with patch("run_agent.AIAgent", side_effect=fake_agent):
            acp_agent = DaedalusACPAgent(session_manager=manager)
