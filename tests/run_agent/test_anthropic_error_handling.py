...
def _run_with_agent(monkeypatch, agent_cls):
    ...
    monkeypatch.setenv("DAEDALUS_TOOL_PROGRESS", "false")
...
class _TransientJSONDecodeError(json.JSONDecodeError):
    """Simulates a truncated/corrupted response body (transient, not a bug)."""
    def __init__(self):
        super().__init__("Expecting value", "garbled response body", 0)


class _TransientSSLError(ssl.SSLError):
    """Simulates a TLS transport hiccup (transient, not a bug)."""
    def __init__(self):
        super().__init__("SSL: UNEXPECTED_EOF_WHILE_READING")


class _NoneTypeNotIterableError(TypeError):
    """Simulates a provider/SDK shape mismatch, e.g. a null `output` field."""
    def __init__(self):
        super().__init__("'NoneType' object is not iterable")


def test_json_decode_error_is_retried_and_recovers(monkeypatch):
    """A truncated/corrupted response body should be retried, not aborted as
    a local validation error (JSONDecodeError is a ValueError subclass)."""
    agent_cls = _make_agent_cls(_TransientJSONDecodeError, recover_after=1)
    result = _run_with_agent(monkeypatch, agent_cls)
    assert result["final_response"] == "Recovered"


def test_ssl_error_is_retried_and_recovers(monkeypatch):
    """A TLS transport hiccup should be retried, not aborted.

    On this environment's Python (3.14), ssl.SSLError's MRO is
    SSLError -> OSError -> Exception (NOT a ValueError subclass, unlike the
    upstream commit's comment describes for the Python version it was
    written against) -- so isinstance(err, (ValueError, TypeError)) is
    already False here regardless of the explicit ssl.SSLError exclusion,
    and this test would pass with or without that exclusion on THIS
    interpreter. The exclusion is kept for portability (other Python
    versions/interpreters, or third-party SSL wrappers that do subclass
    ValueError) and to match upstream's intent -- this test documents the
    contract, it just isn't a regression guard on this specific runtime.
    """
    agent_cls = _make_agent_cls(_TransientSSLError, recover_after=1)
    result = _run_with_agent(monkeypatch, agent_cls)
    assert result["final_response"] == "Recovered"


def test_nonetype_not_iterable_error_is_retried_and_recovers(monkeypatch):
    """A provider/SDK 'NoneType is not iterable' shape mismatch (e.g. a null
    `output` field) should be retried, not aborted as a programming bug."""
    agent_cls = _make_agent_cls(_NoneTypeNotIterableError, recover_after=1)
    result = _run_with_agent(monkeypatch, agent_cls)
    assert result["final_response"] == "Recovered"
