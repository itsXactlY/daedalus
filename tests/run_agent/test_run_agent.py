...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._daedalus_home / "logs" / "errors.log").resolve()
...
class _FakeMazemakerManager:
    """Small fake that exposes only the MemoryManager methods AIAgent needs."""

    def __init__(self, raw_result=None, prefetch_result=""):
        self.calls = []
        self.prefetch_queries = []
        self.synced = []
        self.queued = []
        self.prefetch_result = prefetch_result
        self.raw_result = raw_result or json.dumps({
            "count": 1,
            "results": [
                {
                    "id": 42,
                    ...
                    "similarity": 0.91,
                    "content": "Use mazemaker memory before answering project-context questions.",
                    "connections": [1, 2, 3],
                }
            ],
        })
    ...
    def has_tool(self, tool_name):
        return tool_name == "mazemaker_recall"
...
class TestMazemakerBootstrap:
    def test_system_prompt_includes_mazemaker_guidance(self, agent):
    ...
    def test_compact_mazemaker_recall_result_removes_graph_bloat(self, agent):
        ...
        compact = json.loads(agent._compact_mazemaker_recall_result(raw, "query"))

        assert compact["auto_bootstrap"] is True
        assert compact["tool"] == "mazemaker_recall"
        assert compact["results"][0]["connection_count"] == 100
        assert "connections" not in compact["results"][0]
        assert compact["results"][0]["similarity"] == 0.8765
    ...
    def test_compact_mazemaker_recall_result_handles_pod_list_shape(self, agent):
        ...
        raw = json.dumps([
            {
                "id": 1164057,
                "label": "auto:turn:abc:1786",
                "similarity": 0.4809,
                "content": "deployment status — commits pushed, engine-sha aligned",
            },
            {
                "id": 1163887,
                "label": "auto:turn:abc:1786",
                "similarity": 0.5044,
                "content": "runtime map written and verified",
            },
        ])
        ...
        assert compact["results"][0]["id"] == 1164057
        assert compact["results"][0]["similarity"] == 0.4809
        assert "raw" not in compact
        assert compact["count"] == 2

    def test_build_mazemaker_bootstrap_messages_creates_tool_pair(self, agent):
        manager = _FakeMazemakerManager()
        agent._memory_manager = manager
        agent.valid_tool_names.add("mazemaker_recall")

        messages = agent._build_mazemaker_bootstrap_messages("what did we decide about mazemaker memory?")

        assert len(messages) == 2
        assert messages[0]["role"] == "assistant"
        assert messages[0]["tool_calls"][0]["function"]["name"] == "mazemaker_recall"
        assert messages[1]["role"] == "tool"
        assert messages[1]["tool_call_id"] == messages[0]["tool_calls"][0]["id"]
        result = json.loads(messages[1]["content"])
        assert result["auto_bootstrap"] is True
        assert result["results"][0]["id"] == 42
        assert manager.calls[0][0] == "mazemaker_recall"
        assert manager.calls[0][1]["limit"] == 3
    ...
    def test_run_conversation_injects_mazemaker_bootstrap_without_persisting(self, agent):
        ...
        with patch.object(agent, "_interruptible_api_call", side_effect=fake_api_call):
            result = agent.run_conversation("need past mazemaker memory context")
        ...
        assert manager.synced == [("need past mazemaker memory context", "done", agent.session_id)]
    ...
    def test_run_conversation_skips_legacy_memory_context_when_mazemaker_bootstrap_active(self, agent):
        ...
        with patch.object(agent, "_interruptible_api_call", side_effect=fake_api_call):
            result = agent.run_conversation("fresh question that needs current recall")

        assert result["final_response"] == "done"
        current_user_messages = [
            msg for msg in captured["messages"] if msg.get("role") == "user"
        ]
        assert len(current_user_messages) == 1
        current_user_content = current_user_messages[0]["content"]
        assert "<memory-context>" not in current_user_content
...
class TestRunConversation:
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch("daedalus_cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
...
class TestNousCredentialRefresh:
    ...
    def test_try_refresh_nous_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "daedalus_cli.auth.resolve_nous_runtime_credentials", _fake_resolve
        )
