...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("daedalus_cli.config.get_config_path", return_value=config_path), \
             patch("daedalus_cli.config.ensure_daedalus_home"):
    ...
    def test_save_env_value_sets_0600(self):
        ...
        with patch("daedalus_cli.config.get_env_path", return_value=env_path), \
             patch("daedalus_cli.config.ensure_daedalus_home"):
            from daedalus_cli.config import save_env_value
            save_env_value("TEST_KEY", "test_value")

            file_mode = stat.S_IMODE(os.stat(env_path).st_mode)
            self.assertEqual(file_mode, 0o600)

    def test_ensure_daedalus_home_sets_0700(self):
        home = Path(self.tmpdir) / ".daedalus"
        with patch("daedalus_cli.config.get_daedalus_home", return_value=home):
