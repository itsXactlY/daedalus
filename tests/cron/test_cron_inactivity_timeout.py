...
- Unlimited timeout (DAEDALUS_CRON_TIMEOUT=0)
- Backward compat: DAEDALUS_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """DAEDALUS_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("DAEDALUS_CRON_TIMEOUT", "1200")
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """DAEDALUS_CRON_TIMEOUT=0 yields None (unlimited)."""
        monkeypatch.setenv("DAEDALUS_CRON_TIMEOUT", "0")
        _cron_timeout = float(os.getenv("DAEDALUS_CRON_TIMEOUT", 600))
        _cron_inactivity_limit = _cron_timeout if _cron_timeout > 0 else None
...
class TestSysPathOrdering:
    ...
    def test_daedalus_constants_importable(self):
        """daedalus_constants should be importable from cron context."""
        from daedalus_constants import get_daedalus_home
        assert callable(get_daedalus_home)
