...
- Path resolution (absolute, relative to DAEDALUS_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp DAEDALUS_HOME."""
    daedalus_home = tmp_path / ".daedalus"
    daedalus_home.mkdir()
    (daedalus_home / "cron").mkdir()
    (daedalus_home / "cron" / "output").mkdir()
    ...
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", daedalus_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", daedalus_home / "cron" / "output")
