"""
Tests for mcp_serve — Daedalus MCP server.
...
def _isolate_daedalus_home(tmp_path, monkeypatch):
    """Redirect DAEDALUS_HOME to a temp directory."""
    monkeypatch.setenv("DAEDALUS_HOME", str(tmp_path))
    try:
        import daedalus_constants
        monkeypatch.setattr(daedalus_constants, "get_daedalus_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking daedalus_state schema."""
