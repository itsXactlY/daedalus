...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        for key in (
            "DAEDALUS_ENABLE_NOUS_MANAGED_TOOLS",
            "FIRECRAWL_API_KEY",
            "FIRECRAWL_API_URL",
        ...
        os.environ["DAEDALUS_ENABLE_NOUS_MANAGED_TOOLS"] = "1"
    ...
    def test_nous_auth_token_respects_daedalus_home_override(self, tmp_path):
        """Auth lookup should read from DAEDALUS_HOME/auth.json, not ~/.daedalus/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".daedalus").mkdir(parents=True)

        daedalus_home = tmp_path / "daedalus-home"
        daedalus_home.mkdir()
        (daedalus_home / "auth.json").write_text(json.dumps({
            "providers": {
                "nous": {
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "DAEDALUS_HOME": str(daedalus_home),
        }, clear=False):
            import tools.web_tools
...
class TestBackendSelection:
    ...
    ``daedalus tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def setup_method(self):
        ...
        for key in self._ENV_KEYS:
            if key != "DAEDALUS_ENABLE_NOUS_MANAGED_TOOLS":
                os.environ.pop(key, None)
