...
def _restore_tool_modules():
    original_daedalus_home = os.environ.get("DAEDALUS_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "daedalus_cli"
        or name.startswith("daedalus_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_daedalus_home is None:
            os.environ.pop("DAEDALUS_HOME", None)
        else:
            os.environ["DAEDALUS_HOME"] = original_daedalus_home
        _reset_modules(("tools", "daedalus_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    daedalus_cli = types.ModuleType("daedalus_cli")
    daedalus_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["daedalus_cli"] = daedalus_cli
    daedalus_home = tmp_path / "daedalus-home"
    os.environ["DAEDALUS_HOME"] = str(daedalus_home)
    sys.modules["daedalus_cli.config"] = types.SimpleNamespace(
        get_daedalus_home=lambda: daedalus_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="daedalus", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": daedalus_home / "modal_snapshots.json",
