...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        daedalus_home.mkdir()
        (daedalus_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(daedalus_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.daedalus/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        daedalus_home = tmp_path / ".daedalus"
        daedalus_home.mkdir()
        (daedalus_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        daedalus_home = tmp_path / ".daedalus"
        daedalus_home.mkdir()
        (daedalus_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        daedalus_home = tmp_path / ".daedalus"
        daedalus_home.mkdir()
        (daedalus_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.daedalus/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        daedalus_home = tmp_path / ".daedalus"
        (daedalus_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"DAEDALUS_HOME": str(daedalus_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.daedalus")

        assert mounts[0]["container_path"] == "/home/user/.daedalus/skills"
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape DAEDALUS_HOME."""
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside DAEDALUS_HOME must still be registered."""
        daedalus_home = tmp_path / ".daedalus"
        daedalus_home.mkdir()
        monkeypatch.setenv("DAEDALUS_HOME", str(daedalus_home))
        (daedalus_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_daedalus_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of DAEDALUS_HOME must be allowed."""
        daedalus_home = tmp_path / ".daedalus"
        daedalus_home.mkdir()
        subdir = daedalus_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside DAEDALUS_HOME pointing outside must be rejected."""
        daedalus_home = tmp_path / ".daedalus"
        daedalus_home.mkdir()
        monkeypatch.setenv("DAEDALUS_HOME", str(daedalus_home))
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, daedalus_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape DAEDALUS_HOME."""
        daedalus_home = tmp_path / ".daedalus"
        daedalus_home.mkdir()
        monkeypatch.setenv("DAEDALUS_HOME", str(daedalus_home))
        ...
        sensitive.write_text("{}")
        self._write_config(daedalus_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(daedalus_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside DAEDALUS_HOME via config must still mount."""
        daedalus_home = tmp_path / ".daedalus"
        daedalus_home.mkdir()
        monkeypatch.setenv("DAEDALUS_HOME", str(daedalus_home))

        (daedalus_home / "oauth.json").write_text("{}")
        self._write_config(daedalus_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        (daedalus_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("DAEDALUS_HOME", str(daedalus_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.daedalus/cache/documents" in paths
        assert "/root/.daedalus/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.daedalus/cache/documents"
    ...
    def test_empty_daedalus_home(self, tmp_path, monkeypatch):
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        daedalus_home = tmp_path / ".daedalus"
        doc_dir = daedalus_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        daedalus_home = tmp_path / ".daedalus"
        ss_dir = daedalus_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.daedalus/cache/screenshots/session_abc/screen1.png"
