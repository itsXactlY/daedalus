...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._daedalus_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_daedalus_bin(self):
        """After _INSTALL_FAILED, manual install in DAEDALUS_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        daedalus_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(daedalus_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(daedalus_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._daedalus_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == daedalus_bin
            assert _tirith_mod._resolved_path == daedalus_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# DAEDALUS_HOME isolation
# ---------------------------------------------------------------------------

class TestDaedalusHomeIsolation:
    def test_daedalus_bin_dir_respects_daedalus_home(self):
        """_daedalus_bin_dir must use DAEDALUS_HOME, not hardcoded ~/.daedalus."""
        from tools.tirith_security import _daedalus_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"DAEDALUS_HOME": tmpdir}):
            result = _daedalus_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_daedalus_home(self):
        """_failure_marker_path must use DAEDALUS_HOME, not hardcoded ~/.daedalus."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"DAEDALUS_HOME": "/custom/daedalus"}):
            result = _failure_marker_path()
        assert result == "/custom/daedalus/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets DAEDALUS_HOME; verify it's active."""
        daedalus_home = os.getenv("DAEDALUS_HOME")
        assert daedalus_home is not None, "DAEDALUS_HOME should be set by conftest"
        assert "daedalus_test" in daedalus_home, "Should point to test temp dir"

    def test_get_daedalus_home_fallback(self):
        """Without DAEDALUS_HOME set, falls back to ~/.daedalus."""
        from tools.tirith_security import _get_daedalus_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove DAEDALUS_HOME entirely
            os.environ.pop("DAEDALUS_HOME", None)
            result = _get_daedalus_home()
        assert result == os.path.join(os.path.expanduser("~"), ".daedalus")
