...
def _clean_state():
    ...
    key = os.getenv("DAEDALUS_SESSION_KEY", "default")
    clear_session(key)
    approval_module._permanent_approved.clear()
    saved = {}
    for k in ("DAEDALUS_INTERACTIVE", "DAEDALUS_GATEWAY_SESSION", "DAEDALUS_EXEC_ASK", "DAEDALUS_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithBlock:
    ...
    def test_tirith_block_gateway_returns_approval_required(self, mock_tirith):
        ...
        os.environ["DAEDALUS_GATEWAY_SESSION"] = "1"
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["DAEDALUS_INTERACTIVE"] = "1"
        session_key = os.getenv("DAEDALUS_SESSION_KEY", "default")
