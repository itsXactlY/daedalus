...
and the shared slash-command handlers (daedalus_cli/write_approval_commands.py).
...
@pytest.fixture
def daedalus_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="daedalus_wa_test_")
    home = os.path.join(d, ".daedalus")
    os.makedirs(home)
    monkeypatch.setenv("DAEDALUS_HOME", home)
    yield home
    shutil.rmtree(d, ignore_errors=True)


def _set_approval(subsystem, enabled):
    import daedalus_cli.config as cfg
    c = cfg.load_config()
...
def test_invalid_subsystem_is_off(daedalus_home):
    from tools import write_approval as wa
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(daedalus_home, capsys):
...
def test_load_on_disk_store_honors_configured_char_limits(daedalus_home, monkeypatch):
    ...
    monkeypatch.setattr("daedalus_cli.config.load_config", _boom)
...
def test_handle_approval_on(daedalus_home):
    from daedalus_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(daedalus_home):
    from daedalus_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_memory_inline_approve_writes(daedalus_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(daedalus_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
