...
def checkpoint_base(tmp_path):
    """Isolated checkpoint base — never writes to ~/.daedalus/."""
...
class TestShadowRepoInit:
    ...
    def test_has_workdir_file(self, work_dir, checkpoint_base, monkeypatch):
        ...
        shadow = _shadow_repo_path(str(work_dir))
        _init_shadow_repo(shadow, str(work_dir))
        workdir_file = shadow / "DAEDALUS_WORKDIR"
...
@pytest.fixture()
def store_scratch():
    """A scratch root OUTSIDE /tmp for store-management tests.

    ensure_checkpoint() deliberately refuses to checkpoint anything under
    /tmp (see the "directory too broad" guard) -- which is also why the
    pre-existing TestTakeCheckpoint/TestListCheckpoints/TestRestore classes
    above fail on this box already (their work_dir fixture uses tmp_path,
    a pre-existing, unrelated gap, not something this addition touches).
    Store-management tests need ensure_checkpoint to actually succeed, so
    they get real project/checkpoint dirs under here instead -- fully
    isolated (own random subdir, torn down after the test), never the real
    ~/.daedalus/checkpoints/.
    """
    import tempfile
    parent = Path.home() / ".daedalus" / "_test_scratch_ckpt"
    parent.mkdir(parents=True, exist_ok=True)
    root = Path(tempfile.mkdtemp(dir=str(parent)))
    yield root
    shutil.rmtree(root, ignore_errors=True)


def _make_project(base, name, workdir, n_commits=2, touch_offset=0):
    """Build a synthetic shadow-repo project dir under an isolated base."""
    import time
    ...
    with patch("tools.checkpoint_manager.CHECKPOINT_BASE", base):
        for i in range(n_commits):
            ...
            mgr.ensure_checkpoint(str(workdir), reason=f"take-{i}")
    if touch_offset:
        shadow = base / name
        past = time.time() - touch_offset
        os.utime(shadow, (past, past))
    return project_dir


class TestStoreStatus:
    def test_empty_base_reports_zero(self, store_scratch):
        from tools.checkpoint_manager import store_status
        info = store_status(checkpoint_base=store_scratch / "nope")
        assert info["project_count"] == 0
        assert info["total_size_bytes"] == 0
        assert info["projects"] == []

    def test_reports_live_and_orphan_projects(self, store_scratch):
        from tools.checkpoint_manager import _shadow_repo_path, store_status
        base = store_scratch / "checkpoints"
        live_wd = store_scratch / "live_project"
        live_wd.mkdir()
        with patch("tools.checkpoint_manager.CHECKPOINT_BASE", base):
            mgr = CheckpointManager(enabled=True, max_snapshots=50)
            (live_wd / "a.txt").write_text("hello\n")
        ...
        with patch("tools.checkpoint_manager.CHECKPOINT_BASE", base):
            ...
            (orphan_wd / "b.txt").write_text("bye\n")
            assert mgr2.ensure_checkpoint(str(orphan_wd), reason="init")
        shutil.rmtree(orphan_wd)

        info = store_status(checkpoint_base=base)
        assert info["project_count"] == 2
        by_exists = {p["exists"] for p in info["projects"]}
        assert by_exists == {True, False}
        live = next(p for p in info["projects"] if p["exists"])
...
class TestPruneCheckpoints:
    def test_deletes_orphans(self, store_scratch):
        from tools.checkpoint_manager import prune_checkpoints, store_status
        base = store_scratch / "checkpoints"
        wd = store_scratch / "proj"
        wd.mkdir()
        with patch("tools.checkpoint_manager.CHECKPOINT_BASE", base):
            mgr = CheckpointManager(enabled=True, max_snapshots=50)
            (wd / "a.txt").write_text("x\n")
            assert mgr.ensure_checkpoint(str(wd), reason="init")
        shutil.rmtree(wd)

        result = prune_checkpoints(delete_orphans=True, retention_days=999,
                                    max_total_size_mb=10_000, checkpoint_base=base)
        assert result["deleted_orphan"] == 1
        assert result["bytes_freed"] > 0
        assert store_status(checkpoint_base=base)["project_count"] == 0

    def test_keeps_orphans_when_disabled(self, store_scratch):
        from tools.checkpoint_manager import prune_checkpoints, store_status
        base = store_scratch / "checkpoints"
        wd = store_scratch / "proj"
        wd.mkdir()
        ...
        result = prune_checkpoints(delete_orphans=False, retention_days=999,
                                    max_total_size_mb=10_000, checkpoint_base=base)
        assert result["deleted_orphan"] == 0
        assert store_status(checkpoint_base=base)["project_count"] == 1

    def test_orphan_allowlist_restricts_deletion(self, store_scratch):
        from tools.checkpoint_manager import prune_checkpoints, store_status
        base = store_scratch / "checkpoints"
        wd1, wd2 = store_scratch / "p1", store_scratch / "p2"
        wd1.mkdir(); wd2.mkdir()
        with patch("tools.checkpoint_manager.CHECKPOINT_BASE", base):
            for wd in (wd1, wd2):
                m = CheckpointManager(enabled=True, max_snapshots=50)
                (wd / "a.txt").write_text("x\n")
                assert m.ensure_checkpoint(str(wd), reason="init")
        shutil.rmtree(wd1)
        shutil.rmtree(wd2)
        ...
        assert result["deleted_orphan"] == 0
        assert store_status(checkpoint_base=base)["project_count"] == 2

    def test_max_size_cap_drops_oldest_first(self, store_scratch):
        from tools.checkpoint_manager import prune_checkpoints, store_status
        base = store_scratch / "checkpoints"
        wds = []
        with patch("tools.checkpoint_manager.CHECKPOINT_BASE", base):
            for i in range(3):
                wd = store_scratch / f"p{i}"
                wd.mkdir()
                wds.append(wd)
                m = CheckpointManager(enabled=True, max_snapshots=50)
                (wd / "a.txt").write_text("x" * 1000 + "\n")
                assert m.ensure_checkpoint(str(wd), reason="init")

        before = store_status(checkpoint_base=base)
        assert before["project_count"] == 3
        result = prune_checkpoints(delete_orphans=False, retention_days=999,
                                    max_total_size_mb=0, checkpoint_base=base)
        after = store_status(checkpoint_base=base)
        assert after["total_size_bytes"] < before["total_size_bytes"]
        assert result["deleted_stale"] >= 1


class TestClearAll:
    def test_clears_entire_base(self, store_scratch):
    ...
    def test_nonexistent_base_is_noop(self, tmp_path):
        from tools.checkpoint_manager import clear_all
        result = clear_all(checkpoint_base=tmp_path / "nope")
        assert result["deleted"] is False
        assert result["bytes_freed"] == 0


class TestClearLegacy:
    def test_always_reports_nothing_on_this_forks_layout(self, tmp_path):
        from tools.checkpoint_manager import clear_legacy
        result = clear_legacy(checkpoint_base=tmp_path / "checkpoints")
