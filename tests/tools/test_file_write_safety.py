"""Tests for file write safety and DAEDALUS_WRITE_SAFE_ROOT sandboxing.
...
class TestSafeWriteRoot:
    """DAEDALUS_WRITE_SAFE_ROOT should sandbox writes to a specific subtree."""
    ...
    def test_writes_inside_safe_root_are_allowed(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setenv("DAEDALUS_WRITE_SAFE_ROOT", str(safe_root))
    ...
    def test_safe_root_env_ignores_empty_value(self, tmp_path: Path, monkeypatch):
        target = tmp_path / "regular.txt"
        monkeypatch.setenv("DAEDALUS_WRITE_SAFE_ROOT", "")
        assert _is_write_denied(str(target)) is False

    def test_safe_root_unset_allows_all(self, tmp_path: Path, monkeypatch):
        target = tmp_path / "regular.txt"
        monkeypatch.delenv("DAEDALUS_WRITE_SAFE_ROOT", raising=False)
        assert _is_write_denied(str(target)) is False
