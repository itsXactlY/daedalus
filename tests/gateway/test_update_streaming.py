...
Tests the new --gateway mode for daedalus update, including:
...
def _make_runner(daedalus_home=None):
...
class TestGatewayPrompt:
    ...
    def test_prompt_file_content(self, tmp_path):
        ...
        def capture_and_respond():
            ...
            prompt_path = daedalus_home / ".update_prompt.json"
            for _ in range(20):
                if prompt_path.exists():
                    prompt_data = json.loads(prompt_path.read_text())
                    (daedalus_home / ".update_response").write_text("n")
                    return
                time.sleep(0.1)
...
class TestUpdateCommandGatewayFlag:
    """Verify the gateway spawns daedalus update --gateway."""
...
class TestWatchUpdateProgress:
    ...
    async def test_detects_and_forwards_prompt(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222",
                   "session_key": "agent:main:telegram:dm:111"}
        (daedalus_home / ".update_pending.json").write_text(json.dumps(pending))
        (daedalus_home / ".update_output.txt").write_text("output\n")
    ...
    async def test_failure_exit_code(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222",
                   "session_key": "agent:main:telegram:dm:111"}
        (daedalus_home / ".update_pending.json").write_text(json.dumps(pending))
        (daedalus_home / ".update_output.txt").write_text("error occurred\n")
