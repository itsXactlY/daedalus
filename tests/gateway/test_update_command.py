...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        runner = _make_runner()
        event = _make_event()
        monkeypatch.setenv("DAEDALUS_MANAGED", "homebrew")

        result = await runner._handle_update_command(event)

        assert "managed by Homebrew" in result
        assert "brew upgrade daedalus" in result
    ...
    @pytest.mark.asyncio
    async def test_no_daedalus_binary(self, tmp_path):
        """Returns error when daedalus is not on PATH and daedalus_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "daedalus update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m daedalus_cli.main when daedalus not on PATH."""
        runner = _make_runner()
        event = _make_event()
    ...
    @pytest.mark.asyncio
    async def test_resolve_daedalus_bin_prefers_which(self, tmp_path):
        """_resolve_daedalus_bin returns argv parts from shutil.which when available."""
        from gateway.run import _resolve_daedalus_bin

        with patch("shutil.which", return_value="/custom/path/daedalus"):
            result = _resolve_daedalus_bin()

        assert result == ["/custom/path/daedalus"]

    @pytest.mark.asyncio
    async def test_resolve_daedalus_bin_fallback(self):
        """_resolve_daedalus_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_daedalus_bin
        ...
        assert result == [sys.executable, "-m", "daedalus_cli.main"]

    @pytest.mark.asyncio
    async def test_resolve_daedalus_bin_returns_none_when_both_fail(self):
        """_resolve_daedalus_bin returns None when both strategies fail."""
        from gateway.run import _resolve_daedalus_bin
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._daedalus_home", daedalus_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/daedalus" if x == "daedalus" else "/usr/bin/setsid"), \
             patch("subprocess.Popen"):
            result = await runner._handle_update_command(event)

        pending_path = daedalus_home / ".update_pending.json"
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (daedalus_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = daedalus_home / ".update_pending.claimed.json"
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (daedalus_home / ".update_pending.json").write_text(json.dumps(pending))
        (daedalus_home / ".update_output.txt").write_text("x" * 5000)
        (daedalus_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (daedalus_home / ".update_pending.json").write_text(json.dumps(pending))
        (daedalus_home / ".update_output.txt").write_text("Traceback: boom")
        (daedalus_home / ".update_exit_code").write_text("1")
    ...
    async def test_cleans_up_files_after_notification(self, tmp_path):
        ...
        pending_path = daedalus_home / ".update_pending.json"
        output_path = daedalus_home / ".update_output.txt"
        exit_code_path = daedalus_home / ".update_exit_code"
        pending_path.write_text(json.dumps({
