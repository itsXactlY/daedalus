...
class TestVoiceChannelCommands:
    ...
    async def test_join_success(self, runner):
        ...
        event.source.chat_name = "Daedalus Server / #general"
        runner.adapters[event.source.platform] = mock_adapter
    ...
    async def test_input_reuses_bound_source_metadata(self, runner):
        ...
        bound_source = SessionSource(
            chat_id="123",
            chat_name="Daedalus Server / #general",
            chat_type="group",
            user_id="user1",
            user_name="user1",
        ...
        assert event.source.chat_name == "Daedalus Server / #general"
...
class TestDiscordVoiceChannelMethods:
    ...
    def test_discord_voice_timeout_config_loaded(self):
        ...
        with patch("daedalus_cli.config.read_raw_config", return_value={
            "discord": {
...
class TestVoiceChannelAwareness:
    ...
    def _make_adapter(self):
        ...
        adapter._voice_receivers = {}
        adapter._client = MagicMock()
        adapter._client.user = SimpleNamespace(id=99999, name="DaedalusBot")
        return adapter
    ...
    def test_returns_info_with_members(self):
        ...
        bot_member = self._make_member(99999, "DaedalusBot", is_bot=True)
        ...
        assert "Bob" in names
        assert "DaedalusBot" not in names
...
class TestVoiceReception:
    ...
    def test_unknown_ssrc_late_speaking_event(self):
        ...
        receiver.start()
        self._fill_buffer(receiver, 100, age_s=0.0)
