...
class TestReasoningCommand:
    ...
    @pytest.mark.asyncio
    async def test_reasoning_auto_answers_clearly_not_unknown_arg(self, tmp_path, monkeypatch):
        """/reasoning auto is a CLI-side feature; gateway explains, not unknown-arg."""
        daedalus_home = tmp_path / "daedalus"
        daedalus_home.mkdir()
        (daedalus_home / "config.yaml").write_text("agent:\n  reasoning_effort: medium\n", encoding="utf-8")
        monkeypatch.setattr(gateway_run, "_daedalus_home", daedalus_home)
        ...
        assert "Unknown argument" not in result
        assert "task-intensity gauge" in result
    ...
    def test_run_agent_reloads_reasoning_config_per_message(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_run, "_daedalus_home", daedalus_home)
        monkeypatch.setattr(gateway_run, "_env_path", daedalus_home / ".env")
    ...
    def test_run_agent_homeassistant_uses_default_platform_toolset(self, tmp_path, monkeypatch):
        daedalus_home = tmp_path / "daedalus"
        daedalus_home.mkdir()
        (daedalus_home / "config.yaml").write_text("", encoding="utf-8")
