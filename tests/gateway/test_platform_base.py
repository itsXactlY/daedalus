...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    daedalus_home.mkdir()
    monkeypatch.setattr(base, "_DAEDALUS_HOME", daedalus_home)
    monkeypatch.setattr(base, "_DAEDALUS_ROOT", daedalus_home)
    path = daedalus_home / "cache" / "bws_cache.enc.json"
    path.parent.mkdir()
...
class TestSecretCaptureGuidance:
    def test_gateway_secret_capture_message_points_to_local_setup(self):
        ...
        assert "~/.daedalus/.env" in message
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.daedalus/media/'
...
class TestMediaDeliveryPathValidation:
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("DAEDALUS_KANBAN_HOME", str(tmp_path / "daedalus"))
        monkeypatch.setenv("DAEDALUS_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "daedalus" / "kanban" / "boards" / "research"
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.daedalus/mcp-tokens/ must
        ...
        (daedalus_dir / "mcp-tokens").mkdir(parents=True)
        secret = daedalus_dir / rel
        secret.write_text('{"access_token": "live-bearer-abc123"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._DAEDALUS_HOME",
            daedalus_dir,
        )
        monkeypatch.setattr(
            "gateway.platforms.base._DAEDALUS_ROOT",
            daedalus_dir,
        )
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the DAEDALUS_HOME root (google_token.json)
        ...
        token = daedalus_dir / "google_token.json"
    ...
    def test_denylist_blocks_non_cache_file_under_daedalus_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.daedalus
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting DAEDALUS_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose DAEDALUS_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.daedalus) emits
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._DAEDALUS_ROOT", daedalus_root
        )
...
class TestDockerContainerMediaPathTranslation:
    ...
    def test_cache_dir_container_path_translates_to_host_cache(self, tmp_path, monkeypatch):
        """MEDIA:/root/.daedalus/cache/images/... (the agent_visible_image path
        under docker) must translate to the HOST cache file, not the sandbox
        home copy."""
        daedalus_home = tmp_path / ".daedalus"
        cache = daedalus_home / "cache" / "images"
        ...
        assert BasePlatformAdapter.validate_media_delivery_path(
            "/root/.daedalus/cache/images/generated.png"
        ) == str(media.resolve())

    def test_container_credential_path_never_translates_through_home(self, tmp_path, monkeypatch):
        """/root/.daedalus/* outside a cache mount (the sandbox's credential
...
class TestGetHumanDelay:
    ...
    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "DAEDALUS_HUMAN_DELAY_MODE": "natural",
            "DAEDALUS_HUMAN_DELAY_MIN_MS": "oops",
    ...
    def test_custom_mode_tolerates_malformed_env_vars(self):
        env = {
            "DAEDALUS_HUMAN_DELAY_MODE": "custom",
            "DAEDALUS_HUMAN_DELAY_MIN_MS": "oops",
            "DAEDALUS_HUMAN_DELAY_MAX_MS": "still-bad",
        }
        with patch.dict(os.environ, env):
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"DAEDALUS_MEDIA_DELIVERY_STRICT": "1",
                                     "DAEDALUS_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("DAEDALUS_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Daedalus
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../daedalus/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.daedalus/cache/media/sensitive_host_path_abc123.bin"
