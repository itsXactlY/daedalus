...
# 2. Creates a virtual environment with Python 3.14 via uv
# 3. Installs all dependencies (main package + submodules)
# 4. Creates .env from template (if not exists)
# 5. Symlinks the 'daedalus' CLI command into ~/.local/bin
...
echo ""
echo -e "${CYAN}⚕ Daedalus Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink daedalus into ~/.local/bin
# ============================================================================

echo -e "${CYAN}→${NC} Setting up daedalus command..."

DAEDALUS_BIN="$SCRIPT_DIR/venv/bin/daedalus"
mkdir -p "$HOME/.local/bin"
ln -sf "$DAEDALUS_BIN" "$HOME/.local/bin/daedalus"
echo -e "${GREEN}✓${NC} Symlinked daedalus → ~/.local/bin/daedalus"
...
# ============================================================================
# Seed bundled skills into ~/.daedalus/skills/
# ============================================================================

DAEDALUS_SKILLS_DIR="${DAEDALUS_HOME:-$HOME/.daedalus}/skills"
mkdir -p "$DAEDALUS_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.daedalus/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$DAEDALUS_SKILLS_DIR/" 2>/dev/null || true
...
echo ""
echo "Other commands:"
echo "  daedalus status        # Check configuration"
echo "  daedalus gateway install # Install gateway service (messaging + cron)"
echo "  daedalus cron list     # View scheduled jobs"
echo "  daedalus doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m daedalus_cli.main setup
fi
