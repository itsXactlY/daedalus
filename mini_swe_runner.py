#!/usr/bin/env python3
"""
SWE Runner with Daedalus Trajectory Format

A runner that uses Daedalus-Agent's built-in execution environments
(local, modal) and outputs trajectories in the Daedalus-Agent format
compatible with batch_runner.py and trajectory_compressor.py.

Features:
- Uses Daedalus-Agent's Modal or Local environments for command execution
- Outputs trajectories in Daedalus format (from/value pairs with <tool_call>/<tool_response> XML)
...
Usage:
    ...
    python mini_swe_runner.py --task "List files in /tmp" --env modal
...
**Environment:**
- Isolated execution environment (local or Modal cloud)
...
def create_environment(
    ...
    **kwargs
    ...
    """
    Create an execution environment using Daedalus-Agent's built-in backends.
...
class MiniSWERunner:
    """
    Agent runner that uses Daedalus-Agent's built-in execution environments
    and outputs trajectories in Daedalus-Agent format.
    """
    ...
    def _convert_to_daedalus_format(
        self,
        ...
        """
        Convert internal message format to Daedalus trajectory format.
...
def main(
    ...
    """
    Run SWE tasks with Daedalus trajectory format output.
