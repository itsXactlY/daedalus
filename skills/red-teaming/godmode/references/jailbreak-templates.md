...
## 5. GODMODE FAST — Daedalus 4 405B (Zero Refusal)

**Model:** `nousresearch/daedalus-4-405b`
**Strategy:** Daedalus models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Daedalus Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.daedalus/config.yaml`:
