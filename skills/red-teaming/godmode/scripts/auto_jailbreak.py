...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("DAEDALUS_HOME", os.path.expanduser("~/.daedalus")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("DAEDALUS_HOME", Path.home() / ".daedalus")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Daedalus config paths
# ═══════════════════════════════════════════════════════════════════

DAEDALUS_HOME = Path(os.getenv("DAEDALUS_HOME", Path.home() / ".daedalus"))
CONFIG_PATH = DAEDALUS_HOME / "config.yaml"
PREFILL_PATH = DAEDALUS_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Daedalus models — already uncensored, just needs clean prompt
    "daedalus": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "daedalus" in model_lower or "nous" in model_lower:
        return "daedalus"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Daedalus config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.daedalus/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Daedalus for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Daedalus for changes to take effect.")
