#!/usr/bin/env python3
"""Thin Ollama-compatible proxy -> llama.cpp OpenAI-compat backend.

The pulse-pod (and any service wired for Ollama) only speaks Ollama protocol
(/api/tags, /api/generate). llama.cpp speaks OpenAI-compat (/v1/chat/completions).
This proxy translates so the pod can use a local llama.cpp model without a rebuild.

Endpoints:
  GET  /api/tags        -> fixed model list (the llama.cpp model name)
  POST /api/generate    -> translate Ollama {model,prompt,stream,options}
                            to OpenAI chat completions, return {response}

Env:
  LLAMA_BASE_URL  (default http://127.0.0.1:8080)
  LLAMA_KEY       (default sk-llama)
  PROXY_PORT      (default 11435)

Run: python3 ollama-compat-proxy.py   (listens on :11435)
"""
import json
