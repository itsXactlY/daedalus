...
# Python ≥ 3.14 workaround for --background:
cd ~/comfy/ComfyUI && .venv/bin/python main.py --listen 127.0.0.1 --port 8188
# Run via terminal(background=true, ...). Background output may be empty
# (goes to stderr or is buffered) — watch_patterns won't trigger.
# Verify with curl instead of relying on output.
