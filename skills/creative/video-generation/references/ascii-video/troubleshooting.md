...
### `np.maximum()` and `np.clip()` Require Float Bounds

NumPy 2.x changed the signature of `np.maximum()`, `np.clip()`, and related functions:
integer scalar arguments (like `0`, `1`) where the array dtype is `float32`/`float64`
now raise a `TypeError` in strict mode or produce deprecation warnings.

**Symptom:**
```
TypeError: maximum() takes from 2 to 3 positional arguments but 1 was given
```

This happens because `np.maximum(scalar_int)` with one arg is interpreted as a reduction.

**Fix**: Always use float literals:
...
**Affected code patterns in the ASCII video stack:**
- `oklch_to_linear_rgb()`: `np.clip(r, 0.0, 1.0)` — all three channels
- `oklch_arrays_to_rgb()`: same clip calls + `np.power(np.maximum(x, 0.0), 3)` for cube
- `linear_to_srgb()`: `np.power(np.maximum(c, 0.0), 1/2.4)`
- `tonemap()`: already uses float division, but verify

**Quick fix script:**
```bash
sed -i 's/np\.clip(\([a-z_]\), 0, 1)/np.clip(\1, 0.0, 1.0)/g' reel.py
sed -i 's/np\.maximum(\([a-z_]\), 0)/np.maximum(\1, 0.0)/g' reel.py
```

All templates and reference implementations in this skill have been updated to use float
literals. If you copy code from external sources (docs, Stack Overflow, older projects),
apply the float-literal fix as part of integration.

---

## Tooling Workflow

### `patch` Tool Duplicate Matches

The `patch` tool's `replace` mode requires `old_string` to match **exactly one** location in
the file. If the same code pattern appears in multiple places (e.g., `np.clip(r, 0, 1)` in
both `oklch_to_linear_rgb` and `oklch_arrays_to_rgb`), the tool refuses:

```
Error: Found 2 matches for old_string. Provide more context to make it unique.
```

**Fixes:**
1. Add more surrounding context lines to `old_string` until it's unique
2. Use `replace_all=True` if every occurrence should be changed (risky — review the diff)
3. Fall back to `terminal` + `sed -i` for bulk find-and-replace:
   ```bash
   sed -i 's/np\.clip(r, 0, 1)/np.clip(r, 0.0, 1.0)/g' reel.py
   ```

### `execute_code` Sandbox Limitations

The `execute_code` tool runs in a **separate sandbox** that does **not** have access to
numpy, scipy, PIL, or other project dependencies. Attempting `import numpy` will fail
with `ModuleNotFoundError`.

**Fix**: Always use `terminal` for Python code that requires project dependencies:
```python
# Instead of execute_code:
terminal(command="cd /home/alca && python3 -c '...'")

# Or for longer scripts:
terminal(command="cd /home/alca && python3 mazemaker_ascii.py")
```
...
A complete working ASCII video script (961 lines) with 5 scenes, OKLCH colors,
feedback buffer, particles, and text overlay is available at:
`/home/alca/mazemaker_ascii.py`

It demonstrates: plasma, rings, spiral, tunnel, voronoi, strange attractor effects;
OKLCH color system; FeedbackBuffer with Zoom+HashShift; particle emitters;
and text overlay with convergence. Copy and customize.
