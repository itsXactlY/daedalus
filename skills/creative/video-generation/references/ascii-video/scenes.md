...
| File | Contents |
|------|----------|
| `templates/mazemaker_ascii.py` | Complete working generative ASCII video (961 lines). 5-scene narrative: Birth → Memory Field → Dream Engine → Graph Burst → Convergence. OKLCH colors, feedback buffer, particles, text overlay. NumPy 2.x compatible. Use as a starting point for new generative videos. |
