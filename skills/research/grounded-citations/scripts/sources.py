...
Ledger path resolution (first wins):
  --ledger PATH
  $DAEDALUS_CITATION_LEDGER
  $DAEDALUS_HOME/cache/citations/ledger.json
"""
...
from _daedalus_home import get_daedalus_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("DAEDALUS_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_daedalus_home() / "cache" / "citations" / "ledger.json"
