...
the 16:30 UTC cycle learnings, and again with the 17:30 UTC cycle
extremesaturation data point. Captures the meta-pattern of cron-job
system-prompt-vs-skill disagreement, the saturated-window pattern at
40+ prior decisions per 2h, and the connectedness-counting workflow.
...
## Connectedness-counting script (NEW — 2026-06-22 16:30 cycle)
...
## Pair-into-narrative rule at 50+ saturation (NEW — 2026-06-22 21:48 cycle)
...
## Single-rank NICE_TO_KNOW output at high saturation (NEW — 2026-06-23 00:15 cycle)
...
**Operational rule — new case 4 added to the saturation heuristic:**
...
3. If pool has 0 candidates with `connectedness > 0.3` AND `novelty > 0.4` →
   write a single NICE_TO_KNOW "saturation reached, no novel candidates in 2h
   window" note.
4. **NEW (2026-06-23 00:15):** If pool has 1 candidate with `connectedness > 0.5`
...
**What to expect at this saturation level (14-20 decisions/2h with 1 viable pick):**
the 1-rank output is typically a NICE_TO_KNOW corpus anchor for a sparse cluster
that previously had no primary source. The "Cluster Context: RANK #1 of 1" framing
+ the explicit list of rejected candidates in the decision body makes it clear
to the ACT phase that this is a single-corpus-anchor cycle, not a partial
3-rank output. The cycle report to the operator should explicitly state the
saturation level ("14+ prior decisions in 2h window, 1 viable pick remaining")
so the operator can verify the saturation heuristic fired correctly.
...
## Novel-axis-with-cluster-bridging at 50+ saturation (NEW — 2026-06-22 23:35 cycle)
