...
### ❌ Don't cite stats from the auto-injected prompt context:
The system-prompt `<memory-context>` block (and any operator-written `MEMORY.md` header that mentions "~190k+ facts, 1M+ knowledge graph edges") is **stale by definition** — it was written when the brain was a different size, and the writer (you, a previous session) may have conflated dream-phase bridges (ephemeral, 71M+ lifetime) with persistent graph connections (tens of thousands). If your answer cites any number about the mazemaker corpus — memory count, edge count, dream session count, AFE fact count, DAE coverage — that number **MUST come from a live `mazemaker_health` (or `mazemaker_stats` + `mazemaker_graph` + `mazemaker_dream_stats`) call this turn**, not from the prompt context.

**The trap:** the prompt block is presented with authority ("It holds ~190k+ facts... 1M+ knowledge-graph edges") and is easy to lift into a comparison table verbatim. Don't. The user will burn you for performing "worse than a free-tier model" if you ship a comparison table with a 20× hallucination because the prompt block said "1M+" but reality is 54,891.

**Verification rule:** before any comparison/status/snapshot answer that contains a number, fire `mazemaker_health()` in parallel with your first recall. Cite by handle from the response, never from the prompt.

### ❌ Don't hardcode corpus numbers in the narrative template:
The "🧠 **Mazemaker Live** — 200K memories, 57K connections, 1024-d BGE-M3" line in the demo template below is a **placeholder**, not a fact. The actual live values shift between sessions. Always substitute from the `mazemaker_health()` response. If the narrative is shipped to Discord with stale numbers, the audience will see a number that doesn't match `mazemaker_stats` — worse than no number.
...
```
🧠 **Mazemaker Live** — {health.corpus.memories} memories, {health.corpus.connections} connections, {health.embedding.dim}-d {health.embedding.backend}

**Health snapshot:** (substitute from mazemaker_health() response, do NOT hardcode)
...
**All `{...}` placeholders are mandatory substitutions from the live `mazemaker_health()` call in step 1. Never ship hardcoded corpus numbers in a delivered message.**
