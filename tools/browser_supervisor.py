...
One ``CDPSupervisor`` runs per Daedalus ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Daedalus logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "daedalus-dialog-bridge.invalid"
...
(() => {
  if (window.__daedalusDialogBridgeInstalled) return;
  window.__daedalusDialogBridgeInstalled = true;
  const ENDPOINT = "http://daedalus-dialog-bridge.invalid/";
