#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Daedalus Skills Hub.
...
Used by daedalus_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            daedalus_meta = metadata.get("daedalus", {})
            if isinstance(daedalus_meta, dict):
                tags = daedalus_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.daedalus/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                daedalus_meta = meta_block.get("daedalus", {})
                if isinstance(daedalus_meta, dict):
                    tags = daedalus_meta.get("tags", [])
