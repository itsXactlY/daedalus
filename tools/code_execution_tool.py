...
Lets the LLM write a Python script that calls Daedalus tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `daedalus_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `daedalus_tools.py` with file-based RPC stubs
...
def generate_daedalus_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the daedalus_tools.py stub module.
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["DAEDALUS_RPC_SOCKET"])
...
_RPC_DIR = os.environ.get("DAEDALUS_RPC_DIR", "/tmp/daedalus_rpc")
_seq = 0
''' + _COMMON_HELPERS + '''\
...
def _execute_remote(
    ...
    The script and the generated daedalus_tools.py module are shipped to
    ...
    sandbox_dir = f"/tmp/daedalus_exec_{sandbox_id}"
    ...
    try:
        ...
        tz = os.getenv("DAEDALUS_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
_TOOL_DOC_LINES = [
    ...
    ("web_extract",
     "  web_extract(urls: list[str]) -> dict  # {\"results\": [...]}"),
    ("read_file",
     "  read_file(path: str, offset: int = 1, limit: int = 500) -> dict  # {\"content\", \"total_lines\"}"),
    ("write_file",
     "  write_file(path: str, content: str) -> dict  # overwrites file"),
    ("search_files",
     "  search_files(pattern, target=\"content\", path=\".\", file_glob=None, limit=50) -> dict  # {\"matches\"}"),
    ("patch",
     "  patch(path, old_string, new_string, replace_all=False) -> dict  # applies edit"),
    ("terminal",
...
def build_execute_code_schema(enabled_sandbox_tools: set = None) -> dict:
    ...
    When tools are disabled via ``daedalus tools`` (e.g. web is turned off),
    ...
    description = (
        "Run a Python script that calls Daedalus tools programmatically. "
        "Use when you need 3+ tool calls with processing between them, "
        "want to reduce large outputs before they hit your context, "
        "or need branching or loops.\n\n"
        "Use normal calls for: single calls, or tasks needing full-result "
        "reasoning or interactive input.\n\n"
        f"Available via `from daedalus_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        "Limits: 5-minute timeout, 50KB stdout cap, max 50 tool calls per script. "
        "terminal() is foreground-only (no background/pty). Cloud sandboxes are resumable, not durable.\n\n"
        "Print your final result to stdout. Helpers: json_parse(text) for control "
        "chars in terminal output; shell_quote(s) for interpolating into shell; "
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from daedalus_tools import {import_str}` "
