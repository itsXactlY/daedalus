...
Design contract (matches the Daedalus "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    daedalus_meta = meta.get("daedalus") if isinstance(meta, dict) else None
    if not isinstance(daedalus_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.daedalus.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in daedalus_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.daedalus.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("daedalus", "agent", "daedalus agent") and (
        ...
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Daedalus Agent' (proper caps) "
