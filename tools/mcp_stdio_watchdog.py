...
<url>``) is spawned as a direct child of the Daedalus process. Daedalus's own
...
Daedalus process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Daedalus startup's opt-in
