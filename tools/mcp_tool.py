...
transport, discovers their tools, and registers them into the daedalus
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.daedalus/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.daedalus/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Daedalus' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Daedalus explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Daedalus and
    its subprocesses can consume those credentials without duplicating them
    in every MCP server's ``env:`` block.
    """
    try:
        from daedalus_cli.env_loader import get_secret_source
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            daedalus_home = os.path.expanduser(
                os.getenv(
                    "DAEDALUS_HOME", os.path.join(os.path.expanduser("~"), ".daedalus")
                )
            )
            candidates = [
                os.path.join(daedalus_home, "node", "bin", resolved_command),
                ...
                # image (which the Daedalus Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Daedalus MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Daedalus gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Daedalus document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`daedalus_cli.mcp_config._unwrap_exception_group` with
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Daedalus' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (DAEDALUS_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit DAEDALUS_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Daedalus' logging (agent.log via daedalus_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "it is not installed. Run `daedalus setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this Daedalus process (kill -9, crash, force-quit)
                ...
                if (
                    ...
                    post_resp = await client.post(
                        ...
                        content=(
                            ...
                            '"capabilities":{},'
                            '"clientInfo":{"name":"daedalus-probe",'
                            '"version":"0.1"}}}'
                        ),
                    )
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Daedalus' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Daedalus is fully
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `daedalus mcp login {server_name}` (or delete the tokens "
        f"file under ~/.daedalus/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Daedalus processes (e.g.
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(
                get_daedalus_home() / ".mcp-discovery.lock"
            )
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local DAEDALUS_HOME override into ``coro``.
    ...
    """
    try:
        from daedalus_constants import (
            get_daedalus_home_override,
            reset_daedalus_home_override,
            set_daedalus_home_override,
        )

        home_override = get_daedalus_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_daedalus_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local DAEDALUS_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_daedalus_home()
...
def _filter_suspicious_mcp_servers(servers: Dict[str, dict]) -> Dict[str, dict]:
    ...
    try:
        from daedalus_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
    except Exception:
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Daedalus config file.
    ...
    ``os.environ`` (which includes ``~/.daedalus/.env`` loaded at startup).
    """
    try:
        from daedalus_cli.config import load_config
        from utils import env_var_enabled as _env_enabled

        if _env_enabled("DAEDALUS_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Daedalus' MEDIA: tag convention and out to messaging
            ...
            # Daedalus' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Daedalus's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Daedalus uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Daedalus registry schema format.
...
def get_mcp_status() -> List[dict]:
    ...
    # The mazemaker pod is the operator's MCP surface — its tools are exposed
    # to the agent as mcp__mazemaker__* via the memory provider. Surface it in
    # the MCP view (like v0.20.0 shows its registered servers) even when no
    # mcp_servers are configured. One cheap /tools fetch; pod-down → skipped.
    if not any(srv.get("name") == "mazemaker" for srv in result):
        try:
            import urllib.request as _ur
            _url = os.environ.get("MM_WONDERLAND_URL", "http://127.0.0.1:8765")
            with _ur.urlopen(f"{_url}/tools", timeout=1.5) as _resp:
                _data = json.loads(_resp.read().decode("utf-8"))
            _tools = _data if isinstance(_data, list) else (
                (_data.get("tools") or _data.get("result")) if isinstance(_data, dict) else []
            )
            if _tools:
                result.append({
                    "name": "mazemaker",
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``daedalus tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Daedalus registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple daedalus
