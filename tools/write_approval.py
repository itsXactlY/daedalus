...
(see ``daedalus_cli`` slash handlers): memory shows full content, skills show
...
Pending records live under ``<DAEDALUS_HOME>/pending/{memory,skills}/<id>.json``
...
def write_approval_enabled(subsystem: str) -> bool:
    ...
    if subsystem not in _SUBSYSTEMS:
        return False
    try:
        from daedalus_cli.config import load_config, cfg_get
        cfg = load_config()
