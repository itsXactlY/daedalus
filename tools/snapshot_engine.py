"""
Runtime Snapshot Engine — Self-versioning state snapshots for Daedalus.

Content-addressed storage (like git-lite) for the internal state of ~/.daedalus.
...
class SnapshotEngine:
    """Content-addressed snapshot engine for Daedalus runtime state."""

    def __init__(self, daedalus_home: Optional[Path] = None):
        self.daedalus_home = daedalus_home or get_daedalus_home()
        self.store = self.daedalus_home / ".snapshots"
    ...
    def wal_replay(self) -> List[str]:
        ...
        for rel_path, sha in latest.items():
            ...
            if not obj_path.exists():
                continue
            target = self.daedalus_home / rel_path
