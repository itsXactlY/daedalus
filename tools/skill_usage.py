...
Tracks per-skill usage metadata in a sidecar JSON file (~/.daedalus/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.daedalus/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.daedalus/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def read_suppressed_names() -> Set[str]:
    ...
    One skill name per line in ``~/.daedalus/skills/.curator_suppressed``. This is
    what makes pruning a built-in durable: without it, ``daedalus update`` would
...
def list_archived_skill_names() -> List[str]:
    """Enumerate skills in ``~/.daedalus/skills/.archive/``.

    Archive layout is flat (``.archive/<skill>/``) as set by ``archive_skill``,
    so the directory name is the skill name. Used by ``daedalus curator
    list-archived`` to help users pass a name to ``daedalus curator restore``.
    """
...
def _is_curator_managed_record(record: Any) -> bool:
    ...
    * management = "may autonomous curation mutate/archive this" — a policy
      decision the user can change at any time via ``daedalus curator adopt``.
...
def list_unmanaged_skill_names() -> List[str]:
    ...
    * It was created by a FOREGROUND ``skill_manage(action="create")`` call,
    ...
    every automatic transition. ``daedalus curator status`` surfaces this count
    so the blind spot is legible instead of silent, and ``daedalus curator
...
def _emit_skill_lifecycle(
    ...
    *,
    ...
    try:
        from daedalus_cli.lifecycle import has_hook, invoke_hook
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.daedalus/skills/. Restores to the flat
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.daedalus/skills/<skill>/SKILL.md) and category-nested
    (~/.daedalus/skills/<category>/<skill>/SKILL.md) layouts. Uses the gated
