"""
Lazy dependency installer for opt-in Daedalus Agent backends.

Many Daedalus features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``daedalus[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``daedalus tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``DAEDALUS_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/daedalus`` read-only), setting
  ``DAEDALUS_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Daedalus, which is what made it safe to seal
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``DAEDALUS_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``daedalus update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Daedalus under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``daedalus update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="daedalus-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``daedalus_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        from tools.environments.local import daedalus_subprocess_env
    ...
    if _lazy_install_target() is None:
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Daedalus cannot install "
...
def feature_install_command(feature: str, *, venv_pip: bool = False) -> Optional[str]:
    ...
    (default install, ``DAEDALUS_HOME`` overrides, profile installs) and
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``DAEDALUS_DISABLE_LAZY_INSTALLS=1``) and sets
      ``DAEDALUS_LAZY_INSTALL_TARGET``, installs are redirected to the writable
...
def active_features() -> list[str]:
    ...
    Used by ``daedalus update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``daedalus update``. Never raises; lazy-install failures
