...
`daedalus skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.daedalus.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.daedalus.blueprint`` is a mapping containing
    ...
    daedalus = meta.get("daedalus") if isinstance(meta, dict) else None
    blueprint = daedalus.get("blueprint") if isinstance(daedalus, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.daedalus.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.daedalus.blueprint`` block) they can hand
    to ``daedalus skills publish`` to share. ``body`` is the plain-language
