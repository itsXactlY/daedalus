...
etc.) surface as specific blocked checks via `daedalus computer-use doctor`
...
def _action_result_from(
    ...
    *,
    ...
    cua-driver's mcp-tool-notes and NousResearch/daedalus#67052.
    """
...
# fetches the latest release, so a `DAEDALUS_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `DAEDALUS_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD_ENV = "DAEDALUS_CUA_DRIVER_CMD"
...
def _cua_no_overlay() -> bool:
    """True when Daedalus should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when Daedalus should disable cua-driver telemetry for this user.
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when Daedalus runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the Daedalus process uses POSIX subprocess spawning inside WSL.
    ...
    if not re.match(r"^[A-Za-z]:[\\/]", path):
        return path
    try:
        from daedalus_constants import is_wsl
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one Daedalus session's YOLO choice
    ...
    def __init__(self, driver_cmd: str, permission_mode: str) -> None:
        ...
        if sys.platform == "win32":
            self.socket_path = rf"\\.\pipe\daedalus-cua-{token}"
        else:
    ...
    def start(self) -> None:
        ...
        self._stderr_thread = threading.Thread(
            ...
            args=(self._process,),
            name="daedalus-cua-daemon-stderr",
            daemon=True,
        )
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of NousResearch/daedalus#47072: instead of hardcoding
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted Daedalus an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``DAEDALUS_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    Daedalus Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `daedalus computer-use doctor` succeeds from a login shell.
    """
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``DAEDALUS_CUA_DRIVER_CMD``) is never silently
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `daedalus computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        "  daedalus computer-use install\n"
        "Or run the upstream installer directly:\n"
        f"{installer}\n"
        "Or run `daedalus tools` and enable the Computer Use toolset to install it automatically."
    )
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of NousResearch/daedalus#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(
        ...
        self._started = False
        # Surface 4 of NousResearch/daedalus#47072: per-tool
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Daedalus-managed secrets out of the child env.
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `daedalus computer-use doctor` and check "
                f"{display_daedalus_home()}/logs/agent.log for the phase timings."
            )
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    NousResearch/daedalus#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        # Surface 6 of NousResearch/daedalus#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per Daedalus run, with
        ...
        #     concurrent Daedalus runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one Daedalus run = one backend = one session — and pass it as
        ...
        self._session_id: str = f"daedalus-{uuid.uuid4().hex[:12]}"
    ...
    def start(self) -> None:
        ...
        # `computer-use` / `mcp` extras), not part of Daedalus' minimal core.
    ...
    def capture(
        ...
        Maps daedalus `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of NousResearch/daedalus#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of NousResearch/daedalus#47072: prefer the
    ...
    def _apply_delivery(
        ...
        to proceed. See NousResearch/daedalus#67052 phase B.
        """
    ...
    def click(
        ...
        *,
        ...
        # NousResearch/daedalus#47072). cua-driver-rs gained an explicit
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the Daedalus adapter, and uses cua-driver's
