...
This module provides the glue:
    - ``DaedalusTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Daedalus Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses DAEDALUS_HOME so each profile gets its own OAuth tokens.
    Layout: ``DAEDALUS_HOME/mcp-tokens/``
    """
    try:
        from daedalus_constants import get_daedalus_home
        base = Path(get_daedalus_home())
    except ImportError:
        base = Path(os.environ.get("DAEDALUS_HOME", str(Path.home() / ".daedalus")))
...
class DaedalusTokenStorage:
    ...
    File layout::
        ...
        DAEDALUS_HOME/mcp-tokens/<server_name>.json         -- tokens
        DAEDALUS_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Daedalus.</p></body></html>"
            ) if code else (
