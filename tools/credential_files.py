...
def _resolve_daedalus_home() -> Path:
...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.daedalus",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``DAEDALUS_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside DAEDALUS_HOME so that a malicious
    ...
    try:
        ...
        logger.warning(
            ...
            "(resolves to %s, outside DAEDALUS_HOME %s)",
            relative_path,
            resolved,
            daedalus_home_resolved,
        )
        return False
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if os.path.isabs(rel):
                        ...
                        continue
                    host_path = (daedalus_home / rel).resolve()
                    try:
                        host_path.relative_to(daedalus_home_resolved)
                    except ValueError:
                        logger.warning(
                            "credential_files: rejected config path traversal %r "
                            "(resolves to %s, outside DAEDALUS_HOME %s)",
                            rel, host_path, daedalus_home_resolved,
                        )
                        continue
                    if host_path.is_file():
                        container_path = f"/root/.daedalus/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="daedalus-skills-safe-"))
...
def get_cache_directory_mounts(
    ...
    ``get_daedalus_dir()`` for backward compatibility with old directory layouts.
    """
