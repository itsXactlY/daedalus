...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from DAEDALUS_HOME/.env."""
    env_path = get_daedalus_home() / ".env"
...
def _is_gateway_surface() -> bool:
    if os.getenv("DAEDALUS_GATEWAY_SESSION"):
        return True
    return bool(os.getenv("DAEDALUS_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return "Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to ~/.daedalus/.env manually."
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("DAEDALUS_PLATFORM")
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.daedalus/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``daedalus skills`` config UI). Default False
...
def skill_view(name: str, file_path: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        try:
            ...
            content = preprocess_skill_content(content, skill_dir, task_id)
        except Exception:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.daedalus/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `daedalus skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        if isinstance(metadata, dict):
            daedalus_meta = metadata.get("daedalus", {}) or {}

        tags = _parse_tags(daedalus_meta.get("tags") or frontmatter.get("tags", ""))
...
SKILL_VIEW_SCHEMA = {
    "name": "skill_view",
    "description": "Load a skill's full content or its linked files (references, templates, scripts). First call returns SKILL.md plus a 'linked_files' dict; call again with file_path to access a specific linked file.",
    "parameters": {
        "type": "object",
        "properties": {
            "name": {
