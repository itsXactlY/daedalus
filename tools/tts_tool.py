...
- Edge TTS (default, free, no API key): Microsoft Edge mazemaker voices
...
Configuration is loaded from ~/.daedalus/config.yaml under the 'tts:' key.
...
def _get_default_output_dir() -> str:
    ...
    return str(get_daedalus_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.daedalus/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.daedalus/config.yaml.
    ...
    try:
        ...
        logger.debug("daedalus_cli.config not available, using default TTS config")
        return {}
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.daedalus/config.yaml (tts: section).
    ...
    platform = os.getenv("DAEDALUS_SESSION_PLATFORM", "").lower()
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run daedalus setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": "Optional custom file path to save the audio. Defaults to ~/.daedalus/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
