...
SESSION_SEARCH_SCHEMA = {
    "name": "session_search",
    "description": (
        "Search past Daedalus sessions (FTS5 over raw transcripts) for verbatim content: "
        "exact phrasing, what was literally said, session titles/previews. Empty query "
        "returns a chronological list of recent sessions.\n\n"
        "Syntax: OR for broad match (a OR b OR c), quotes for phrases (\"docker networking\"), "
        "NOT for exclusion (python NOT java), * for prefix (deploy*). Defaults to AND, so use OR for breadth."
    ),
    "parameters": {
        "type": "object",
        "properties": {
