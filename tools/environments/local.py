...
def _build_provider_env_blocklist() -> frozenset:
    ...
    optional config registry, so new Daedalus-managed secrets are blocked
    ...
    try:
        from daedalus_cli.config import OPTIONAL_ENV_VARS
...
_DAEDALUS_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Daedalus-managed secrets from a subprocess environment.

    `_DAEDALUS_FORCE_<VAR>` entries in ``extra_env`` opt a blocked variable back in
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_DAEDALUS_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _DAEDALUS_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_DAEDALUS_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_DAEDALUS_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _DAEDALUS_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
...
def _find_bash() -> str:
    ...
    raise RuntimeError(
        "Git Bash not found. Daedalus Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set DAEDALUS_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_DAEDALUS_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_DAEDALUS_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _DAEDALUS_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
...
class LocalEnvironment(PersistentShellMixin, BaseEnvironment):
    ...
    def _temp_prefix(self) -> str:
        return f"/tmp/daedalus-local-{self._session_id}"
