"""
Cron job management tools for Daedalus Agent.
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    origin_platform = os.getenv("DAEDALUS_SESSION_PLATFORM")
    origin_chat_id = os.getenv("DAEDALUS_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        return {
            "platform": origin_platform,
            "chat_id": origin_chat_id,
            "chat_name": os.getenv("DAEDALUS_SESSION_CHAT_NAME"),
            "thread_id": os.getenv("DAEDALUS_SESSION_THREAD_ID"),
        }
    return None
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via daedalus model.
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within DAEDALUS_HOME/scripts/.
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": "Optional path to a Python script that runs before each cron job execution. Its stdout is injected into the prompt as context. Use for data collection and change detection. Relative paths resolve under ~/.daedalus/scripts/. On update, pass empty string to clear."
            },
        },
        "required": ["action"]
    }
...
def check_cronjob_requirements() -> bool:
    ...
    """
    return bool(
        os.getenv("DAEDALUS_INTERACTIVE")
        or os.getenv("DAEDALUS_GATEWAY_SESSION")
        or os.getenv("DAEDALUS_EXEC_ASK")
    )
