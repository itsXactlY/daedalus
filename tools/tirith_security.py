...
it is automatically downloaded from GitHub releases to $DAEDALUS_HOME/bin/tirith.
...
def _get_daedalus_home() -> str:
    """Return the Daedalus home directory, respecting DAEDALUS_HOME env var."""
    return str(get_daedalus_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_daedalus_home(), ".tirith-install-failed")
...
def _daedalus_bin_dir() -> str:
    """Return $DAEDALUS_HOME/bin, creating it if needed."""
    d = os.path.join(_get_daedalus_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $DAEDALUS_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_daedalus_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $DAEDALUS_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $DAEDALUS_HOME/bin/tirith
    ...
    daedalus_bin = os.path.join(_daedalus_bin_dir(), "tirith")
    if os.path.isfile(daedalus_bin) and os.access(daedalus_bin, os.X_OK):
        _resolved_path = daedalus_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return daedalus_bin
