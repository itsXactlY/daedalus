...
`mcp_tool.py`, and `daedalus_cli/mcp_config.py`. This module is the ONLY place
...
def _make_daedalus_provider_class() -> Optional[type]:
    ...
    class DaedalusMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`DaedalusTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, DaedalusTokenStorage)
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._daedalus_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._daedalus_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import DaedalusTokenStorage
            if not isinstance(storage, DaedalusTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``daedalus mcp reauth``.
            ...
            back to ``daedalus mcp reauth``.
            """
            try:
                if self._daedalus_preregistered:
                    return
        ...
        async def async_auth_flow(self, request):  # type: ignore[override]
            ...
            try:
                await get_manager().invalidate_if_disk_changed(
                    self._daedalus_server_name,
                    daedalus_home=self._daedalus_home,
                )
            except Exception as exc:  # pragma: no cover — defensive
                logger.debug(
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._daedalus_home = key[0]
    ...
    def _key(
        server_name: str,
        daedalus_home: str | Path | None = None,
    ) -> tuple[str, str]:
        from daedalus_constants import get_daedalus_home

        home = Path(daedalus_home) if daedalus_home is not None else get_daedalus_home()
    ...
    def _build_provider(
        ...
        Constructs :class:`DaedalusMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _DAEDALUS_PROVIDER_CLS is None:
            logger.warning(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `daedalus mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``daedalus mcp remove <name>`` and (indirectly) by
        ``daedalus mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, daedalus_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, daedalus_home=daedalus_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, daedalus_home), entry)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, daedalus_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(daedalus_home) / f"{_safe_filename(server_name)}.json"
            try:
