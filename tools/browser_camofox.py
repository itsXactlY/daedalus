...
Setup::
    ...
    # Option 2: Podman
    podman run -p 9377:9377 -e CAMOFOX_PORT=9377 jo-inc/camofox-browser

Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.daedalus/.env``.
"""
...
def _managed_persistence_enabled() -> bool:
    """Return whether Daedalus-managed persistence is enabled for Camofox.
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the Daedalus profile so the Camofox server can map it
    ...
    with _sessions_lock:
        ...
        if _managed_persistence_enabled():
            ...
            session = {
                "user_id": f"daedalus_{uuid.uuid4().hex[:10]}",
                "tab_id": None,
    ...
    try:
        ...
        return json.dumps({
            ...
            "error": f"Cannot connect to Camofox at {get_camofox_url()}. "
                     ...
                     "or: podman run -p 9377:9377 -e CAMOFOX_PORT=9377 jo-inc/camofox-browser",
        })
