...
def _get_file_ops(task_id: str = "default") -> ShellFileOperations:
    ...
    environment yet, creates one using the configured backend (local,
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Daedalus internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files.
        from daedalus_constants import get_daedalus_home as _get_hh
        _daedalus_home = _get_hh().resolve()
        _blocked_dirs = [
            _daedalus_home / "skills" / ".hub" / "index-cache",
            _daedalus_home / "skills" / ".hub",
        ]
        for _blocked in _blocked_dirs:
            try:
                ...
                return json.dumps({
                    "error": (
                        f"Access denied: {path} is an internal Daedalus cache file "
...
READ_FILE_SCHEMA = {
    "name": "read_file",
    "description": "Read a text file with line numbers and pagination (output 'LINE_NUM|CONTENT'). Use instead of cat/head/tail in terminal. Reads over ~100K chars are rejected — use offset/limit for large files. Cannot read images/binary — use vision_analyze.",
    "parameters": {
        "type": "object",
        "properties": {
...
PATCH_SCHEMA = {
    "name": "patch",
    "description": "Targeted find-and-replace edits in files. Fuzzy matching tolerates minor whitespace/indentation differences. Returns a unified diff; auto-runs syntax checks after editing.\n\nReplace mode (default): find a unique string and replace it.\nPatch mode: apply V4A multi-file patches for bulk changes.",
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "old_string": {"type": "string", "description": "Text to find (required for 'replace' mode). Must be unique unless replace_all=true. Include surrounding context for uniqueness."},
            "new_string": {"type": "string", "description": "Replacement text (required for 'replace' mode). Can be empty to delete the match."},
...
SEARCH_FILES_SCHEMA = {
    "name": "search_files",
    "description": "Search file contents (regex) or find files by name. Ripgrep-backed, faster than shell equivalents.\n\nContent search (target='content'): regex inside files; output_mode = full matches with line numbers, files_only, or count.\nFile search (target='files'): glob pattern (e.g. '*.py'); results sorted by modification time.",
    "parameters": {
        "type": "object",
        "properties": {
            "pattern": {"type": "string", "description": "Regex (content) or glob e.g. '*.py' (files)"},
            "target": {"type": "string", "enum": ["content", "files"], "description": "'content' = inside files, 'files' = find by name", "default": "content"},
            "path": {"type": "string", "description": "Where to search (default: cwd)", "default": "."},
            "file_glob": {"type": "string", "description": "Filter files in content mode (e.g. '*.py')"},
            "limit": {"type": "integer", "description": "Maximum number of results to return (default: 50)", "default": 50},
            "offset": {"type": "integer", "description": "Skip first N results for pagination (default: 0)", "default": 0},
            "output_mode": {"type": "string", "enum": ["content", "files_only", "count"], "description": "Content-mode output: 'content' = matching lines, 'files_only' = paths, 'count' = per-file counts", "default": "content"},
            "context": {"type": "integer", "description": "Context lines around each match (content mode only)", "default": 0}
        },
        "required": ["pattern"]
    }
}
