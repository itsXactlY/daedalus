    ...
    try:
        for i, t in enumerate(task_list):
            child = _build_child_agent(
                ...
                memory_tools=creds.get("memory_tools", False),
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    configured_memory_tools = bool(cfg.get("memory_tools", False))
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'daedalus auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (daedalus_cli/config.py load_config()) so that
...
DELEGATE_TASK_SCHEMA = {
    ...
    "description": (
        ...
        "only the final summary is returned.\n\n"
        "MODES (one of 'goal' or 'tasks' required):\n"
        "1. Single: provide 'goal' (+ optional context, toolsets)\n"
        "2. Batch: 'tasks' array, up to 3, run in parallel, results returned together.\n\n"
        "Use for: reasoning-heavy subtasks (debugging, code review), tasks with lots of "
        "intermediate data, parallel work.\n"
        "Not for: mechanical multi-step work (use execute_code), single calls, or user "
        "interaction (subagents cannot use clarify).\n\n"
        "Subagents have NO memory of your conversation — pass all needed info via 'context'. "
        "They cannot call delegate_task, clarify, memory, send_message, execute_code. "
    ...
    "parameters": {
        ...
        "properties": {
            "goal": {
                "type": "string",
                "description": (
                    "What the subagent should accomplish. Be specific and self-contained."
                ),
            },
            "context": {
                "type": "string",
                "description": (
                    "Background the subagent needs: file paths, errors, constraints. "
            ...
            "toolsets": {
                ...
                "items": {"type": "string"},
                "description": (
                    "Toolsets for this subagent (default: inherit yours). "
            ...
            "tasks": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "toolsets": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "Toolsets for this task.",
                        },
                        "acp_command": {
                            "type": "string",
                ...
                "maxItems": 3,
                "description": (
                    "Batch mode: up to 3 tasks run in parallel. When set, "
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Spawns Claude Code or other ACP-capable agents from any parent."
                ),
            },
            "acp_args": {
                "type": "array",
                "items": {"type": "string"},
                "description": (
                    "Args for acp_command (default: ['--acp', '--stdio']). "
