"""
DaedalusAgentBaseEnv -- Abstract Base Environment for Daedalus-Agent + Atropos

Provides the Atropos integration plumbing that all daedalus environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via DaedalusAgentLoop
...
class DaedalusAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for daedalus Atropos environments.
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/daedalus-results/ "
...
class DaedalusAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for daedalus Atropos integration.
    ...
    name: Optional[str] = "daedalus"
    env_config_cls = DaedalusAgentEnvConfig

    def __init__(
        self,
        config: DaedalusAgentEnvConfig,
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY daedalus tool (terminal, file, web,
