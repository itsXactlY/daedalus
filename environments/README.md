...
This directory contains the integration layer between **daedalus's** tool-calling capabilities and the **Atropos** RL training framework. It provides everything needed to run agentic LLMs through multi-turn tool-calling loops, score their output with arbitrary reward functions, and feed results into Atropos for training or evaluation.
...
```
                    ...
                    └───────────┬───────────┘
                                │ inherits
                    ┌───────────┴───────────┐
                    │  DaedalusAgentBaseEnv    │  daedalus_base_env.py
                    │  - Terminal backend    │
                    │  - Tool resolution     │
                    │  - Agent loop          │
                    │  - ToolContext          │
              ...
              ┌─────────────────┼─────────────────┐
              │                 │                  │
     TerminalTestEnv     DaedalusSweEnv    TerminalBench2EvalEnv
...
**DaedalusAgentBaseEnv** (`daedalus_base_env.py`) extends BaseEnv with daedalus specifics:
- Sets `os.environ["TERMINAL_ENV"]` to configure the terminal backend (local, docker, modal, daytona, ssh, singularity)
- Resolves daedalus toolsets via `_resolve_tools_for_group()` (calls `get_tool_definitions()` which queries `tools/registry.py`)
- Implements `collect_trajectory()` which runs the full agent loop and computes rewards
- Supports two-phase operation (Phase 1: OpenAI server, Phase 2: VLLM ManagedServer)
- Applies monkey patches for async-safe tool operation at import time

Concrete environments inherit from `DaedalusAgentBaseEnv` and implement:
...
`DaedalusAgentLoop` is the reusable multi-turn agent engine. It runs the same pattern as daedalus's `run_agent.py`:
...
`ToolContext` is a per-rollout handle that gives reward/verification functions direct access to **all** daedalus tools, scoped to the rollout's `task_id`. The same `task_id` means the terminal/browser session is the SAME one the model used during its rollout -- all state (files, processes, browser tabs) is preserved.
...
- **Generic**: `call_tool(name, args)` -- call any daedalus tool by name
- **Cleanup**: `cleanup()` -- release all resources (called automatically after `compute_reward`)

### Patches (`patches.py`)

**Problem**: Some daedalus tools use `asyncio.run()` internally (e.g., the Modal backend). This crashes when called from inside Atropos's event loop because `asyncio.run()` cannot be nested.
...
Available parsers:
- `daedalus` -- Daedalus/ChatML `<tool_call>` XML format
...
├── daedalus_base_env.py            # Abstract base (DaedalusAgentBaseEnv)
├── agent_loop.py                 # Multi-turn agent engine (DaedalusAgentLoop)
...
│
├── daedalus_swe_env/               # SWE-bench style training environment
│   └── daedalus_swe_env.py
│
...
### DaedalusSweEnv (`daedalus_swe_env/`)

SWE-bench style training environment. The model gets a coding task, uses terminal + file + web tools to solve it, and the reward function runs tests in the same Modal sandbox.

```bash
python environments/daedalus_swe_env/daedalus_swe_env.py serve \
...
2. Create your env file inheriting from `DaedalusAgentBaseEnv`
...
class MyEnvConfig(DaedalusAgentEnvConfig):
    pass  # Add custom fields as needed

class MyEnv(DaedalusAgentBaseEnv):
    name = "my-env"
...
2. Inherit from `DaedalusAgentBaseEnv`
...
|-------|-------------|---------|
| `enabled_toolsets` | Which daedalus toolsets to enable | `None` (all) |
...
| `tool_call_parser` | Parser name for Phase 2 | `daedalus` |
