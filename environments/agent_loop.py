"""
DaedalusAgentLoop -- Reusable Multi-Turn Agent Engine

Runs the daedalus tool-calling loop using standard OpenAI-spec tool calling.
...
identical to daedalus's run_agent.py. Tool execution is dispatched via
...
def resize_tool_pool(max_workers: int):
    ...
    Called by DaedalusAgentBaseEnv.__init__ based on config.tool_pool_size.
...
class DaedalusAgentLoop:
    """
    Runs daedalus's tool-calling loop using standard OpenAI-spec tool calling.
