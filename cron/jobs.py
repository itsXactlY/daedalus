...
Jobs are stored in ~/.daedalus/cron/jobs.json
Output is saved to ~/.daedalus/cron/output/{job_id}/{timestamp}.md
"""
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Daedalus configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Daedalus timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _daedalus_now().tzinfo
    if dt.tzinfo is None:
...
def _jobs_lock():
    ...
    In-process threading lock only. Upstream also takes a cross-process
    advisory file lock (via a `_current_cron_store()` abstraction this fork
    doesn't have); porting that is a separate, larger piece of the cron
    rework. This still prevents the same failure mode within one process
    (e.g. the gateway's parallel tick threads racing a curator pass).
    """
...
def save_jobs(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _daedalus_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job_id,
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _daedalus_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job_id,
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _daedalus_now().isoformat(),
        },
    )
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _daedalus_now().strftime("%Y-%m-%d_%H-%M-%S")
...
def _canonical_skill_ref(raw: Any) -> str:
    ...
    A job may store an absolute path under ``DAEDALUS_HOME/skills`` or an
