...
Uses a file-based lock (~/.daedalus/cron/.tick.lock) so only one tick
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("DAEDALUS_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid DAEDALUS_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within DAEDALUS_HOME/scripts/.  Both relative and
    ...
    Args:
        script_path: Path to a Python script.  Relative paths are resolved
            against DAEDALUS_HOME/scripts/.  Absolute and ~-prefixed paths
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_daedalus_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    try:
        ...
        try:
            ...
            load_dotenv(str(_daedalus_home / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            os.environ["DAEDALUS_CRON_AUTO_DELIVER_PLATFORM"] = delivery_target["platform"]
            os.environ["DAEDALUS_CRON_AUTO_DELIVER_CHAT_ID"] = str(delivery_target["chat_id"])
            if delivery_target.get("thread_id") is not None:
                os.environ["DAEDALUS_CRON_AUTO_DELIVER_THREAD_ID"] = str(delivery_target["thread_id"])
        ...
        try:
            import yaml
            _cfg_path = str(_daedalus_home / "config.yaml")
        ...
        try:
            runtime_kwargs = {
                "requested": job.get("provider") or os.getenv("DAEDALUS_INFERENCE_PROVIDER"),
            }
            if job.get("base_url"):
...
**Job ID:** {job_id}
**Run Time:** {_daedalus_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        for key in (
            ...
            "DAEDALUS_SESSION_CHAT_NAME",
            "DAEDALUS_CRON_AUTO_DELIVER_PLATFORM",
            "DAEDALUS_CRON_AUTO_DELIVER_CHAT_ID",
            "DAEDALUS_CRON_AUTO_DELIVER_THREAD_ID",
        ):
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _daedalus_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _daedalus_now().strftime('%H:%M:%S'), len(due_jobs))
