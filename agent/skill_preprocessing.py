...
Ported from upstream hermes 0.20 (agent/skill_preprocessing.py) onto
the 0.8.0 tree. The only behavioural adaptation is the config loader:
0.8.0 has ``daedalus_cli.config.load_config`` (no ``load_config_readonly``),
so ``load_skills_config`` calls that instead.
"""
...
from daedalus_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
def substitute_template_vars(
    ...
    """Replace ${DAEDALUS_SKILL_DIR} / ${DAEDALUS_SESSION_ID} in skill content.
    ...
    def _replace(match: re.Match) -> str:
        token = match.group(1)
        if token == "DAEDALUS_SKILL_DIR" and skill_dir_str:
            return skill_dir_str
        if token == "DAEDALUS_SESSION_ID" and session_id:
