...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.daedalus/models_dev_cache.json)
...
def _get_reverse_mapping() -> Dict[str, str]:
    """Return models.dev ID → Daedalus provider ID mapping."""
...
def _get_cache_path() -> Path:
    ...
    return get_daedalus_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Daedalus provider ID to its models dict from models.dev.
...
def search_models_dev(
    ...
    Args:
        ...
        provider: Optional Daedalus provider ID to restrict search scope.
    ...
    if provider is not None:
        ...
        for daedalus_prov, mdev_prov in PROVIDER_TO_MODELS_DEV.items():
            ...
            if isinstance(provider_data, dict):
                ...
                if isinstance(models, dict):
                    for mid, mdata in models.items():
                        candidates.append((daedalus_prov, mid, mdata))
...
def list_all_providers() -> Dict[str, ProviderInfo]:
    ...
    a Daedalus alias, both the models.dev ID and the Daedalus ID are included.
    """
    data = fetch_models_dev()
...
def get_model_info(
    ...
    Accepts Daedalus or models.dev provider ID.  Tries exact match then
...
def get_model_info_any_provider(model_id: str) -> Optional[ModelInfo]:
    ...
    a bare name and want to find it anywhere.  Checks Daedalus-mapped providers
