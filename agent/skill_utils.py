...
from daedalus_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    DAEDALUS_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        try:
            from daedalus_constants import is_container
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``daedalus_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``DAEDALUS_PLATFORM`` or
            ``DAEDALUS_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("DAEDALUS_PLATFORM")
        or get_session_env("DAEDALUS_SESSION_PLATFORM")
    )
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.daedalus/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``daedalus`` cold-start time
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.daedalus/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.daedalus/skills/`` (including via symlinks) or configured
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Daedalus can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(daedalus, dict):
        daedalus = {}
    return {
        "fallback_for_toolsets": daedalus.get("fallback_for_toolsets", []),
        "requires_toolsets": daedalus.get("requires_toolsets", []),
        "fallback_for_tools": daedalus.get("fallback_for_tools", []),
        "requires_tools": daedalus.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    daedalus = metadata.get("daedalus")
    if not isinstance(daedalus, dict):
        return []
    raw = daedalus.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Daedalus metadata, VCS, virtualenv/dependency, cache, and skill
