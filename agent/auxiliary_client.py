...
Resolution order for text tasks (auto mode):
  ...
  2. Nous Portal (~/.daedalus/auth.json active provider)
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.daedalus/auth.json for an active Nous provider.
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Daedalus auth store."""
    ...
    try:
        from daedalus_cli.auth import _read_codex_tokens
...
def _resolve_forced_provider(forced: str) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if forced == "nous":
        ...
        if client is None:
            logger.warning("auxiliary.provider=nous but Nous Portal not configured (run: daedalus auth)")
        return client, model

    if forced == "codex":
        client, model = _try_codex()
        if client is None:
            logger.warning("auxiliary.provider=codex but no Codex OAuth token found (run: daedalus model)")
        return client, model
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: daedalus auth)")
            return None, None
        final_model = model or default
        return (_to_async_client(client, final_model) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: daedalus model)")
                return None, None
    ...
    try:
        ...
        logger.debug("daedalus_cli.auth not available for provider %s", provider)
        return None, None
