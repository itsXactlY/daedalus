...
def describe_skill_invocation(content: Any, separator: str = " — ") -> Optional[str]:
    ...
    invocation, or ``None`` when *content* is not skill scaffolding.
    """
...
def build_plan_path(
    ...
    *,
    ...
    workspace instead of the Daedalus host's global home directory.
    """
    ...
    return Path(".daedalus") / "plans" / f"{timestamp}-{slug}.md"
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.daedalus.config``
    ...
    try:
        ...
        lines = ["", "[Skill config (from ~/.daedalus/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.daedalus/skills/ and return a mapping of /command -> skill info.
