...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the DAEDALUS_HOME override).
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Daedalus writes
    ...
    ``daedalus_cli.config._parse_env_value``) but never mutates the process
...
def build_profile_secret_scope(daedalus_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(daedalus_home)
    secrets = load_env_file(home / ".env")

    try:
        from daedalus_cli.env_loader import get_secret_source_values
