...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - daedalus_home (str): The active DAEDALUS_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.daedalus``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "daedalus").
    ...
    def history_pointer(self, session_id: str = "") -> str:
        ...
        Soak architecture: every turn is written to the provider (see
        sync_turn), so the full transcript does NOT need to be carried in
        context. Providers that soak the full history should override this
        to return e.g. "history is in <store> under session X, recall on
        demand". Empty string (default) means the provider carries no pointer.
        """

    def continuity_context(self, session_id: str = "") -> str:
        """Return the previous session's soaked content (session-continuity TTL).

        A new session that starts within the provider's TTL window should not
        begin from absolute scratch — this returns the most recent prior
        session's content so the agent can continue. Empty string (default)
        means the provider carries no continuity.
        """
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'daedalus memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], daedalus_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'daedalus memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``daedalus_home`` is the active DAEDALUS_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE DAEDALUS_HOME.

        ``daedalus backup`` only walks DAEDALUS_HOME, so any provider state kept
        ...
        of the archive, and ``daedalus import`` restores them to their original
