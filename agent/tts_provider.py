...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Daedalus with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.daedalus/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Daedalus
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``daedalus tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``daedalus setup`` for
