...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `daedalus update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``daedalus curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``daedalus update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``daedalus curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `daedalus curator run --dry-run` to preview now"
            )
            save_state(state)
...
def _bundle_referenced_skills() -> Set[str]:
    """Skill names referenced by any skill bundle's static membership list.

    Bundles are persisted as ~/.daedalus/skill-bundles/*.yaml with a static
    ``skills:`` list (agent/skill_bundles.py) -- archiving a member skill
    would silently break the bundle (build_bundle_invocation_message drops
    it into ``missing`` at invocation time with no warning to the operator
    until someone actually invokes the bundle). Mirrors
    _cron_referenced_skills()'s same-shape protection for cron jobs.

    Best-effort: a bundle-module import error or corrupt bundle file must
    never break the curator, so any failure yields an empty set (no
    protection, but no crash).
    """
    try:
        from agent.skill_bundles import get_skill_bundles
        names: Set[str] = set()
        for info in get_skill_bundles().values():
            names.update(info.get("skills") or [])
        return names
    except Exception as e:
        logger.debug("Curator could not read bundle skill references: %s", e, exc_info=True)
        return set()
...
def apply_automatic_transitions(now: Optional[datetime] = None) -> Dict[str, int]:
    ...
    bundle_referenced = _bundle_referenced_skills()
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Daedalus' background skill CURATOR. This is an "
    ...
    "into ~/.daedalus/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: daedalus-config-*, daedalus-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.daedalus/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.daedalus/logs/curator/``)
    ...
    authored skill data in ``~/.daedalus/skills/``.

    ``ensure_daedalus_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_daedalus_home() / "logs" / "curator"
    try:
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: daedalus curator status
        keep an umbrella stable: daedalus curator pin document-tools
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: daedalus curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.daedalus/skills/.archive/` for "
            "safety and can be restored via `daedalus curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.daedalus/skills/.archive/`. "
            "Restore any via `daedalus curator restore <name>`._\n"
        )
        SHOW = 50
...
def _render_candidate_list() -> str:
    ...
    for r in rows:
        lines.append(
            ...
            f"bundle={'yes' if r['name'] in bundle_referenced else 'no'}  "
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``daedalus curator run --consolidate`` flag. When consolidation is off, only
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `daedalus update` only if "
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``daedalus model`` →
