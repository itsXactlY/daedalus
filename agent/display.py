...
def safe_json_loads(text: str) -> Any:
    """Parse a JSON string without raising; return None for non-JSON input.

    Local equivalent of the 0.20 ``utils.safe_json_loads`` (which this tree
    does not have yet) — used across display helpers that inspect tool
    results for success/failure without letting malformed payloads crash
...
def build_status_phrase(tool_name: str, args: dict | None, max_len: int = 49) -> str | None:
    ...
    name ("Daedalus is running …"), so it starts lowercase with "is".
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in Daedalus' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("DAEDALUS_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
...
def get_waiting_faces() -> list:
    """Module-level alias for :meth:`KawaiiSpinner.get_waiting_faces`."""
    return KawaiiSpinner.get_waiting_faces()


def get_thinking_faces() -> list:
    """Module-level alias for :meth:`KawaiiSpinner.get_thinking_faces`."""
    return KawaiiSpinner.get_thinking_faces()


def get_thinking_verbs() -> list:
    """Module-level alias for :meth:`KawaiiSpinner.get_thinking_verbs`."""
