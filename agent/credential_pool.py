...
class CredentialPool:
    ...
    def _sync_codex_entry_from_cli(self, entry: PooledCredential) -> PooledCredential:
        ...
        When the Codex CLI (or another Daedalus profile) refreshes its token,
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("daedalus_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("daedalus_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        from agent.anthropic_adapter import read_claude_code_credentials, read_daedalus_oauth_credentials

        for source_name, creds in (
            ("daedalus_pkce", read_daedalus_oauth_credentials()),
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "daedalus_pkce"}
        )
    ]
