...
Storage
-------
Bundles live in ``~/.daedalus/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``DAEDALUS_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``daedalus bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under DAEDALUS_HOME.

    Honors ``DAEDALUS_BUNDLES_DIR`` for tests; falls back to
    ``<DAEDALUS_HOME>/skill-bundles``.
    """
    override = os.environ.get("DAEDALUS_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_daedalus_home() / "skill-bundles"
