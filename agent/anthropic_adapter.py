"""Anthropic Messages API adapter for Daedalus Agent.

Translates between Daedalus's internal OpenAI-style message format and
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Daedalus historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Daedalus)
    ...
    # 1. Daedalus-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Daedalus configs.
    api_key = os.getenv("ANTHROPIC_API_KEY", "").strip()
    if api_key:
        return api_key
...
# ── Daedalus-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.daedalus/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_DAEDALUS_OAUTH_FILE = get_daedalus_home() / ".anthropic_oauth.json"
...
def run_daedalus_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Daedalus-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize Daedalus with your Claude Pro/Max subscription.")
    print()
...
def _save_daedalus_oauth_credentials(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Save OAuth credentials to ~/.daedalus/.anthropic_oauth.json."""
    data = {
        "accessToken": access_token,
        "refreshToken": refresh_token,
        "expiresAt": expires_at_ms,
    ...
    try:
        _DAEDALUS_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
        _DAEDALUS_OAUTH_FILE.write_text(json.dumps(data, indent=2), encoding="utf-8")
        _DAEDALUS_OAUTH_FILE.chmod(0o600)
    except (OSError, IOError) as e:
        logger.debug("Failed to save Daedalus OAuth credentials: %s", e)


def read_daedalus_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Daedalus-managed OAuth credentials from ~/.daedalus/.anthropic_oauth.json."""
    if _DAEDALUS_OAUTH_FILE.exists():
        try:
            data = json.loads(_DAEDALUS_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Daedalus OAuth credentials: %s", e)
    return None
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Daedalus Agent", "Claude Code")
                text = text.replace("Daedalus agent", "Claude Code")
                text = text.replace("daedalus", "claude-code")
