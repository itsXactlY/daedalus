...
class MemoryManager:
    ...
    def provider_names(self) -> List[str]:
        ...
        Dropped in the upstream 0.20.0 rewrite this file was wholesale-adopted
        from; restored because the fork's own test suite and other callers
        (e.g. agent/memory_provider.py's single-provider gating) depend on it
        as a convenience accessor over ``providers``.
        """
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Daedalus expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``daedalus_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_daedalus_home()`` themselves.
        """
        if "daedalus_home" not in kwargs:
            from daedalus_constants import get_daedalus_home
            kwargs["daedalus_home"] = str(get_daedalus_home())
        for provider in self._providers:
            try:
