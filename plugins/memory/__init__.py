...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with daedalus)
...
from daedalus_cli.config import cfg_get
...
def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    ...
    User-installed providers import as ``_daedalus_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_daedalus_user_memory'`` — the
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$DAEDALUS_HOME/plugins/<name>/``) directories.  Bundled takes
