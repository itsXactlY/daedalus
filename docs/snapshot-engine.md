# Runtime Snapshot Engine — Time-Travel for Daedalus

## TL;DR

Daedalus speichert jetzt automatisch seinen eigenen Zustand — nicht als Git-Backup,
sondern als eingebettete Snapshot-Engine mit Content-Addressed Storage.
Du kannst Daedalus jederzeit in einen früheren Zustand zurückversetzen,
...
# ✅ Daedalus ist wieder im alten Zustand
```
...
Daedalus braucht ein Versionskontrollsystem für seinen **Laufzeit-Zustand**.
...
Die Engine snapshottet NUR den internen Zustand von `~/.daedalus`:
...
NICHT gesnapshotted: `daedalus/` (das ist Dev-Code, Git manages das),
...
```
# Daedalus crashed. Beim Neustart:
...
beeinflusst das Daedalus nicht.

---

## Zusammenfassung

Die Runtime Snapshot Engine macht Daedalus zu einem **selbst-heilenden System**:
